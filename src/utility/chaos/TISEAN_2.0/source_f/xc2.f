...
c usage message
 ...
 10      ipairs(i)=0
      ...
      do 20 n=(m-1)*id+1,nmax
         call neigh1(nmax,y,x,n,id,2,jh,jpntr,eps,nlist,nfound)
      ...
      do 10 j=jj-1,jj+1                               ! scan neighbouring boxes
         do 20 k=kk-1,kk+1
            ...
            do 30 ip=jh(jk+1),jh(jk)+1,-1               ! this is in time order
               np=jpntr(ip)
               do 40 i=0,m-1
 40               if(abs(x(n-i*id)-y(np-i*id)).ge.eps) goto 30
               nfound=nfound+1
