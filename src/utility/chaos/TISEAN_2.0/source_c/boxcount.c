...
using a partition instead of the Grassberger-Procaccia scheme"
...
unsigned int column=1,MAXDIM=10,DELAY=1,EPSCOUNT=20;
...
void show_options(char *progname)
{
  ...
  fprintf(stderr,"\t-M maximal embedding dimension [Default: %u]\n",MAXDIM);
  fprintf(stderr,"\t-d delay [Default: %u]\n",DELAY);
  fprintf(stderr,"\t-Q order of the Renyi entropy [Default: %.1f]\n",Q);
  fprintf(stderr,"\t-e minimal epsilon (data is rescaled to [0:1])"
	  " [Default: %.2e]\n",EPSMIN);
  fprintf(stderr,"\t-E maximal epsilon (data is rescaled to [0:1])"
	  " [Default: %.2e]\n",EPSMAX);
...
void scan_options(int n,char **in)
{
  ...
  if ((out=check_option(in,n,'Q','f')) != NULL)
    sscanf(out,"%lf",&Q);
  if ((out=check_option(in,n,'e','f')) != NULL)
...
hliste *make_histo(void)
{
  ...
  element->ptr=NULL;
  check_alloc(element->hist=(double*)malloc(sizeof(double)*MAXDIM));
  for (i=0;i<MAXDIM;i++)
    element->hist[i]=0.0;
...
void next_dim(int dim,int n,unsigned int *first)
{
  int i,which,d1=(dim-1)*DELAY;
  ...
  for (i=0;i<n;i++) {
    which=(int)(series[first[i]+d1]*epsinv);
    hf= ++found[which];
...
int main(int argc,char **argv)
{
  ...
  check_alloc(histo=(double*)malloc(sizeof(double)*MAXDIM));
  check_alloc(deps=(double*)malloc(sizeof(double)*EPSCOUNT));

  histo_el=make_histo();
  root=histo_el;
  
  EPSFAKTOR=pow(EPSMAX/EPSMIN,1.0/(double)EPSCOUNT);
  length=LENGTH-(MAXDIM-1)*DELAY;
