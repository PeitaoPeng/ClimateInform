...
char epsset=0,dimset=0;
char *infile=NULL;
char *outfile=NULL,stdo=1;
char *COLUMNS=NULL;
unsigned int embed=1,dim=2,DELAY=1,MINN=30,STEP=1;
...
void make_fit(long act,unsigned long number,long istep,double **error)
{
  ...
  for (j=0;j<dim;j++) {
    ...
    for (i=0;i<number;i++)
      casted += help[found[i]];
    casted /= number;
    error[j][istep-1] += sqr(casted-help[act]);
  }
}
...
int main(int argc,char **argv)
{
  ...
  if (outfile == NULL) {
    if (!stdi) {
      ...
      sprintf(outfile,"%s.mzr",infile);
    }
    else {
      check_alloc(outfile=(char*)calloc((size_t)10,(size_t)1));
      sprintf(outfile,"stdin.mzr");
    }
  }
  if (!stdo)
  ...
  interval=0.0;
  for (i=0;i<dim;i++) {
    rescale_data(series[i],LENGTH,&min,&hinter);
  ...
  while (!alldone) {
    alldone=1;
    epsilon*=EPSF;
    make_multi_box(series,box,list,LENGTH-STEP,NMAX,(unsigned int)dim,
	...
	actfound=exclude_interval(actfound,i-STEP+1,i+STEP+(embed-1)*DELAY-1,
				  hfound,found);
	if (actfound >= MINN) {
	  for (j=1;j<=STEP;j++)
	    make_fit(i,actfound,j,error);
	  done[i]=1;
	}
	alldone &= done[i];
      }
