      SUBROUTINE MSU22 (AM,AP,BM,BP,FLN,QLNWCS,QDER,
     1                  IMAX,MEND1,NEND1,JEND1,MNWV2,KMAX,S)
      save
C***********************************************************************
C
C     MSU22 : CALCULATES THE SPECTRAL REPRESENTATIONS OF THE HORIZONTAL
C             CONVERGENCE OF PSEUDO-VECTOR FIELDS FROM THE FOURIER
C             REPRESENTATIONS OF THE SYMMETRIC AND ANTI-SYMMETRIC
C             PORTIONS OF THE TWO INDIVIDUAL FIELDS.
C
C***********************************************************************
C
C     MSU22 IS CALLED BY THE SUB GLOOP.
C
C     MSU22 CALLS NO SUBS.
C
C***********************************************************************
C
...
C                                          THE DIVERGENCE OF THE GLOBAL
...
C
      ...
      S(MM        ,1)= AP(MM+1,K)
      S(MM+1      ,1)=-AP(MM  ,K)
      S(MM  +MEND2,1)= AM(MM+1,K)
      S(MM+1+MEND2,1)=-AM(MM  ,K)
  220 CONTINUE
C
      IF(IFAC.GE.2) THEN
      DO 240 ITR=1,IFAC-1
