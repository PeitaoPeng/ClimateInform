...
CC    PARAMETER(NSKPIN=44,NFIELD=51)
      ...
      PARAMETER(NEDDY=2*NMAND,NTIME=90)              
     ...
     1           EDYSRV(2,NMP,NLEG,MAND),EDYGRV(NLONG,NLATG,MAND),
     2           EDYSPS(2,NMP,NLEG,MAND),GLBAVT(NMAND)
      ...
      DIMENSION AVGPTT(NLONG,NLATG,NMAND)
...
C.....modify coriolis to avoid zero at equator
      DO 26 K=47,NLATG
         CORIOLIS(K)=2*OMEGA*SIN(RADG(46))
   26 CONTINUE
...
C.....compute global mean potential temperature
      DO 635 K=1,NMAND
      CALL AVG2D(AVGPTT(1,1,K),GLBAVT(K),39)
  635 CONTINUE     
CC
CC====compute eddy by subtracting mean from daily data
CC
C.....only want eddy stream functon and potential temperature
      IREAD=0
      IUNIT=61
      REWIND IUNIT
      IF(JINITZ .EQ. 1) THEN
...
C.....transform absolute temp to potential temp         
      DO 440 IG=NMAND+1,NEDDY
      DO 440 J=1,NLATG
      DO 440 I=1,NLONG
      EDDYGD(I,J,IG)=EDDYGD(I,J,IG)*POOVRP(IG-NMAND) 
  440 CONTINUE
c.....compute eddy height and potential temp
      DO 450 K=1,NEDDY 
      DO 450 J=1,NLATG
      DO 450 I=1,NLONG
      EDDYGD(I,J,K)=EDDYGD(I,J,K)-AVGGRD(I,J,23+K) 
  450 CONTINUE
C.....transform eddy height into eddy stream function
      DO 430 IG=1,NMAND
      DO 430 J=1,NLATG
      DO 430 I=1,NLONG
      EDDYGD(I,J,IG)=GRAVITY*EDDYGD(I,J,IG)/CORIOLIS(J)
  430 CONTINUE
...
C
  ...
  505 CONTINUE
      DO 510 NT=1,NTIME
CC....compute  spectral UCOS & VCOS
      DO 515 L=1,MAND
      CALL VELOSP(EDYHST(1,1,1,L+1,NT),EDYSPU(1,1,1,L),
     1            EDYSPV(1,1,1,L))
  515 CONTINUE
C 
...
C.....for writing out eddy stream funct of last time step
         CALL EVLSCA(EDYHST(1,1,1,L+1,NT),EDYGDS(1,1,L))
      DO 540 J=1,NLATG
      DO 540 I=1,NLONG
         EDYGDU(I,J,L)=EDYGDU(I,J,L)/COSLAT(J)
         EDYGDV(I,J,L)=EDYGDV(I,J,L)/COSLAT(J)
  540 CONTINUE
  535 CONTINUE
      DO 536 L=1,NMAND
         CALL EVLSCA(EDYHST(1,1,1,L+NMAND,NT),EDYGDT(1,1,L))
  536 CONTINUE
C
...
C
      ...
      DO 555 J=1,NLEG 
      DO 555 I=1,2 
      EDYSPS(I,J,K,L)=EDYHST(I,J,K,L+1,NT)
  555 CONTINUE
      CALL QGPTV
C
...
C.....eddy stream function at last time step
      CALL XWRITE(EDYGDS,JUNIT,LENG3)
      STOP
      END
...
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
     ...
     2           EDYSPS(2,NMP,NLEG,MAND),GLBAVT(NMAND)
      ...
      DO 715 J=1,NLEG 
      DO 715 I=1,2 
      EDYSRV(I,J,K,L)=-(XXN(J,K)+1)*XXN(J,K)*EDYSPS(I,J,K,L)
     1/RR
  715 CONTINUE
      DO 720 L=1,MAND
         CALL EVLSCA(EDYSRV(1,1,1,L),EDYGRV(1,1,L))
  720 CONTINUE
      DO 725 K=1,MAND
      DO 725 J=1,NLATG
      DO 725 I=1,NLONG
