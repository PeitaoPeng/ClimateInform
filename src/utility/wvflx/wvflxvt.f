...
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
     ...
     3           GLBAVT(NMAND)
      ...
      DIMENSION AVU(NLONG,NLATG,NMAND),AVV(NLONG,NLATG,NMAND)
      DIMENSION AVGGDU2(NLONG,NLATG,NMAND)
      ...
      DIMENSION UGRGRQ(NLONG,NLATG,3)
      DIMENSION AVSPTV(2,NLEG,NMP,MAND),XGGRPV(NLONG,NLATG,MAND)
      DIMENSION SGGRPV(2,NLEG,NMP,MAND)
      ...
      DIMENSION AVEDES(NLONG,NLATG,MAND),EVCTOR(NLONG,NLATG,MAND) 
      ...
      DIMENSION XMM(NLONG,NLATG,MAND),DIVAVV(NLONG,NLATG,MAND)
      DIMENSION WORKSP(2,NLEG,NMP,MAND)
      OPEN(10,FILE='~/ppt/gcm1/gtwv87w.hp',FORM='UNFORMATTED',
     +RECORDTYPE='STREAM',CONVERT='BIG_ENDIAN')
      OPEN(60,FILE='/data/pool/ppt/pbhpwv87w',FORM='UNFORMATTED')
C     OPEN(60,FILE='~/ppt/gcm1/pbhpwv87w',FORM='UNFORMATTED')
C     OPEN(61,FILE='test1.dat',FORM='FORMATTED')
...
C.....compute rotational velocity
      DO 885 K=1,NMAND
      CALL RVDVVPST(AVU(1,1,K),AVV(1,1,K),
...
C.....compute div of avg velocity
C     CALL DIVERG2(AVGGDU,AVGGDV,DIVAVV)
C
...
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
      SUBROUTINE GRADSP2(FILDIN,OUTX,OUTY)
...
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
C  3-D divergence over Gausssian grids with input level=7
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
      SUBROUTINE DIVERG2(xin,yin,out)
      parameter(nlong=128,nlatg=102,lin=7,lout=lin-2) 
      parameter(nlongw=nlong+2,nlatgw=nlatg+2) 
      COMMON/GAUSS/RADG(nlatg),GWGT(nlatg)
      dimension xin(nlong,nlatg,lin),yin(nlong,nlatg,lin)
      dimension zin(nlong,nlatg,lin)
      ...
      dimension workx(nlongw,nlatg,lin)
      dimension worky(nlong,nlatgw,lin)
      dimension coslat(nlatg),plevel(7)
      dimension wcos(nlatgw),wlat(nlatgw)
      data plevel/100000.,85000.,70000.,50000.,30000.,20000.,10000./
      hscale=8000.
      r=6.37e+06
...
C     CALL RLVORT(UWN(1,1,L),VWN(1,1,L),AVGDRV(1,1,L))
  100 CONTINUE
      DO 720 L=1,NMAND
