...
C    PROGRAM TO compute the Plumb st wave activity flux using          C
C    2 year pressure history data(U,V & T only)                        C
C                                                                      C
...
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
     ...
     3           GLBAVT(NMAND)
      ...
      COMMON/STWV/AVGGDU(NLONG,NLATG,NMAND),AVGGDV(NLONG,NLATG,NMAND),
     1            AVGGDH(NLONG,NLATG,NMAND),AVGGDT(NLONG,NLATG,NMAND),
     2            XFS(NLONG,NLATG,MAND),YFS(NLONG,NLATG,MAND),
     3            ZFS(NLONG,NLATG,MAND),EDGDT(NLONG,NLATG,NMAND),
     4            EDGDH(NLONG,NLATG,NMAND),EDGDU(NLONG,NLATG,NMAND),
     5            EDGDV(NLONG,NLATG,NMAND),
     6            ZMH(NLATG,NMAND),ZMT(NLATG,NMAND),
     7            ZMU(NLATG,NMAND),ZMV(NLATG,NMAND)
      DIMENSION TOTU(NLONG,NLATG,NMAND),TOTV(NLONG,NLATG,NMAND)
...
C     OPEN(10,FILE='~/ppt/gcm1/amjmn87',FORM='UNFORMATTED',
C    +RECORDTYPE='STREAM',CONVERT='BIG_ENDIAN')
C     OPEN(11,FILE='~/ppt/gcm1/amjmn88',FORM='UNFORMATTED',
C    +RECORDTYPE='STREAM',CONVERT='BIG_ENDIAN')
      OPEN(10,FILE='/data/pool/peng/stwv/gcmdjf79_83',
     +FORM='UNFORMATTED')
      OPEN(11,FILE='/data/pool/peng/stwv/gcmdjf8283',
     +FORM='UNFORMATTED')
      OPEN(60,FILE='/data/pool/peng/stwv/gcmstwv',FORM='UNFORMATTED')
C     OPEN(61,FILE='GRDESX.dat',FORM='FORMATTED')
...
C.....compute rotational velocity
      DO 700 K=1,NMAND
      CALL RVDVVPST(TOTU(1,1,K),TOTV(1,1,K),
     1              AVGGDU(1,1,K),AVGGDV(1,1,K))
  700 CONTINUE
  999 FORMAT(6E12.4)
C.....compute PTT
      DO 850 K=1,NMAND
  ...
  850 CONTINUE
C.....compute deviation PT from global mean
      DO 860 K=1,NMAND
      CALL AVERAGE(AVGPTT(1,1,K),GLBAVT(K),39)
      PRINT 104,GLBAVT(K)
  104 FORMAT('GLBAVT(K)=',f7.1)
  860 CONTINUE
      DO 870 K=1,NMAND
      DO 870 J=1,NLATG
      DO 870 I=1,NLONG
...
C.....stwvlfx
      CALL XWRITE(XFS,JUNIT,LENG3)
      CALL XWRITE(YFS,JUNIT,LENG3)
      CALL XWRITE(ZFS,JUNIT,LENG3)
      IF(IREAD.LT.2) THEN
        IUNIT=11
        GO TO 666
      END IF
...
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
     ...
     6            ZMH(NLATG,NMAND),ZMT(NLATG,NMAND),
     7            ZMU(NLATG,NMAND),ZMV(NLATG,NMAND)
      DIMENSION EDGDS(NLONG,NLATG,NMAND)
      DIMENSION EDSPS(2,NLEG,NMP,NMAND),EDSPU(2,NLEG,NMP,NMAND)
      DIMENSION EDSPV(2,NLEG,NMP,NMAND)
      DIMENSION EDGDUH(NLONG,NLATG,NMAND),EDSPUH(2,NLEG,NMP,NMAND)
      DIMENSION EDGDVH(NLONG,NLATG,NMAND),EDSPVH(2,NLEG,NMP,NMAND)
      DIMENSION EDGDTH(NLONG,NLATG,NMAND),EDSPTH(2,NLEG,NMP,NMAND)
      DIMENSION GRGDTH(NLONG,NLATG,NMAND),GRSPTH(2,NLEG,NMP,NMAND)
      DIMENSION GRGDUH(NLONG,NLATG,NMAND),GRSPUH(2,NLEG,NMP,NMAND)
      DIMENSION GRGDVH(NLONG,NLATG,NMAND),GRSPVH(2,NLEG,NMP,NMAND)
      DIMENSION GLBAVT(NMAND)
      R=6.371E+06
      GASCON=287.
      CP=1004.
      XKAPA=287./1004.       
...
C.....compute zonal mean fild & global avg T
      DO 100 L=1,NMAND
      CALL ZMEAN(AVGGDH(1,1,L),ZMH(1,L))
      CALL ZMEAN(AVGGDT(1,1,L),ZMT(1,L))
      CALL ZMEAN(AVGGDU(1,1,L),ZMU(1,L))
      CALL ZMEAN(AVGGDV(1,1,L),ZMV(1,L))
      CALL AVERAGE(AVGGDT(1,1,L),GLBAVT(L),39)   
  100 CONTINUE
C.....compute eddy fild  
      DO 120 L=1,NMAND
      DO 120 J=1,NLATG
      DO 120 I=1,NLONG
         EDGDH(I,J,L)=AVGGDH(I,J,L)-ZMH(J,L)
         EDGDT(I,J,L)=AVGGDT(I,J,L)-ZMT(J,L)
C=====compute u' & v' from real u & v
         EDGDU(I,J,L)=AVGGDU(I,J,L)-ZMU(J,L)
         EDGDV(I,J,L)=AVGGDV(I,J,L)-ZMV(J,L)
  120 CONTINUE
C
C=====compute Q.G. velocity
C
C.....compute eddy stream function
C     DO 140 L=1,NMAND
C     DO 140 J=1,NLATG
C     DO 140 I=1,NLONG
C        EDGDS(I,J,L)=EDGDH(I,J,L)*GRAVITY/CORIOLIS(J)
C 140 CONTINUE
C.....transform eddy strmfuct to spc space
C     DO 160 L=1,NMAND
C        CALL TRNSFM(EDGDS(1,1,L),EDSPS(1,1,1,L))
C 160 CONTINUE
C.....compute spctral u' and v'              
C     DO 180 L=1,NMAND
C     CALL VELOSP(EDSPS(1,1,1,L),EDSPU(1,1,1,L),EDSPV(1,1,1,L))
C 180 CONTINUE
C.....transform spectral u' and v' to Gaussian grids
C     DO 200 L=1,NMAND
C        CALL EVLSCA(EDSPU(1,1,1,L),EDGDU(1,1,L))
C        CALL EVLSCA(EDSPV(1,1,1,L),EDGDV(1,1,L))
C     DO 220 J=1,NLATG
C     DO 220 I=1,NLONG
C        EDGDU(I,J,L)=EDGDU(I,J,L)/COS(RADG(J))   
C        EDGDV(I,J,L)=EDGDV(I,J,L)/COS(RADG(J))   
C 220 CONTINUE
C 200 CONTINUE
...
C.....compute  zonal gradient of u'h', v'h', T'h'
      ...
      CALL ZNGRSP(EDSPTH(1,1,1,L),GRSPTH(1,1,1,L))
  260 CONTINUE
CC....transform gradients to gaussian grids
      DO 270 L=1,NMAND
         CALL EVLSCA(GRSPUH(1,1,1,L),GRGDUH(1,1,L))
         CALL EVLSCA(GRSPVH(1,1,1,L),GRGDVH(1,1,L))
         CALL EVLSCA(GRSPTH(1,1,1,L),GRGDTH(1,1,L))
      DO 270 J=1,NLATG
      DO 270 I=1,NLONG
         GRGDUH(I,J,L)=GRGDUH(I,J,L)/COS(RADG(J))   
         GRGDVH(I,J,L)=GRGDVH(I,J,L)/COS(RADG(J))   
         GRGDTH(I,J,L)=GRGDTH(I,J,L)/COS(RADG(J))   
  270 CONTINUE
C.....compute  components of wvflx                       
      DO 280 L=1,MAND
      DO 280 J=1,NLATG
      DO 280 I=1,NLONG
      F1=(PLEVEL(L+1)/PLEVEL(1))*COS(RADG(J))
      F2=2.*CORIOLIS(J)
      XFS(I,J,L)=F1*(EDGDV(I,J,L+1)**2-GRGDVH(I,J,L+1)/F2)
      YFS(I,J,L)=F1*(-EDGDU(I,J,L+1)*EDGDV(I,J,L+1)+
     1           GRGDUH(I,J,L+1)/F2)
      SS=(XKAPA*GLBAVT(L+1)-PLEVEL(L+1)*(GLBAVT(L)-
     1   GLBAVT(L+2))/(PLEVEL(L)-PLEVEL(L+2)))/HSCALE
      F3=CORIOLIS(J)/SS
      ZFS(I,J,L)=F1*F3*(EDGDV(I,J,L+1)*EDGDT(I,J,L+1)-
     1           GRGDTH(I,J,L+1)/F2)
  280 CONTINUE
      RETURN
      END
