...
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
     ...
     7            ZMU(NLATG,NMAND),ZMV(NLATG,NMAND)
      ...
      DIMENSION GLBAVH(NMAND),AVGGDS(NLONG,NLATG,NMAND)
      DIMENSION AVGSPS(2,NLEG,NMP,MAND) 
...
C.....Compute the Coriolis Parameter
      OMEGA=2*PI/(24.*3600.)
      DO 35 K=1,NLATG
C        CORIOLIS(K)=2*OMEGA*SIN(RADG(K))
         CORIOLIS(K)=2*OMEGA*SIN(PI/4.)
         COSLAT(K)=COS(RADG(K))
      WRITE(6,*) 'COSLAT=',K,COSLAT(K) 
      WRITE(6,*) 'CORIOLIS=  ',CORIOLIS(K)
   35 CONTINUE
...
C.....compute globaly averaged geoht
      DO 195 L=1,NMAND
      CALL AVERAGE(AVGGDH(1,1,L),GLBAVH(L),39)
      WRITE(6,*) 'GLBAVH= ', GLBAVH(L)
  195 CONTINUE
C.....transform avg geoht into avg strmfct
      DO 200 L=1,NMAND
      DO 200 J=1,NLATG
      DO 200 I=1,NLONG
      AVGGDS(I,J,L)=GRAVITY*(AVGGDH(I,J,L)-GLBAVH(L))/CORIOLIS(J)
  200 CONTINUE
...
C
      JUNIT=60
C.....avg ht
C     CALL XWRITE(AVGGDH,JUNIT,LENG2)
