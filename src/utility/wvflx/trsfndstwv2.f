...
C
      parameter(nlon=128,nlat=102,nlev=2)
      parameter(lonout=48,latout=40)
      DIMENSION R1(nlon,nlat,nlev),fild(nlon,nlat)
      DIMENSION ROUT(lonout,latout),fld2(lonout,latout)
      DIMENSION fld1(nlon,nlat)
      DIMENSION RZ(nlat,nlev)
      OPEN(10,FILE='/data/pool/peng/stwv/ecndstwv',FORM='UNFORMATTED')
C    +RECORDTYPE='STREAM',CONVERT='BIG_ENDIAN')
      OPEN(80,FILE='/data/pool/peng/stwv/ecndstwv15',           
     +FORM='UNFORMATTED',ACCESS='DIRECT',RECL=lonout*latout)
      OPEN(82,FILE='/data/pool/peng/ecnd15',FORM='FORMATTED')
      UNDEF = -2.56E33               
      CALL XSTGRD                                                       GAU00220
      IREC=0
      ndata=0
666   continue
      ndata=ndata+1
C.....read and write 2-D data
      do 100 iread=1,10
      read(10) R1
      ...
      do 120 I=1,nlon
        fild(I,J)=R1(I,J,k)
120    CONTINUE
c     call norsou(fild,fld1,128,102)
      call G40G15(fild,fld2)
      call norsou(fld2,ROUT,48,40)
      IREC=IREC+1
      WRITE (80,REC=IREC) ROUT
      WRITE (6,*) ' NUMBER OF RECORDS = ',IREC
130   continue
100   continue
C.....read and write  zonal mean data
      do 140 iread=1,3 
      read(10) RZ
      WRITE (6,*) ' NUMBER OF READ = ',iread
      do 150 k=1,nlev
