...
C    one year pressure history data(U,V & T only)                      C
C                                                                      C
...
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
     ...
     7            ZMU(NLATG,NMAND),ZMV(NLATG,NMAND)
      ...
      OPEN(10,FILE='~/ppt/gcm1/djfmn8788',FORM='UNFORMATTED',
     +RECORDTYPE='STREAM',CONVERT='BIG_ENDIAN')
      OPEN(60,FILE='~/ppt/gcm1/djfmn878840',FORM='UNFORMATTED')
...
C.....compute deviation PT from global mean
      DO 860 K=1,NMAND
      CALL AVERAGE(AVGPTT(1,1,K),GLBAVT(K),40)
      PRINT 104,GLBAVT(K)
...
C.....compute zonal mean fild & global avg T
      ...
      CALL AVERAGE(AVGGDT(1,1,L),GLBAVT(L),40)   
  100 CONTINUE
C.....compute eddy fild  
      DO 120 L=1,NMAND
      DO 120 J=1,NLATG
