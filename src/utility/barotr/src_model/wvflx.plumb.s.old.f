...
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
     ...
     6            ZMH(NLATG),
     7            ZMU(NLATG),ZMV(NLATG)
      DIMENSION EDGDS(NLONG,NLATG)
      DIMENSION EDSPS(2,NLEG,NMP),EDSPU(2,NLEG,NMP)
      DIMENSION EDSPV(2,NLEG,NMP)
      DIMENSION EDGDUH(NLONG,NLATG),EDSPUH(2,NLEG,NMP)
      DIMENSION EDGDVH(NLONG,NLATG),EDSPVH(2,NLEG,NMP)
      DIMENSION GRGDUH(NLONG,NLATG),GRSPUH(2,NLEG,NMP)
      DIMENSION GRGDVH(NLONG,NLATG),GRSPVH(2,NLEG,NMP)
      R=6.371E+06
...
C.....compute u'h' and v'h' and T'h'
      DO 240 J=1,NLATG
      DO 240 I=1,NLONG
         EDGDUH(I,J)=EDGDU(I,J)*EDGDH(I,J)
         EDGDVH(I,J)=EDGDV(I,J)*EDGDH(I,J)
  240 CONTINUE
         CALL transform15(EDGDUH,EDSPUH)
         CALL transform15(EDGDVH,EDSPVH)
C.....compute  zonal gradient of u'h', v'h'        
      CALL ZNGRSP(EDSPUH,GRSPUH)
      CALL ZNGRSP(EDSPVH,GRSPVH)
CC....transform gradients to gaussian grids
         CALL EVLSCA(GRSPUH,GRGDUH)
         CALL EVLSCA(GRSPVH,GRGDVH)
      DO 270 J=1,NLATG
      DO 270 I=1,NLONG
         GRGDUH(I,J)=GRGDUH(I,J)/COS(RADG(J))   
         GRGDVH(I,J)=GRGDVH(I,J)/COS(RADG(J))   
  270 CONTINUE
C.....compute  components of wvflx                       
      ...
      F1=COS(RADG(J))
      F2=2               
      XFS(I,J)=F1*(EDGDV(I,J)**2-GRGDVH(I,J)/F2)
      YFS(I,J)=F1*(-EDGDU(I,J)*EDGDV(I,J)+
     1           GRGDUH(I,J)/F2)
  280 CONTINUE
      RETURN
      END
