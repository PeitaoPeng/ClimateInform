...
C     CALL SETXY(XX, imx, YY ,jmx, 0.0, DXX, -88., DYY) !jmx=89
...
c     write(6,*) grd2
      ...
      subroutine shift(fld1,fld2,imx,jmx)
      real fld1(imx,jmx),fld2(imx,jmx)
      imxh=imx/2
      do i=1,imxh
      do j=1,jmx
        fld2(i,j)=fld1(imxh+i,j)/9.8
        fld2(i+imxh,j)=fld1(i,j)/9.8
      end do
      end do
      return
      end
