...
c  For the higher modes, the data read-in are from remp.dat
c  which is the reminder after one more mode is extracted
c
c     open(13,file="remp.dat",form="formatted",status="old")
c============================================================
c
...
c  Read in the original data
      call read_origin(ximf)
c
c  or read in the remainder after low order modes are extracted
c     read(13,201) (ximf(i),i=1,LEX)
c     close(13)
c
...
c
      MM=2*LXY
      do i=LXY+1,MM
        write(12,200) remp(i),spmax(i),ximf(i),yimf(i-LXY),
     |                omega(i-LXY),rem(i),spmin(i),ave(i)
      enddo
 200  format(10f10.5)

      write(15,201) (rem(i),i=1,Lex)
 201  format(10f10.6)
...
C*******************  read_origin  *******************************
c
      subroutine read_origin(ximf)
c
c  Read in the original time series. In this case it is
c  tavegl.dat, the normalized Southern Oscillation Index. This
c  set of data is used to calculate mode 1 of the SOI index.
c
      parameter(LXY=1584,LEX=LXY*3)
c
      dimension tavegl(LXY)      ! read-in original time series
      dimension ximf(LEX)     ! The IMF for the extended
      dimension temp(12)
      dimension mtmp(12)
      dimension data1(200)    ! the first 200 value of tavegl
      dimension data2(200)    ! the last 200 value of tavegl
      real rightRef           ! the right reference value
      real leftRef            ! the left reference value
      real tmp, a, b
c
      open(12,file="tavegl.dat",form="formatted",status="old")
c
      do i=1,132
        k=12*(i-1)
        read(12,100) myear, (tavegl(k+j),j=1,12), tmp
        read(12,101) myear, (mtmp(kk),kk=1,12)
      enddo
 100  format(i5,13f7.2)
 101  format(i5,12i7)
      close(12)
...
c
      do i = 1, 200
        data1(i) = tavegl(i)
        data2(i) = tavegl(LXY-200+i)
      enddo

c     call regression(200, data1, a, b, leftRef, tmp)
c     call regression(200, data2, a, b, tmp, rightRef)
      call cubic_fit(200, data1, leftRef, tmp)
      call cubic_fit(200, data2, tmp, rightRef)
...
c
      do i=1,1000
        ximf(i)=(ximf(i)-afront)*float(i)/1000.0
        ximf(i)=ximf(i)+afront
      enddo

      is = N - 1000 + 1
      do i=is,N
        ximf(i)=(ximf(i)-aend)*float(N-i+1)/1000.0
        ximf(i)=ximf(i)+aend
      enddo
