      ...
      do ld=1,nld 
        ...
        do j=1,jmx
          if(f2d(i,j).gt.-900.) then
            ...
            w2d2(i,j)=w2d2(i,j)+wts(i,j,ld,m,1)*w5d(ld,it,m,i,j)
            w2d3(i,j)=w2d3(i,j)+wts(i,j,ld,m,2)*w5d(ld,it,m,i,j)
            w2d4(i,j)=w2d4(i,j)+wts(i,j,ld,m,3)*w5d(ld,it,m,i,j)
            enddo
          else
            w2d(i,j)=undef
            w2d2(i,j)=undef
...
c
      ...
      do ld=1,nld
        ...
        do j=1,jmx
          if(f2d(i,j).gt.-900.) then
          aavef(iv,ld)=aavef(iv,ld)+coslat(j)*stdf(i,j,iv,ld)/area
          endif
        enddo
        enddo
      ...
      enddo
      write(6,*) 'area=',area
      write(6,*) 'avgo=',avgo(180,90,16)
      write(6,*) 'avgf=',avgf(180,90,4,16)
      write(6,*) 'stdo=',stdo(180,90,16)
      write(6,*) 'stdf=',stdf(180,90,4,16)
c     write(6,*) 'stdf=',stdf
      write(6,*) 'aaveo=',aaveo
      write(6,*) 'aavef=',aavef
c
