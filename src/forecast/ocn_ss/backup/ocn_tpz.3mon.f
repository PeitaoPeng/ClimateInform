...
c
      if(fld(i,j).gt.-900.) then
        ...
        enddo
        call cor_rms(ts1,ts2,nyr,ny_tpz-30,cor3d(i,j,ld),rms3d(i,j,ld))
c       call hss3c_t(ts1,ts2,nyr,ny_skill,hss3d(i,j,ld))
      else
        cor3d(i,j,ld)=undef
        rms3d(i,j,ld)=undef
c       hss3d(i,j,ld)=undef
      endif
      enddo
      enddo
...
c     call hss3c_s(w2d,w2d2,imx,jmx,1,360,115,160,coslat,h1)
c     call hss3c_s(w2d,w2d2,imx,jmx,230,300,115,140,coslat,h2)
