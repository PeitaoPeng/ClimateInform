...
dataot1=/home/ppeng/data/svd_prd
#
var1=ersst
...
ncv=1
id=0   # =0 cov mtx; =1 for cor mtx 
#
# svd modes kept
msvd=13 # NA=10, NHML=13
#
# grid of var1 & var2
imx=180
jmx=89
...
nclm_start=1955 # to have yrs clm for more stable than 30 yrs 
...
outd=/home/ppeng/data/ss_fcst/svd/$icyr
outdata=${outd}/$icmon
if [ ! -d $outd ] ; then
  mkdir -p $outd
fi
...
if [ $tpz_area = na ]; then ngrdv2=1456; fi
...
#=======================================
#
cp $lcdir/grd_svd_ersst_2_tpz.test.f $tmp/svd.f
cp $lcdir/backup/reof.s.f $tmp/reof.s.f
cp $lcdir/backup/svd.s.f $tmp/svd.s.f

for var2 in hgt; do # prec, t2m, hgt
...
eof
#
#gfortran -o svd.x pcr.f reof.s.f
#gfortran -mcmodel=large -o svd.x svd.f reof.s.f
gfortran -mcmodel=medium -o svd.x svd.f svd.s.f reof.s.f
echo "done compiling"
...
#
outfile3=fcst.$var1.2.$var2.mlag$lagmax.mlead$mlead.msvd$msvd.cv$ncv.tpz_area_${tpz_area}.3mon
outfile4=skill_1d.$var1.2.$var2.mlag$lagmax.mlead$mlead.msvd$msvd.cv$ncv.tpz_area_${tpz_area}.3mon
outfile5=hcst.$var1.2.$var2.mlag$lagmax.mlead$mlead.msvd$msvd.tpz_area_${tpz_area}.cv$ncv.3mon

outfile6=grd_svd_$var1.msvd$msvd.tpz_area_${tpz_area}.cv$ncv.3mon
outfile7=grd_svd_$var2.msvd$msvd.tpz_area_${tpz_area}.cv$ncv.3mon
...
#
./svd.x > $dataot2/grd_svd_test.$var1.2.$var2.mlag$lagmax.mlead$mlead.out
#
...
ydef $jmx2 linear $yds $xydel
zdef  1 linear 1 1
tdef $msvd linear ${tgtmon}$outyr_s 1yr
vars  2
xx    1 99 x to x 
xy    1 99 y to x
endvars
