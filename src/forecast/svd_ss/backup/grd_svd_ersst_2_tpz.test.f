...
c     its_sst=icmon-2+lagmax-ilag+1 ! for 3-mon sst 11-2+12-1+1=21(son)
      its_sst=icmon-2+12-ilag+1 ! for 3-mon sst 11-2+12-1+1=21(son)
...
c     its_tpz=icmon+12+ld+1 !11+12+1+1=25(jfm)
      its_tpz=icmon-2+12+ld+1 !11+12+1+1=25(jfm)
c     its_tpz=icmon-2+12 !test same as sst
      ir=0
...
c have lead-ld hcst for itgt yr with SVD
      ...
      enddo
C
      call grd_svd(w3dv1,w3dv2,nyr,nfld,imx,jmx,imx2,jmx2,isv1,
     &iev1,jsv1,jev1,isv2,iev2,jsv2,jev2,id1,jd1,id2,jd2,ng1,ng2,
     &cosr,cosr2,msvd,cof1,cof2,cic,undef,id)
c
c normalize cof & cic
...
c regression of cof1 to tpz
      ...
      call regr_t(ts1,ts2,nyr,nfld,corr12(i,j,m),regr12(i,j,m))
...
c
     ...
     &iev1,jsv1,jev1,isv2,iev2,jsv2,jev2,id1,jd1,id2,jd2,ng1,ng2,
     &cosr1,cosr2,nmod,cof1,cof2,cic,undef,id)
...
c have cic, the projection of var1_ic to u
      ...
      do j=jsv1,jev1,jd1
        ...
        ngd3=ngd3+1
        if(id.eq.1) then
        aic(ngd3)=wic(i,j)*cosr1(j)/std1(i,j)
        else
        aic(ngd3)=wic(i,j)*cosr1(j)
        endif
        endif
      enddo
      enddo
