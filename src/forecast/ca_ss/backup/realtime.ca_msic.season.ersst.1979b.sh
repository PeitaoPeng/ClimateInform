...
#
yrn2=`expr $curyr - 1979` # total full years,=44 for 1979-2022
if [ $cmon = 1 ]; then yrn2=`expr $yyym - 1979`; fi
mmn2=`expr $yrn2 \* 12`
#
tmax=`expr $mmn2 + $icmoe` # 816=dec2015
#
nyear=`expr $curyr - 1979`  # total full year data used for CA, 44 for 1979-2022
...
#
outfilem=${sst_analysis}.mon.jan1979-curr
cat >sstmon<<EOF
run mon.gs
EOF
cat >mon.gs<<EOF
...
outfile=${sst_analysis}.3mon.1979-curr
nts=2
nte=`expr $tmax - 1`
cat >sst3monavg<<EOF
run avg.gs
...
*anom wrt clim from 1979 to curr
...
#
cp $lcdir/realtime.ca_msic.season.sst.1979b.f $tmp/sst_prd.f
...
ln -s $datadir/${sst_analysis}.3mon.1979-curr.1x1.gr fort.11
#
