...
c     parameter(ntt2=(nseason-32))
      parameter(ntt2=(nseason-1))
...
c       do nmode=1,modemax-1
        ...
        enddo
c         efld2d(modemax,ic)=40*xn34ic(ic)
c
...
c     go to 888
c
c* 3-yr cross validated(CV-3) prd for having statistics
...
c
      ...
      do j=1,jms
      if(abs(fld4d2(i,j,1,nseason)).lt.999) then
c       do is=1,nseason-32
        do is=1,nseason-1
          ts1(is)=fld4d2(i,j,ldpen,is+1)
          ts2(is)=sstprd4d(i,j,is+1,ldpen)
        enddo
...
c write out real time prd
      ...
      enddo
      iwr1=iwr1+1
      call screen_big(prdsst,ims,jms,100.,undef1)
      write(85,rec=iwr1) prdsst
      iwr1=iwr1+1
      write(85,rec=iwr1) wk2d
      iwr1=iwr1+1
      call screen_big(wk2d2,ims,jms,100.,undef1)
      write(85,rec=iwr1) wk2d2
      iwr1=iwr1+1
      write(85,rec=iwr1) wk2d3
      iwr1=iwr1+1
      call screen_big(wk2d4,ims,jms,100.,undef1)
...
c
      ...
      subroutine screen_big(f,imx,jmx,crt,undef)
      dimension f(imx,jmx)
      do i=1,imx
      do j=1,jmx
        if(abs(f(i,j)).gt.crt) then
        f(i,j)=undef
      endif
      enddo
      enddo
...
c
      ...
      dl=0.
      do ib1=1,n
      dl=dl+a(ib1,ib1)/float(n)
      enddo
      do ib1=1,n
       a(ib1,ib1)=a(ib1,ib1)+dl*ridge
      enddo
