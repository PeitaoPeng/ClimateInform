...
C===========================================================
      ...
      real rpc(mrpc,nyr),pc(nyr,mpls),pco(mpls)
      real pt1(imx,jmx,mpls),pt2(imx,jmx,mpls)
      real xcorr(mpls),xvar(mpls)
...
c     its_sst=icmon-2+lagmax-ilag+1 ! for 3-mon sst 11-2+12-1+1=21(son)
      its_sst=icmon-2-ilag+1 ! for 3-mon sst 11-2+12-1+1=21(son)
c     its_sst=icmon+2-ilag+1 ! sst and tpz in the same season
...
c have lead-ld hcst for itgt yr with PLS
      ...
      call reof_rpc(w3dv2,cosr2,nyr,nfld,ng2,mrpc,undef,
...
c hindcast var2 on grids
      ...
      if(fld2(i,j).gt.undef) then
        do m=1,mrpc
          w2d(i,j)=w2d(i,j)+rpcf(m,itgt)*regv2(i,j,m)
        enddo
