...
c     its_sst=icmon+12-2-ilag+1 ! for 3-mon sst 11-2+12-1+1=21(son)
...
c     its_tpz=icmon+12+ld+1 !11+1+1=13(jfm)
...
c
      ...
      enddo
C
c     call reof_rpc(w3dv2,cosr2,nyr,ny_tpz,ng2,mrpc,undef,
c    &imx2,jmx2,id2,jd2,isv2,iev2,jsv2,jev2,rpc,corv2,regv2,ideof)
c
c fcst var2 
      iw1=0 ! pls patterns
      iw2=0 ! reof patterns
...
c forecast var2 on grids
      ...
      if(fld2(i,j).gt.undef) then
        do m=1,mrpc
c         w2d(i,j)=w2d(i,j)+mcho(m)*rpcf(m,ny_sst)*regv2(i,j,m)
          w2d(i,j)=w2d(i,j)+mcho(m)*rpcf2(m,krpc(m),ny_sst)*regv2(i,j,m)
        enddo
      else
          w2d(i,j)=undef
      endif
