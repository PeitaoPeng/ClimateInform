...
c regression of cof2 to sst & tpz
        do it=1,ny_tpz
          ts1(it)=cof2(m,it)
        enddo
        ...
        if(fld(i,j).gt.undef) then
          ...
          call regr_t(ts1,ts2,nyr,ny_tpz,corr21(i,j,m),regr21(i,j,m))
...
c regression of cof1 to sst & tpz
        do it=1,ny_tpz
          ts1(it)=cof1(m,it)
        enddo
        ...
        if(fld(i,j).gt.undef) then
          ...
          call regr_t(ts1,ts2,nyr,ny_tpz,corr11(i,j,m),regr11(i,j,m))
