...
C===========================================================
      ...
      real cof1(mv1,nyr),cof2(mv1,nyr)
      real cic(mv1),sdc1(mv1),sdc2(mv1)
      ...
      real corr12(mv2,mv1),regr12(mv2,mv1)
      real corr11(mv1,mv1),regr11(mv1,mv1)

      real corr21(mv1,mv1),regr21(mv1,mv1)
      real corr22(mv2,mv1),regr22(mv2,mv1)
...
C
      ...
      open(41,form='unformatted',access='direct',recl=4*imx2*jmx2) !eof
...
c     its_sst=icmon-2+lagmax-ilag+1 ! for 3-mon sst 11-2+12-1+1=21(son)
      its_sst=icmon+12-2-ilag+1 !+12 to avoid < 0
...
c
c have lead-ld hcst for itgt yr with MLR

      do m=1,mv2 !loop over mode
c
        do it=1,nfld
          ts1(it)=pcv2(m,it)
        enddo
        ...
        call mlr_wts(ts1,pcv1,wt,nyr,nfld,mv1,rdg)
        ...
        prdpcv2(m,itgt)=0
        do n=1,mv1
          prdpcv2(m,itgt)=prdpcv2(m,itgt)+wt(n)*tcof(n,itgt)
        enddo
...
c
        ...
        call mlr_wts(ts1,pcv1,wt,nyr,ny_tpz,mv1,rdg)
        ...
        prdpcv2(m,ic_fcst)=0
        do n=1,mv1
          prdpcv2(m,ic_fcst)=prdpcv2(m,ic)+wt(n)*tcof(n,ic_fcst)
        enddo
...
c corr of svd cof1 vs cof2
      do m=1,mv2
        do it=1,ny_tpz
          ...
          ts2(it) = prdpcv2(m,it)
        enddo
          call cor_rms(ts1,ts2,nyr,ny_tpz,cor1d(m),rms)
      enddo

      write(6,*) 'prdpcv2_vs_pcv2'
      write(6,101) cor1d
...
c select grid data
      ...
      print *, 'ngrd=',ig
C EOF analysis
c     call eofs(aaa,ng,nt,nt,eval,evec,coef,wk,id)
...
c
      ...
      subroutine mlr_wts(tso,pc,wt,nyr,nfld,nmod,rdg)
      dimension tso(nyr),pc(nmod,nyr)
      dimension tsw(nfld),pcw(nmod,nfld)
      dimension wt(nmod)
      ...
      do it=1,nfld
        ...
        enddo
      enddo
      call get_mlr_wt(tsw,pcw,wt,nmod,nfld,rdg)
