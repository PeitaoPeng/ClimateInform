...
C============ loop over ilag
c
      DO ilag=1,lagmax ! =1 for lag=0
      its_sst=icmon+lagmax-ilag+1
      ir=0
      do it=its_sst,montot,12
c     do it=its_sst,montot+12,12
        read(10,rec=it) fld
        ir=ir+1
        do i=1,imx
        do j=1,jmx
...
c select tpz for each lead
      its_tpz=icmon+lagmax+ld+1 !icmon=1,ld=1->its_tpz=3(mam)
      ir=0
      ...
      do j=1,jmx2
        if(fld2(i,j).gt.-900.) then
          do m=1,ny_sst-2
           ...
           write(6,*) 'ilag=',ilag,'m=', m,'tcof(ic_fcst)=',
     &     tcof(m,ic_fcst),'tcof(ic_fcst-1)=',tcof(m,ic_fcst-1)
           endif
...
C== have weights for each ld and lag
     ...
     & ac(275,125,ic_fcst-1,ld,12)
      write(6,*) 'ac(ic_fcst)=',ac(275,125,ic_fcst,ld,1),
     & ac(275,125,ic_fcst,ld,12)
...
c       w2d(i,j)=w2d(i,j)+wts(i,j,ic_fcst-1,ld,ilag)*fcst(i,j,ld,ilag)
          enddo
