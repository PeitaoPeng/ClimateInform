...
C PCR to forecst TPZ of recent seasons for checking real skill
C===========================================================
...
c     its_sst=icmon+lagmax-ilag+1 !11+12-1+1=23(nov)
      its_sst=icmon-2+lagmax-ilag+1 !11-2+12-1+1=21(son)
      ir=0
...
c
      ...
      enddo
C
      call pc_eof(sst,imx,jmx,lons,lone,lats,late,nyr,ny_sst,cosr,
     &ngrd,ny_sst,tcof,corr,regr,undef,id)
c
C write out PC and EOF patterns for ilag=1
c     if(ilag.eq.1) then
      if(ilag.eq.lagmaxm) then
      iw=0
      iw2=0
      do m=1,ny_sst
...
c     nfld=ny_tpz - 1 ! skip itgt for regression
c     nfld=ny_tpz - 2 ! skip itgt and ny_tpz for regr
...
c         if(iy.eq.ny_tpz) go to 666
  ...
  666     continue
      ...
      enddo ! m loop
c
c have ld-lead hcst for itgt yr with sst tcof and tpz regr
      call setzero(w2d,imx2,jmx2)
      do i=1,imx2
      do j=1,jmx2
        if(fld2(i,j).gt.-900.) then
          do m=1,ny_sst
            w2d(i,j)=w2d(i,j)+tcof(m,itgt)*regr2(i,j,m,itgt)
          enddo
...
c ac for ic_fcst
      ...
      if(fld2(i,j).gt.-900.) then
        ...
        ir=ir+1
        enddo
        call ac_rms(ts1,ts2,nyr,ir,ac(i,j,ic_fcst,ld,ilag),xrms)
      else
...
c         do m=1,ny_sst-2
      ...
      write(6,*) 'lag=',ilag,'ld=',ld,'ic_fcst=',ic_fcst
      write(6,*) 'tcof(m=1)=',tcof(1,ic_fcst-1),tcof(1,ic_fcst)
      write(6,*) 'tcof(m=2)=',tcof(2,ic_fcst-1),tcof(2,ic_fcst)
      write(6,*) 'regr2=',regr2(275,125,1,ny_tpz),
     & regr2(275,125,20,ny_tpz)
      write(6,*) 'regr3=',regr3(275,125,1),regr3(275,125,20)
...
C== have weights for each ld and lag
      ...
      write(6,*) 'ac((275,125,it,ld,ilag=1,12)=',ac(275,125,it,ld,1),
     & ac(275,125,it,ld,12)

      enddo ! it loop

      write(6,*) 'ac(ic_fcst-1)=',ac(275,125,ic_fcst-1,ld,1),
     & ac(275,125,ic_fcst-1,ld,11)
      write(6,*) 'ac(ic_fcst)=',ac(275,125,ic_fcst,ld,1),
     & ac(275,125,ic_fcst,ld,11)
