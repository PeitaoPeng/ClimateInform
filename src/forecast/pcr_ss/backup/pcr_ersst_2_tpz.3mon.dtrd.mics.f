...
C===========================================================
      ...
      real ac(imx2,jmx2,nyr,nlead,mics)
      ...
      real hcst(imx2,jmx2,nyr,nlead,mics)
      real wthcst(imx2,jmx2,nyr,nlead)
      real fcst(imx2,jmx2,nlead,mics)
      real wtd(imx2,jmx2,nyr,nlead),wts(imx2,jmx2,nyr,nlead,mics)
...
c
      ...
      if(id_mics.eq.0) then
      its_sst=icmon+lagmax-2-ic+1-(ic-1)*2 !for 3-mon data,+lagmax to avoid < 0
      else
      its_sst=icmon+lagmax-2-ic+1 !for over lapped 3-mon ics
      endif
...
C
     ...
     &ny_sst,cosr,ngrd,nmod,tcof,undef,id,mics,id_eof)
c
...
c       hcst(i,j,itgt,ld,ic)=w2d(i,j)
        hcst(i,j,itgt,ld,ic)=w2d(i,j)+v2trd(i,j,itgt)
...
c==temporal ac skill for hcst of ld and ic
      ...
      if(fld2(i,j).gt.-900.) then
        ...
        do iy=1,ny_tpz
          ...
          ir=ir+1
            ...
            ts2(ir)=hcst(i,j,iy,ld,ic)
  777   continue
        enddo

        call ac_rms(ts1,ts2,nyr,nfld,ac(i,j,itgt,ld,ic),xrms)

      else
        ac(i,j,itgt,ld,ic)=undef
      endif
...
c ac for ic_fcst
      ...
      if(fld2(i,j).gt.-900.) then
        do it=1,ny_tpz
            ...
            ts2(it)=hcst(i,j,it,ld,ic)
        enddo
        call ac_rms(ts1,ts2,nyr,ny_tpz,ac(i,j,ic_fcst,ld,ic),xrms)
      else
        ac(i,j,ic_fcst,ld,ic)=undef
      endif
...
c     call setzero_3d(regr3,imx,jmx,nmod)
c
c fcst
      call setzero(w2d,imx2,jmx2)
      do i=1,imx2
...
c       fcst(i,j,ld,ic)=w2d(i,j)
        fcst(i,j,ld,ic)=w2d(i,j)+v2trd(i,j,ny_tpz)
...
C== have denominator of the weights
      ...
      do j=1,jmx2
        if(fld2(i,j).gt.-900.) then
          wtd(i,j,it,ld)=0
          do ic=1,mics
            if(ac(i,j,it,ld,ic).gt.0.) then
              wtd(i,j,it,ld)=wtd(i,j,it,ld)+ac(i,j,it,ld,ic)**2
            else
...
C== have weights for each ld and lag
      do ic=1,mics
        ...
        do j=1,jmx2
          if(fld2(i,j).gt.-900.) then
            if(ac(i,j,it,ld,ic).gt.0.) then
              wts(i,j,it,ld,ic)=ac(i,j,it,ld,ic)**2/wtd(i,j,it,ld)
            else
              wts(i,j,it,ld,ic)=0.
            endif
          else
            wts(i,j,it,ld,ic)=undef
          endif
        enddo
        enddo
      enddo
      ...
      do it=1,ny_tpz
        ...
        do j=1,jmx2
          if(fld2(i,j).gt.-900.) then
            ...
            w2d(i,j)=0.
            do ic=1,mics
c           w2d(i,j)=w2d(i,j)+wts(i,j,it,ld,ic)*hcst(i,j,it,ld,ic)
            w2d(i,j)=w2d(i,j)+hcst(i,j,it,ld,ic)/float(mics)
            enddo
...
C== have ensemble fcst
        ...
        do j=1,jmx2
          ...
          w2d(i,j)=0.
          do ic=1,mics
          w2d(i,j)=w2d(i,j)+wts(i,j,ic_fcst,ld,ic)*fcst(i,j,ld,ic)
          enddo
...
c
     ...
     &ngrd,nmod,tcof,undef,id,mics,id_eof)
...
c
      call eofs_mics(aaa,ngrd*mics,ng,nfld,coef,nmod,id)
c
c normalize coef
      do m=1, nmod
      do it=1,nfld
...
c
      ...
      SUBROUTINE eofs_mics(aaa,ngbig,ng,nfld,coef,nmod,id)
