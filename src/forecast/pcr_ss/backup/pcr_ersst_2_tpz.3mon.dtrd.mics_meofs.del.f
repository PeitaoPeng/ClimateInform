...
C===========================================================
      ...
      real wtpzm(imx2,jmx2,nyr),wtpzo(imx2,jmx2,nyr)
...
c     its_tpz=icmon+lagmax+ld+1 !11+1+1=13(jfm)
      its_tpz=icmon+lagmax+ld+1-12 !backward 12 mon for del
      ir=0
...
c         do it=1,ny_tpz
          do it=1,ny_tpzp
            ts2(it)=wtpz(i,j,it)
          enddo
          call clim_tot(ts2,tpzc_tot(i,j,ld),nyr,1,ny_tpzp)
          call clim_anom_del(ts2,tpzc(i,j),nyr,2,ny_tpzp)
        else
          do it=1,ny_tpz
            ts2(it)=undef
          enddo
          ...
          do it=1,ny_tpz
            wtpz(i,j,it)=ts2(it+1)-ts2(it)
            wtpzm(i,j,it)=ts2(it)
          enddo
          do it=2,ny_tpzp
            wtpzo(i,j,it-1)=ts2(it)
          enddo
...
c have lead-ld hcst for itgt yr with sst tcof and tpz regr
      ...
      if(id_detrd.eq.1) then
        hcst(i,j,itgt,ld,ic,mc)=w2d(i,j)+v2trd(i,j,itgt)+wtpzm(i,j,itgt)
      else
        hcst(i,j,itgt,ld,ic,mc)=w2d(i,j)+wtpzm(i,j,itgt)
      endif
...
c fcst
      ...
      if(id_detrd.eq.1) then
        fcst(i,j,ld,ic,mc)=w2d(i,j)+v2trd(i,j,ny_tpz)+wtpzm(i,j,ny_tpz)
      else
        fcst(i,j,ld,ic,mc)=w2d(i,j)+wtpzm(i,j,ny_tpz)
      endif
...
c
      ...
      SUBROUTINE clim_anom_del(ts,cc,maxt,is,nt)
      DIMENSION ts(maxt)
      cc=0.
      do i=is,nt
         cc=cc+ts(i)
