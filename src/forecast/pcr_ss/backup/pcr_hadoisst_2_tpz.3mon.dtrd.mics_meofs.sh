...
id_ceof=0 # =1: eofs of combind ics; =0 not combined
...
if [ $eof_area = nhml ]; then lons=1;lone=360;lats=110;late=160; fi # 20N-70N
if [ $eof_area = tp_nml ]; then lons=1;lone=360;lats=61;late=150; fi # 20N-70N
if [ $var1 = hgt ] && [ $eof_area = nhml ]; then ngrd=18360; fi
if [ $var1 = hadoisst ] && [ $eof_area = tp_nml ]; then ngrd=21121; fi
echo $ngrd
#
var1file=${var1}.3mon.1948-curr.total.1x1
#
#=======================================
#
cp $lcdir/pcr_z500_2_tpz.3mon.dtrd.mics_meofs.f $tmp/pcr.f
...
c
      ...
      parameter(modmax=$modmax,ncut=$ncut)
c
      parameter(ncv=$ncv)
c
      parameter(nyr=$nyear)
      parameter(mcut1=$mcut1,mcut2=$mcut2,mcut3=$mcut3,mcut4=$mcut4)
...
outfile5=hcst.$var1.2.$var2.mics$mics.mlead$mlead.id_ceof${id_ceof}.ncut$ncut.id_detrd$id_detrd.cv$ncv.3mon
