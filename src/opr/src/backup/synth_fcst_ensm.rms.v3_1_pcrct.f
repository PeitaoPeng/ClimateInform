...
C===========================================================
      ...
      real clm_o(imx,jmx),std_o(imx,jmx)
      real clm_f(imx,jmx),std_f(imx,jmx)
      real rpss(imx,jmx)
...
C=== have wts from cvrms for hcst
      do i=1,imx
      do j=1,jmx
...
c wmo std & clm of hcst
        do i=1,imx
        do j=1,jmx
          if (w2d(i,j).gt.-900.) then

            do it=1,ny_hcst
c             ts1(it)=ehcst(i,j,it)
            enddo

c           call wmo_clm_std_anom(ts1,stdw,clmw,ts4,ny_hcst,ny_hcst,
c    &nwmo)
c           clm_f(i,j)=stdw(nwmo)
c           std_f(i,j)=stdw(nwmo)

            do it=1,ny_hcst
c             ehcst(i,j,it)=ts4(it)
            enddo

          else

            do it=1,ny_hcst
c             ehcst(i,j,it)=undef
            enddo
...
c have standardized eprd
        do i=1,imx
        do j=1,jmx
          if (w2d(i,j).gt.-900.) then
c           eprd(i,j)=(eprd(i,j)-clm_f(i,j))/std_f(i,j)
            eprd(i,j)=eprd(i,j)
          endif
        enddo
        enddo
...
C read in frac, std_o, std_f, avgo, avgf, rpss
      ...
      ir2=ld*6-2
      read(23,rec=ir2) clm_o ! of anom w.r.t. WMO clm
      ir2=ld*6-1
      read(23,rec=ir2) clm_f
      ir2=ld*6
...
c         ts1(it-nskip_y)=obs(i,j,it)/ostd(i,j,it)
