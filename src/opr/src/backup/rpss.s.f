...
c
      ...
      open(unit=60,form='formatted')  !txt file of rpss time series
c
cc have coslat
      dl=2.
      do j=1,jmx
...
c have rps
      do it=1,nss
        ...
        IF(mask(i,j).eq.1) then
          ...
          o3(i,j)=1.
          rps(i,j,it)=(y1(i,j)-o1(i,j))**2 !rps
     &+(y2(i,j)-o2(i,j))**2
          rpsc(i,j,it)=(1./3.-o1(i,j))**2 !rpsc
     &+(2./3.-o2(i,j))**2
        END IF
        enddo
        enddo
...
c
      ...
      enddo
cc formated
  777 format(I4,I2,10x,f7.2)
  888 format(I4,I1,I1,10x,f7.2)
  999 format(12x,A4,f7.2)
      write(60,*) 'Target season','     RPSS'
      it=0
      do iy=1995,2050
      do im=1,12
      iyr=iyr+1
      it=it+1
      if(it.gt.nss) go to 100
      if(im.gt.9) then
      write(60,777) iy,im,rpss_s(it)
      endif
      if(im.le.9) then
      write(60,888) iy,0,im,rpss_s(it)
      endif
      enddo
      enddo
 100  continue
      avg=0.
      do it=1,nss
        avg=avg+rpss_s(it)/float(nss)
      enddo
      write(60,999) 'avg=', avg
c
