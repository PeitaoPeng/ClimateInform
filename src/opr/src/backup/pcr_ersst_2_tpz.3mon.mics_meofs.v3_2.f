...
c         call wmo_clim_tot(ts2,w1d,nyr,nwmo)
          call clim_anom(ts2,cc,nyr,ny_tpz)
...
c normalize obs and hcst
      ...
      do j=1,jmx2
        ...
        do it=its_hcst,ny_tpz
          ...
          wthcst(i,j,it,ld)=wthcst(i,j,it,ld)/stdf(i,j,ld)
        enddo
...
c write out fcst
       ...
       do ld=1,nlead
         ...
         do j=1,jmx2
           if(fld2(i,j).gt.-900.) then
             w2d(i,j)=(wtfcst(i,j,ld)-clmf(i,j,ld))/stdf(i,j,ld)
           else
             w2d(i,j)=undef
           endif
         enddo
