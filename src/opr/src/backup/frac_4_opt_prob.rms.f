...
C avg & std of ehcst & obs
      ...
      do j=1,jmx
        ...
        if (w2d(i,j).gt.-900.) then
          ...
          call mean_std(ts1,ny_hcst,31,ny_hcst,av,sd)
          call mean_std(ts2,ny_hcst,31,ny_hcst,av2,sd2)
...
c prob for each it
      do it=1,ny_hcst
        ...
        tsobs(it)=obs(i,j,it)/ostd(i,j,it)
...
c
      ...
      SUBROUTINE mean_std(s,nt,its,mt,avg,std)
      DIMENSION s(nt),w(mt)
      avg=0
      do i=its,mt
         avg=avg+s(i)/float(mt-its+1)
      enddo
...
c
      ...
      std=sqrt(sd/float(mt-its+1))

      return
      end

      SUBROUTINE mean_std_anom(s,nt,mt,avg,std)
      DIMENSION s(nt),w(mt)
      avg=0
      do i=1,mt
