...
mlead=8   # max lead of ensemble fcst
ncv=1
vnmb=v4
...
#
its_skill=`expr 1981 - 1947 - $lagmax / 12` # skill from 1981
its_hcst=`expr 1951 - 1947 - $lagmax / 12` # write hcst out from 1951
#
...
ny_out=`expr $nyear - $its_skill - $lagmax / 12` # from 1981 to
ny_hcst=`expr $nyear - $its_hcst - $lagmax / 12` # from 1951 to
ny_hcstp=`expr $ny_hcst + 1` # from 1951 to
...
fracfile=frac_rpss.ersst_2_sst.mlead8.3mon.$vnmb
#
#=======================================
#
cp $lcdir/pcr_ersst_2_sst.3mon.mics_meofs.$vnmb.f $tmp/pcr.f
cp $lcdir/backup/reof.s.f $tmp/reof.s.f

cat > parm.h << eof
c
      parameter(icmon=$icmon)  ! sst ic month
      parameter(its_skill=$its_skill,its_hcst=$its_hcst)
...
outfile3=fcst.$var1.2.$var2.mics$mics.mlead$mlead.ncut$ncut.icut1_$icut1.id_ceof${id_ceof}.cv$ncv.3mon.$vnmb
outfile4=skill_1d.$var1.2.$var2.mics$mics.mlead$mlead.ncut$ncut.icut1_$icut1.id_ceof${id_ceof}.cv$ncv.3mon.$vnmb
outfile5=hcst.$var1.2.$var2.mics$mics.mlead$mlead.ncut$ncut.icut1_$icut1.id_ceof${id_ceof}.cv$ncv.3mon.$vnmb
outfile8=${var1}_ic_mics.3mon.$vnmb
outfile9=nino34.prd.mics$mics.mlead$mlead.ncut$ncut.icut1_$icut1.id_ceof${id_ceof}.cv$ncv.3mon.$vnmb
outfile10=nino34.obs.icmon_$icmon.3mon.$vnmb
#
ln -s $datain1/$sstfile.gr  fort.10

ln -s $dataot2/$fracfile.gr  fort.11
#
...
ydef  1 linear -88.  2.
zdef  1 linear 1 1
tdef  $ny_hcstp linear ${tgtmon}$outyr_s 1yr
...
ydef 1 linear -88.  2.
zdef 1 linear 1 1
tdef $ny_out linear ${tgtmon}$skill_yr_s 1yr
edef $mlead names 1 2 3 4 5 6 7 8
vars  6
