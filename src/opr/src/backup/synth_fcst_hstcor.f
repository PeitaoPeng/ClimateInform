...
C===========================================================
      ...
      real clm(imx,jmx,mlead)
      real rms(imx,jmx,mlead,nprd),hss(imx,jmx,mlead,nprd)
      
      real hcst(imx,jmx,nprd)

      real oprd(imx,jmx),ocor(imx,jmx)
      real orms(imx,jmx),ohss(imx,jmx)
      real oclm(imx,jmx)
      dimension kmax(imx,jmx,mlead)
      real obs(imx,jmx),ohcst(imx,jmx)
      real ostd(imx,jmx)
...
C=== synthesize prd with max cor
      ...
      if (w2d(i,j).gt.-900.) then
          ...
          kmax(i,j,ld) = maxloc(w1d,1) 

          oprd(i,j)=prd(i,j,ld,kmax(i,j,ld))
          ostd(i,j)=stdo(i,j,ld)
          ocor(i,j)=cor(i,j,ld,kmax(i,j,ld))
          orms(i,j)=rms(i,j,ld,kmax(i,j,ld))
          ohss(i,j)=hss(i,j,ld,kmax(i,j,ld))
          oclm(i,j)=clm(i,j,ld)
      else
          oprd(i,j)=undef
          ostd(i,j)=undef
      ...
      iw=iw+1
      write(31,rec=iw) oprd
      iw=iw+1
      write(31,rec=iw) ostd
      iw=iw+1
      write(31,rec=iw) ocor
      iw=iw+1
      write(31,rec=iw) orms
      iw=iw+1
      write(31,rec=iw) ohss
      iw=iw+1
...
C synthesize hcst with max cor
      ...
      enddo ! ip loop
        ...
        if (w2d(i,j).gt.-900.) then
          ohcst(i,j)=hcst(i,j,kmax(i,j,ld))
        else
          ohcst(i,j)=undef
        endif
      ...
      write(32,rec=iw) obs
      iw=iw+1
      write(32,rec=iw) ohcst
      iw=iw+1
      write(32,rec=iw) ostd
c
c== spatial skill

      call sp_cor_rms(obs,ohcst,coslat,imx,jmx,
     &1,360,115,160,xcor,xrms)

      iw2=iw2+1
      write(33,rec=iw2) xcor
      iw2=iw2+1
      write(33,rec=iw2) xrms
     ...
     &230,300,115,140,xcor,xrms)
      ...
      call hss3c_s(obs,ohcst,imx,jmx,1,360,115,160,coslat,h1)
      call hss3c_s(obs,ohcst,imx,jmx,230,300,115,140,coslat,h2)
c
      iw2=iw2+1
      write(33,rec=iw2) h1
      iw2=iw2+1
