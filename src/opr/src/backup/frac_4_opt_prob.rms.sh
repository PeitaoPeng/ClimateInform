...
# with ensemble avg tech using rms skill
# Inpot fcst/hcst are NOT standardized
#===============================================
...
vnmb=v3_1
vnmb2=rms
iwts=1 # =1 <0 ignored, =2 <0 flipped

if [ $version = rms ];  then ivs=1; fi
if [ $version = cvrms ];  then ivs=2; fi
...
#
cp $lcdir/frac_4_opt_prob.$vnmb2.f $tmp/opt.f
...
#infile12=nino34.prd.mics4.mlead$mlead.ncut3.icut1_15.id_ceof1.cv1.3mon.$vnmb
infile12=nino34.prd.mics4.mlead$mlead.ncut3.icut1_15.id_ceof1.cv1.3mon.v3
#
outfile=$version.frac_rpss.ensmsynth.$var.mlead$mlead.3mon.nprd$nprd.wts$iwts.$vnmb2
#
