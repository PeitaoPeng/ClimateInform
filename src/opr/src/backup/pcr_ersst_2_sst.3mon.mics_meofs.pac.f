...
c      
      ...
      real pa_adj(imx,jmx,nyr),pb_adj(imx,jmx,nyr)
      real pn_adj(imx,jmx,nyr)
      real prbhcst(imx,jmx,nyr)
      real oa(imx,jmx,nyr),ob(imx,jmx,nyr),on(imx,jmx,nyr)
      real xpa(imx,jmx),xpb(imx,jmx),xpn(imx,jmx)
      real xcoef(imx,jmx)

      real v3c(imx,jmx,nyr)
...
C nino3.4 of hcst
      do it=its_hcst,ny_tpz
        ...
        do i=1,imx
        do j=1,jmx
          fld0(i,j)=stdo(i,j,iwmo,ld)*vfld(i,j,it,ld)
          fld(i,j)=stdo(i,j,iwmo,ld)*wthcst(i,j,it,ld)
        enddo
        enddo
...
C nino3.4 of fcst
        do i=1,imx
        do j=1,jmx
          fld(i,j)=stdo(i,j,nwmo,ld)*wtfcst(i,j,ld)
        enddo
        enddo
...
C have prob of hcst and obs
      ...
      do j=1,jmx
        ...
        w2d(i,j)=vfld(i,j,it,ld) ! already standardized with WMO std
        w2d2(i,j)=wthcst(i,j,it,ld)

      if (w2d2(i,j).gt.-900.and.w2d(i,j).gt.-900) then

      call prob_3c_prd(w2d2(i,j),xprb,
     &pa(i,j,it),pb(i,j,it),pn(i,j,it),kpdf,xbin,xdel,ypdf)

      call obs_2_prob(w2d(i,j),v3c(i,j,it),oa(i,j,it),ob(i,j,it),
     &on(i,j,kr))
...
c PAC correction for pa, pb & pn
      ...
      IF (pa(i,j,its_hcst).gt.-900) then

      do itgt=its_hcst,ny_tpz

      kr=0
      do it=its_hcst,ny_tpz

      if(it.eq.itgt) go to 777
 ...
 777  continue
      ...
      call pac_t(ts1,ts2,nyr,kr,xcoef(i,j))
      pa_adj(i,j,itgt)=xcoef(i,j)*(pa(i,j,itgt)-1./3.)+1./3.

      call pac_t(ts3,ts4,nyr,kr,xcoef(i,j))
      pb_adj(i,j,itgt)=xcoef(i,j)*(pb(i,j,itgt)-1./3.)+1./3.

      call pac_t(ts5,ts6,nyr,kr,xcoef(i,j))
      pn_adj(i,j,itgt)=xcoef(i,j)*(pn(i,j,itgt)-1./3.)+1./3.

      if(wthcst(i,j,itgt,ld).gt.0) then
...
C write out hcsts
      ...
      write(6,*) 'iwmo=',iwmo
       ...
       do i=1,imx
       do j=1,jmx
         w2d7(i,j)=stdo(i,j,iwmo,ld)
       enddo
       enddo
       iw2=iw2+1
       write(32,rec=iw2) w2d7
...
c write out fcst and skill_t
         ...
         iw3=iw3+1
         write(30,rec=iw3) w2d
         iw3=iw3+1
         write(30,rec=iw3) w2d2
         iw3=iw3+1
         write(30,rec=iw3) w2d3
         iw3=iw3+1
         write(30,rec=iw3) w2d4
         iw3=iw3+1
         write(30,rec=iw3) w2d5
         iw3=iw3+1
         write(30,rec=iw3) w2d6
...
c
      iw3=iw3+1
      write(30,rec=iw3) prbprd
      iw3=iw3+1
      write(30,rec=iw3) xpa
      iw3=iw3+1
      write(30,rec=iw3) xpb
      iw3=iw3+1
      write(30,rec=iw3) xpn
