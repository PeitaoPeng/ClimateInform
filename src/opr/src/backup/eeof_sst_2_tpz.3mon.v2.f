...
C 
      ...
      do j=1,jmx2
        if(w2d3(i,j).gt.-900.) then
          ...
          enddo
          ns_tpz2=ir
c         call clim_anom(ts2,xclm,nfld,ns_tpz2)
        else
          do it=1,ns_tpz2
            ts2(it)=undef
          enddo
...
c
      ...
      ise_skill=ite_skill
      ns_skill=ny_skill
      write(6,*) 'iss_skill=',iss_skill
      write(6,*) 'ise_skill=',ise_skill
      write(6,*) 'ns_skill=',ns_skill
      ...
      do ld=1,nlead
        ...
        do j=1,jmx2
          if (w2d3(i,j).gt.-900.) then
            ...
            do it=1,ns_tpz2
              ts2(it)=vfld(i,j,it,ld)
            enddo

            call wmo_clm_std_anom(ts2,w1d,w1d2,ts4,nfld,ns_tpz2,nwmo)
c           call clm_std_anom(ts2,std,clm,ts4,nfld,ns_skill,iss_skill,
c    &ns_tpz2)

            do ii=1,nwmo
              stdo(i,j,ii,ld)=w1d(ii)
              clmo(i,j,ii,ld)=w1d2(ii)
c             stdo(i,j,ii,ld)=std
c             clmo(i,j,ii,ld)=clm
            enddo
...
c== spatial skill
     ...
     &230,300,115,140,xcor,xrms)
      ...
      call hss3c_s(w2d,w2d2,imx2,jmx2,1,360,40,165,coslat,h1)
      call hss3c_s(w2d,w2d2,imx2,jmx2,230,300,115,140,coslat,h2)
...
c
        ...
        IF(w2d3(i,j).gt.-900.) then
          ...
          if(is.eq.itgt) go to 777

          if(ncv.eq.3) then
            if(is.eq.ism)  go to 777
            if(is.eq.isp)  go to 777
          endif

            ir=ir+1
            ts2(ir)=hcst(i,j,is,ld)
            ts3(ir)=vfld(i,j,is,ld)
  777     continue
          enddo

          mfld=ir
          call cor_rms(ts2,ts3,nfld,mfld,cvcor(i,j,itgt,ld),w2d(i,j))
...
c
      ...
      do id=1,nwmo-1
        its=30+(id-1)*10+2+1 ! +2 is for skip 1949-1950
        ite=30+id*10+2
        if(it.ge.its.and.it.le.ite) iwmo=id
      enddo

      its=30+(nwmo-1)*10+2+1
...
C have WMO std and clm
      ...
      its=(id-1)*10+2+1 ! +2 for skiping 1949-1950
      ite=(id-1)*10+30+2 ! +2 for skip 1949-1950
...
c
      ...
      its=30+(nwmo-1)*10+2+1 ! +2 is for skip 1950
      ite=nt
