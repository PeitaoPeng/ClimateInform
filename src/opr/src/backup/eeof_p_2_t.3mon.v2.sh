...
its_skill=`expr $nclm_start - 1947 - $lagmax / 4`
if [ "$icmon" = 1 ] || [ "$icmon" = 2 ]; then its_skill=$(($its_skill - 1)); fi
ite_skill=`expr $its_skill + 39` # have 40 yrs clm for more stable than 30 yrs
ny_skill=`expr $ite_skill - $its_skill + 1`
...
ny_out=`expr $nyear - $its_skill - $lagmax / 4 + 1` # from its_skill to 
...
#=======================================
#
cp $lcdir/eeof_p_2_t.3mon.v2.f $tmp/pcr.f
cp $lcdir/backup/reof.s.f $tmp/reof.s.f

cat > parm.h << eof
c
      parameter(icmon=$icmon)  ! sst ic month
      parameter(ny_skill=$ny_skill,its_skill=$its_skill,ite_skill=$ite_skill) 
      ...
      parameter(ngrd=$ngrd)
      parameter(lons=$lons,lone=$lone,lats=$lats,late=$late) !eof_area
...
outfile3=efcst.${eof_area}.$var1.2.$var2.cv$ncv.3mon.$vnmb
outfile4=eskill_1d.${eof_area}.$var1.2.$var2.cv$ncv.3mon.$vnmb
outfile5=ehcst.${eof_area}.$var1.2.$var2.cv$ncv.3mon.$vnmb
#
...
#
./pcr.x > $dataot2/eeof.${eof_area}.$var1.2.$var2.mlead$mlead.$vnmb.out
#./pcr.x 
#
#
