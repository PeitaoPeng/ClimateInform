...
c
      nskip=1
      if(icmon.le.2) nskip=0 ! =1 skip 1949-1950; =0 just skip 1950
...
c       fcst(i,j,ld)=fcst(i,j,ld)-clmf(i,j)
      else
        fcst(i,j,ld)=undef
      endif
...
c
  ...
  777     continue
          ...
          mfld=ir
          call cor_rms(ts2,ts3,nfld,mfld,cvcor(i,j,itgt,ld),
     & cvrms(i,j,itgt,ld))

        ELSE

          cvcor(i,j,itgt,ld)=undef
          cvrms(i,j,itgt,ld)=undef
...
c write out obs and hcst
        ...
        do it=its_hcst,ns_tpz2 ! from 1951
          ...
          call have_iwmo(it,nwmo,iwmo,nskip)
          write(6,*) 'iwmo=',iwmo
...
c       anom(i)=ts(i)-clm(1)
      ...
      do id=1,nwmo-1 ! 81-90,91-00,01-10,11-20,21-cur
        its=30+(id-1)*10+1+1+ii 
        ite=30+id*10+1+ii
        do i=its,ite
          anom(i)=(ts(i)-clm(id))/std(id)
c         anom(i)=ts(i)-clm(id)
        enddo
      enddo
