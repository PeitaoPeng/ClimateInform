...
C Synthize fcsts from different models
C input fcst/hcst are NOT normalized
C===========================================================
      parameter(kpdf=1000)
...
C=== have wts prd
      ...
      if (w2d(i,j).gt.-900.) then
          ...
          do ip=1,nprd
            ...
            w1d2(ip)=rms(i,j,ld,ip)
          enddo

          if(iwts.eq.1) then
            call weights(w1d,w1d2,nprd,ws1d)
          else
            call weights_2(w1d,w1d2,nprd,ws1d)
          endif
...
C=== synthesize hcst with cvrms & cvcor
...
C=== have wts from cvrms & cvcor for hcst
      do i=1,imx
      do j=1,jmx
...
C have ehcst, prob-hcst and 1-D skill
c     DO it=1,ny_hcst
...
c prob-hcst
      ...
      if (ehcst(i,j,it).gt.-900.and.std_f(i,j).gt.-900) then

        if(std_o(i,j).lt.0.01) std_o(i,j)=0.01
        w2d(i,j)=obs(i,j,it)/ostd(i,j,it) 
        obs(i,j,it)=w2d(i,j) 

        if(std_f(i,j).lt.0.01) std_f(i,j)=0.01
        w2d2(i,j)=ehcst(i,j,it)/std_f(i,j)
        ehcst(i,j,it)=w2d2(i,j)

        if(w2d2(i,j).gt.4.) w2d2(i,j)=4.
        if(w2d2(i,j).lt.-4.) w2d2(i,j)=-4.
...
c
      ...
      if (eprd(i,j).gt.-900.) then
        ...
        if(std_f(i,j).lt.0.01) std_f(i,j)=0.01
        eprd(i,j)=eprd(i,j)/std_f(i,j)

        if(eprd(i,j).gt.4.) eprd(i,j)=4.
        if(eprd(i,j).lt.-4.) eprd(i,j)=-4.
...
c          
      ...
      if(n1.gt.1000.or.n2.gt.1000) then
        n1=-0.43/xdel
        n2= 0.43/xdel
      endif
...
c
      ...
      SUBROUTINE weights_2(cor,rms,n,wts)
      dimension cor(n),rms(n),wts(n) 
...
C== have weights
      if(wd.gt.0.01) then
        ...
        do i=1,n
c        if(cor(i).ge.0.) then
c         wts(i)=(1/rms(i)**2)/wd
          wts(i)=(1/rms(i))/wd
c       else
c         wts(i)=-(1/rms(i)**2)/wd
c         wts(i)=-(1/rms(i))/wd
c       endif
      ...
      SUBROUTINE weights(cor,rms,n,wts)
      dimension cor(n),rms(n),wts(n) 
