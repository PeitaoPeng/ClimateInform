...
C===========================================================
      ...
      real cvrms(imx2,jmx2,nyr,nlead)
...
c       wtfcst(i,j,ld)=wtfcst(i,j,ld)-clmf(i,j)
      else
        wtfcst(i,j,ld)=undef
      endif
...
c
      if(fld2(i,j).gt.-900.) then
        ...
        if (i.eq.250.and.j.eq.130) then
          write(6,*) 'nyr,ny_skill=',nyr,ny_skill
          write(6,*) 'obs=',ts1
          write(6,*) 'hcst=',ts2
          write(6,*) 'rms=',rms3d(i,j,ld)
          write(6,*) 'hss=',hss3d(i,j,ld)
        endif
...
c
  ...
  777     continue
          ...
          nfld=ir
          call cor_rms(ts1,ts2,nyr,nfld,cvcor(i,j,itgt,ld),
     & cvrms(i,j,itgt,ld))
...
c write out fcst and skill_t
       ...
       do ld=1,nlead
         ...
         do j=1,jmx2
           ...
           w2d6(i,j)=tpzc_tot(i,j,nwmo,ld) ! latest wmo clm 
         enddo
         enddo
