...
cp $lcdir/simple_esm.fcst.f $tmp/esm.f
...
#
ifile1=10
for sstdata in ersst; do
for msic in 1 2 3 4; do
for nmod in 15 25 40; do

ifile1=`expr $ifile1 + 1` 
#
ln -s $datadir/real_ca_prd.$var2.$sstdata.tp_ml.$nmod.${msic}ics.${msic}ics.3mon.gr fort.$ifile1
#
done  # nmod
done  # msics
done  # sst
