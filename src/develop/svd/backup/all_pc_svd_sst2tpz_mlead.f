...
c     PARAMETER (ngrd=(late-lats+1)*(lone-lons+1))
      ...
      real pcv1(nyr,nyr),pcv2(nyr,nyr)

      real corv1(imx,jmx,nyr),regv1(imx,jmx,nyr)
      real corv2(imx,jmx,nyr),regv2(imx,jmx,nyr)

      real corr11(nyr,msvd),regr11(nyr,msvd)
      real corr12(nyr,msvd),regr12(nyr,msvd)
      real corr21(nyr,msvd),regr21(nyr,msvd)
      real corr22(nyr,msvd),regr22(nyr,msvd)

      real prdpcv2(nyr,nyr),w1d(nyr)
...
c have fsst & ftpz used as input of svd      
      ...
      enddo
      enddo
      call eof_pc(fsst,cosr,nyr,nfldp,ng1,undef,
...
c
      ...
      enddo
      enddo
      call eof_pc(ftpz,cosr,nyr,nfld,ng2,undef,
...
c SVD bwtween pcv1 & pcv2
      ...
      enddo
C
      call pc_svd(pcv1,pcv2,nyr,nfld,msvd,cof1,cof2,cic)
c
c normalize cof & cic
...
c
      ...
      subroutine eof_pc(fin,cosr,ntot,nt,ng,undef,
...
c
      ...
      dimension pc(ntot,ntot)

      dimension ts1(nt),ts2(nt)
      dimension cor(im,jm,ntot),reg(im,jm,ntot)
c
c select grid data
      do it=1,nt
        ig=0
...
c
      ...
      SUBROUTINE pc_svd(pcv1,pcv2,nt,mt,msvd,cof1,cof2,cic)

      real pcv1(nt,nt),pcv2(nt,nt)
      real aleft(mt,mt),aright(mt,mt)
      real a(mt,mt),w(mt),u(mt,mt),v(mt,mt),rv1(mt)
      real wic(mt),aic(mt),cic(msvd)
c
