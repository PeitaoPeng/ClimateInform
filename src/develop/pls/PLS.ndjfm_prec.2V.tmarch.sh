...
#=========================================================
# test fcst of ndjfm prec with pls_eof 
#=========================================================
lcdir=/home/ppeng/src/develop/pls
tmp=/home/ppeng/tmp
if [ ! -d $tmp ] ; then
  mkdir -p $tmp
fi

datadir1=/home/ppeng/data/tpz
datadir2=/home/ppeng/data/ca_prd
datadir3=/home/ppeng/data/test
#
cd $tmp
# 
model=pls
...
#
var1_ave=3mon  # or mon
nmod_v2=7  #var2 eof mode
var1_lag=3 # monthly: =1; 3mon: =3
#
tgtss=ndjfm
if [ $tgtss == ndjfm ]; then itgtbgn=23; ssn=11; fi # 23 means nov1949
#
nlead=19
byear=1948
nyro=74  #obs period 1950->2023
nts=51   # 1950-2000: latest data used to the first fcst
nyrv=`expr $nyro - $nts`  # 2001-2023: verification period long ??-2023
#
imx=360
jmx=180
idx=1;idy=1 # var2 eof resolution
#ngrd=195   # t2m
#ngrd=660   # prec W.Europe
 ngrd=1329  # prec CONUS
#ngrd=176    # prec snow pack
#
ld=1
while  [ $ld -le $nlead ]
do
...
c
c      parameter(isv1=1,iev1=360,jsv1=70,jev1=110)
c      parameter(isv1=1,iev1=360,jsv1=70,jev1=155)
       parameter(isv1=120,iev1=290,jsv1=70,jev1=111) !120E-290E,20S-20N
 
       parameter(isv2=221,iev2=300,jsv2=110,jev2=140) ! conus
c      parameter(isv2=245,iev2=255,jsv2=125,jev2=140) ! w.conus
c      parameter(isv2=1,iev2=45,jsv2=140,jev2=157)
c      parameter(isv2=1,iev2=150,jsv2=140,jev2=157) !W.Europe
c
...
c
      real npick(nmod),mpick(nmod)
      data npick /1,1,1,1,1,1,1/ ! conus with t&extp sst
      data mpick /2,4,7,1,1,1,1/ ! conus with t&extp sst
c
...
C read data
      ...
      ir2=2 ! from ndjfm of 1949/50
c     ir2=itgtbgn ! for djf 1949/50
      do it=1,nt
...
c
      write(6,*) 'nt=',nt
      call screen_3d(w3dv2,imx,jmx,nt,undef)
c
...
c have anomalous w3dv1 to mtp

        do i=1,imx
        do j=1,jmx
          if(abs(w3dv1(i,j,mt)).lt.1000) then
            it=1,mtp
              ts2(it)=w3dv1(i,j,it)
            enddo
            call anom(ts2,nt,mtp,av)
            it=1,mtp
              w3d(i,j,it)=ts2(it)
            enddo
          else
...
c
     ...
     &ts1,w3d,wic,pc,pco,pat,pat2,var,coslat,undef,ID)
c
...
c forecasted var2 on grids
      ...
      if(abs(w3dv2(i,j,1)).lt.999) then
        do n=1,nmod
          prd(i,j,itp)=prd(i,j,itp)+npick(n)*rpcf(n,itp)*regv2(i,j,n)
        enddo
      else
          prd(i,j,itp)=undef
      endif
...
c
C have anomalous fin over nt
      do i=is,ie,idx
      do j=js,je,idy
        if(abs(fin(i,j,nt)).lt.1000.) then
        do it=1,nt
          ts1(it)=fin(i,j,it)
        enddo
c
        call anom(ts1,ntot,nt,avg)
c
        do it=1,nt
	  fin(i,j,it)=ts1(it)
        enddo
...
C select grid data
      ...
      do i=is,ie,idx
      do j=js,je,idy
        if(abs(fin(i,j,it)).lt.1000.) then
        ig=ig+1
...
c
      ...
      SUBROUTINE screen_3d(fld,n,m,k,undef)
      real fld(n,m,k)
      write(6,*) 'nt=',k
      do i=1,n
      do j=1,m
        do l=1,k
        if(abs(fld(i,j,l)).gt.1000.) then
	do ll=1,k
       	fld(i,j,ll)=undef
	enddo
        endif
...
#
 ...
 ln -s $datadir2/$var1.$var1_ave.${byear}-curr.1x1.gr fort.10
 ln -s $datadir1/$var2.48-cur.ndjfm.gr                fort.20
#
