...
#
var1_tp=3mtn  # or 3mon
var2_tp=3mon  # or mon
#
nmod_v2=13 #var2 eof mode
#
var1_lag=3 # monthly: =1; 3mon: =3
#
nlead=1
byear=1948
nyr=75 #hcst period 1979->2020
#
imx=360
jmx=180
#ngrd=1154 # t2m
...
c      parameter(isv1=1,iev1=360,jsv1=70,jev1=110)
       parameter(isv1=1,iev1=360,jsv1=70,jev1=160)
...
C have sst anomalies over period 1 -> ny_sst
      ...
      do j=1,jmx
        if(w2d(i,j).gt.-900.) then
          do it=1,nt
            ts1(it)=w3dv1(i,j,it)
          enddo
          call clim_anom(ts1,v1clm(i,j),nt,nt)
        else
          do it=1,nt
            ts1(it)=undef
          enddo
      ...
      enddo
      enddo
C have tpz anomalies over period 1 -> ny_sst
      do i=1,imx
      do j=1,jmx
        if(w2d2(i,j).gt.-900.) then
          do it=1,nt
            ts1(it)=w3dv2(i,j,it)
          enddo
          call clim_anom(ts1,v2clm(i,j),nt,nt)
        else
          do it=1,nt
            ts1(it)=undef
          enddo
...
c     call pls_hcst(nt-1,imx,jmx,isv1,iev1,jsv1,jev1,modpls,
c    &w1d,w3dv1n,wic,pc,pco,pat,pat2,var,coslat,undef,ID)
      crit=0.1
      call pls_ana(nt-1,nt-1,imx,jmx,isv1,iev1,jsv1,jev1,modpls,
     &w1d,w3dv1n,wic,pc,pco,pat,pat2,var,coslat,crit,undef,ID)
c
...
c select optimal rpcf
      ...
      enddo
c
      call hpsort(modpls,modpls,corn,ts6)
c
      if(corn(modpls).gt.0.1) then
        do it=1,nt
        rpcf3(m,it)=rpcf2(m,ts6(modpls),it)
        enddo
      endif
      print *, 'corn=',corn
      print *, 'ts6=',ts6
...
cp $lcdir/pls.s.f  pls.s.f
#
 gfortran -o prd.x reof.s.f  pls.s.f pls.f
 ln -s $datain1/$var1.3mon.${byear}-curr.total.1x1.gr fort.10
 ln -s $datain2/$var2.${byear}_cur.3mon.total.1x1.gr  fort.20
#
