...
C===========================================================
      ...
      real ac(imx,jmx,nyr,leadmax),rm(imx,jmx,leadmax)
      real wtd(imx,jmx,nyr,3),wts(imx,jmx,nyr,leadmax,3)
      ...
      do j=1,jmx
        if(fld(i,j).gt.-900) then
          ...
          do ilead=kld,leadmax
            if(ac(i,j,it,ilead).gt.0.) then
              wtd(i,j,it,1)=wtd(i,j,it,1)+ac(i,j,it,ilead)
              wtd(i,j,it,3)=wtd(i,j,it,3)+ac(i,j,it,ilead)**2
            else
              wtd(i,j,it,1)=wtd(i,j,it,1)+0.
              wtd(i,j,it,3)=wtd(i,j,it,3)+0.
            endif
              wtd(i,j,it,2)=wtd(i,j,it,2)+ac(i,j,it,ilead)**2
          enddo
        else
        wtd(i,j,it,1)=undef
        wtd(i,j,it,2)=undef
...
C== have weights for each lead
      do ilead=kld,leadmax
        ...
        do j=1,jmx
          if(fld(i,j).gt.-900) then
            if(ac(i,j,it,ilead).gt.0.) then
              wts(i,j,it,ilead,1)=ac(i,j,it,ilead)/wtd(i,j,it,1)
              wts(i,j,it,ilead,2)=ac(i,j,it,ilead)**2/wtd(i,j,it,2)
              wts(i,j,it,ilead,3)=ac(i,j,it,ilead)**2/wtd(i,j,it,3)
            else
              wts(i,j,it,ilead,1)=0.
              wts(i,j,it,ilead,2)=-ac(i,j,it,ilead)**2/wtd(i,j,it,2)
              wts(i,j,it,ilead,3)=0.
            endif
          else
            wts(i,j,it,ilead,1)=undef
            wts(i,j,it,ilead,2)=undef
            wts(i,j,it,ilead,3)=undef
          endif
        enddo
        enddo
      enddo
      ...
      do it=1,nyr
        ...
        do j=1,jmx
          if(fld(i,j).gt.-900) then
            ...
            w2d2(i,j)=w2d2(i,j)+wts(i,j,it,ilead,1)*fcst(i,j,it,ilead)
            w2d3(i,j)=w2d3(i,j)+wts(i,j,it,ilead,2)*fcst(i,j,it,ilead)
            w2d4(i,j)=w2d4(i,j)+wts(i,j,it,ilead,3)*fcst(i,j,it,ilead)
            enddo
          else
            w2d(i,j)=undef
            w2d2(i,j)=undef
        ...
        enddo ! kld loop
c
c normalize both obs and ensemble mean fcst
c
c std of obs
        do i=1,imx
        do j=1,jmx
          if (vfld(i,j,1).gt.undef) then
            avgo(i,j)=0.
            do it=nclms,nclme
            avgo(i,j)=avgo(i,j)+vfld(i,j,it)/float(nclme-nclms+1)
            enddo
          else
            avgo(i,j)=undef
          endif
...
c
      ...
      do ld=1,nlead
        ...
        do j=1,jmx
          if(wtfcst(i,j,1,1,1).gt.undef) then
            if(stdf(i,j,iv,ld).lt.0.001) then
              stdf(i,j,iv,ld)=0.001
            endif
          endif
        enddo
        enddo
...
c normalize amplitude
      ...
      do j=1,jmx
      do it=1,nyr
      if (vfld(i,j,it).gt.undef) then
        vfld(i,j,it)=(vfld(i,j,it)-avgo(i,j))/stdo(i,j)
      else
        vfld(i,j,it)=undef
      endif
      enddo
...
c
      ...
      do it=1,nyr
      if (wtfcst(i,j,1,1,1).gt.undef) then
        wtfcst(i,j,it,iv,ld)=(wtfcst(i,j,it,iv,ld)-avgf(i,j,iv,ld))/
     &stdf(i,j,iv,ld)
      else
...
c
      if(vfld(i,j,1).gt.-900.) then
        ...
        call hss3c_t(ts0,ts4,nyr,hss2d(i,j))
      else
        cor(i,j)=undef
        rms(i,j)=undef
        cor2(i,j)=undef
      ...
      iw4=iw4+1
      write(92,rec=iw4) hss2d      
c lead-1 skill
      do i=1,imx
      do j=1,jmx
        fld(i,j)=ac(i,j,nyr,kld)
        fld2(i,j)=rm(i,j,kld)
      enddo
      enddo
c
...
c== spatial skill
     ...
     &230,300,115,140,xcor,xrms)
      ...
      call hss3c_s(w2d,w2d2,imx,jmx,1,360,115,160,undef,h1)
      call hss3c_s(w2d,w2d2,imx,jmx,230,300,115,140,undef,h2)
      iw=iw+1
      write(93,rec=iw) h1
      iw=iw+1
      write(93,rec=iw) h2
      ...
      SUBROUTINE hss3c_s(obs,prd,imx,jmx,is,ie,js,je,undef,h)
      ...
      SUBROUTINE hss3c_t(obs,prd,nt,h)
      dimension obs(nt),prd(nt)
      dimension nobs(nt),nprd(nt)
      do it=1,nt
