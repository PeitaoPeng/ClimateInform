#
#======================================
# PCR from sst to tpz with time martching (tm)
#=======================================
...
datain2=/home/ppeng/data/sst
datadir=/home/ppeng/data/test
#datadir=/home/ppeng/tmp
#
nskip=0
undef=-999.0
#
...
ic_mean=3mon # sst ic
out_mean=3mon
nfyr_start=43 # 43: 1991
#
del=1 # =3 for seasonal sst; =1 for monthly sst
#
...
for var2 in ersst; do # prec, t2m, hgt
for tgtss in djf; do
for nmod in 20; do
#for nmod in 46; do
...
#
cp $lcdir/pcr_sst2tpz_mlead.f $tmp/pcr.f
...
eof
#
infile1=$var1.mon.1948-curr.total.1x1
infile2=$var2.3mon.1948-curr.total.1x1
outfile=${var1}_2_$var2.tm.ic_$ic_mean.ld$ilead.nmod$nmod.$tgtss.${eof_area}.$out_mean
outfile1=pcr_$var2.tm.ic_$ic_mean.$tgtss.ld$ilead.nmod$nmod.${var1}_eof_${eof_area}.$out_mean
outfile2=skill.pcr.tm.$var2.ic_$ic_mean.$tgtss.ld$ilead.nmod$nmod.${var1}_eof_${eof_area}.$out_mean
outfile3=${var2}_regr.tm.ic_$ic_mean.$tgtss.ld$ilead.48-21.${var1}_eof_${eof_area}.$out_mean
#
\rm $tmp/fort.*
#
