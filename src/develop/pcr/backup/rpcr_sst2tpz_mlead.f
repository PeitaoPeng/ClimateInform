...
c
      ...
      enddo
C
      call rpc_reof(wsst,imx,jmx,lons,lone,lats,late,nyr,nclm,cosr,
...
c           w2d(i,j)=w2d(i,j)+tcof(m,iyr)*regr2(i,j,m)
          enddo
...
c
      ...
      SUBROUTINE rpc_reof(wf3d,imx,jmx,is,ie,ls,le,ltime,nfld,cosr,
     1ngrd,nmod,tcof,corr,regr,undef,id)
      ...
      real weval(nfld),wevec(ngrd,nfld),wcoef(nfld,nfld)
      real reval(nmod),revec(ngrd,nmod),rcoef(nmod,nfld)
...
c
      ...
      call REOFS(aaa,ngrd,nfld,nfld,wk,id,weval,wevec,wcoef,
