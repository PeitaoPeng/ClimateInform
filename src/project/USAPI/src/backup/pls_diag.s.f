      subroutine pls_diag(nt,im,jm,is,ie,js,je,mode,
     &f1d,f3d,pc,pt1,pt2,var,cosl,undef,ID)
C======================================================
...
c     write(6,*) 'w1d=',w1d
      ...
      do j=1,jm
        if(abs(f3d(i,j,1)).lt.999) then
          ...
          w1d2(it)=w3d(i,j,it)
          enddo
          call normal_a(w1d2,nt,sd)
          do it=1,nt
          w3d(i,j,it)=w1d2(it)
          w3d2(i,j,it)=w1d2(it) !for later reg 
          enddo
        endif
      enddo
      enddo
...
c have correlation pattern
        ...
        if(abs(f3d(i,j,1)).lt.999) then
          do it=1,nt
            w1d2(it)=w3d(i,j,it)
          enddo
          call cor_reg_a(nt,w1d,w1d2,cor,reg)
          w2d(i,j)=cor
        else
          w2d(i,j)=undef
        endif
...
c project 2-d pattern to the 3-d data to have time series
        do it=1,nt
          ...
          enddo
          enddo
          call sp_proj_a(w2d,w2d2,im,jm,is,ie,js,je,cosl,pj)
          w1d2(it)=pj
        enddo
c have reg pattern with standardized predictor
        ...
        if(abs(f3d(i,j,1)).lt.999) then
          ...
          enddo
          call cor_reg_a(nt,w1d2,w1d3,cor,reg)
          call cor_reg_a(nt,w1d2,w1d4,cor2,reg2)
          w2d(i,j)=reg
          w2d2(i,j)=reg2
        else
          w2d(i,j)=undef
...
c regress out z from Y and X
        call cor_reg_a(nt,w1d2,w1d,cor,reg)
        do it=1,nt
...
c explained variance by mode m
        call cor_reg_a(nt,w1d2,f1d,cor,reg)
        var(m)=cor*cor
        do it=1,nt
          pc(it,m)=w1d2(it)*reg
        enddo
...
c
      subroutine cor_reg_a(n,ts1,ts2,cor,reg)
...
c
      ...
      subroutine sp_proj_a(f1,f2,im,jm,ims,ime,jms,jme,cosl,pj)
