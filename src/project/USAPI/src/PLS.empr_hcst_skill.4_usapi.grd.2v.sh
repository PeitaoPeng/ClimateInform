...
#
imon=$curmo
if [ $imon = 01 ]; then icmonw=jan; tmons=feb; itgtbgn=13; ssn=1;fi
if [ $imon = 02 ]; then icmonw=feb; tmons=mar; itgtbgn=14; ssn=2;fi
if [ $imon = 03 ]; then icmonw=mar; tmons=apr; itgtbgn=15; ssn=3;fi
if [ $imon = 04 ]; then icmonw=apr; tmons=may; itgtbgn=16; ssn=4;fi
if [ $imon = 05 ]; then icmonw=may; tmons=jun; itgtbgn=17; ssn=5;fi
if [ $imon = 06 ]; then icmonw=jun; tmons=jul; itgtbgn=18; ssn=6;fi
if [ $imon = 07 ]; then icmonw=jul; tmons=aug; itgtbgn=19; ssn=7;fi
if [ $imon = 08 ]; then icmonw=aug; tmons=sep; itgtbgn=20; ssn=8;fi
if [ $imon = 09 ]; then icmonw=sep; tmons=oct; itgtbgn=21; ssn=9;fi
if [ $imon = 10 ]; then icmonw=oct; tmons=nov; itgtbgn=22; ssn=10;fi
if [ $imon = 11 ]; then icmonw=nov; tmons=dec; itgtbgn=23; ssn=11;fi
if [ $imon = 12 ]; then icmonw=dec; tmons=jan; itgtbgn=24; ssn=12;fi
#
#for tp in mon ss; do
 for tp in ss; do
#for tp in mon; do
...
\rm fort.*
 ...
 ln -s $datadir2/$var1.$var1_tp.${byear}-curr.1x1.gr fort.20
 ln -s ots.ld$ld.gr              fort.30
 ln -s $var1.ld$ld.gr            fort.40
 input.x
#
#
...
infile2=$var1.ld$ld.gr

outfile1=$island.${varo}_$var1.${icmonw}_ic.ld$ld.ts
outfile2=$island.${varo}_$var1.${icmonw}_ic.ld$ld.pat
outfile3=pls.ld$ld
outfile4=$island.${varo}_$var1.${icmonw}_ic.ld$ld.diag.ts
outfile5=$island.${varo}_$var1.${icmonw}_ic.ld$ld.diag.pat
#
cat > parm.h2 << eof
      parameter(nyr=$nyr)
      parameter(im=$imx,jm=$jmx)
...
\rm skill.$nlead
#
mv hss.$nlead $datadir3/skill.$tool.$varn.NMME_${var1}_2_${island}_$varo.$tp.${icmonw}_ic.1982-$lastyr.ld1-$nlead.gr

cat>$datadir3/skill.$tool.$varn.NMME_${var1}_2_${island}_$varo.$tp.${icmonw}_ic.1982-$lastyr.ld1-$nlead.ctl<<EOF
dset ^skill.$tool.$varn.NMME_${var1}_2_${island}_$varo.$tp.${icmonw}_ic.1982-$lastyr.ld1-$nlead.gr
undef -9.99e+33
*
TITLE model
*
...
mv sk.tot $datadir3/skill.$tool.$varn.NMME_${var1}_2_${island}_$varo.$tp.1982-$lastyr.ld-1.gr
\rm sk.*
#
cat>$datadir3/skill.$tool.$varn.NMME_${var1}_2_${island}_$varo.$tp.1982-$lastyr.ld-1.ctl<<EOF
dset ^skill.$tool.$varn.NMME_${var1}_2_${island}_$varo.$tp.1982-$lastyr.ld-1.gr
undef -9.99e+33
*
TITLE model
*
