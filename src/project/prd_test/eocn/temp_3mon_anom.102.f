...
c
      ...
      real afld(imax,12*(ltime+1))
      real afld2(imax,12*(ltime+1))
      real clim(imax,12,2)
      real wk(imax)
      real wko(36,19)
c
      open(unit=10,form='formatted') !monthly cd temp or prec
      open(unit=50,form='unformatted',access='direct',recl=4*36*19)
...
c== read in 102CD data (1931-cur)
 ...
 888  format(10f7.1)
      ...
      enddo
      enddo
c have climate of 1961-1990 & 1971-2000
      do i=1,imax
      do m=1,12
c
      clim(i,m,1)=0.
      ...
      do iy=1,30
      clim(i,m,1)=clim(i,m,1)+fld3D(i,iy+30,m)/30.
      clim(i,m,2)=clim(i,m,2)+fld3D(i,iy+40,m)/30.
      enddo
...
c have anom     
      ...
      do m=1,12
      it=it+1
       afld(i,it)=fld3D(i,iy,m)-clim(i,m,1)
      enddo
      enddo
c
      do iy=71,81
      do m=1,12
      it=it+1
        afld(i,it)=fld3D(i,iy,m)-clim(i,m,2)
      enddo
      enddo
...
c have 3-mon avg
      do i=1,imax
        do it=1,192
          afld2(i,it)=(afld(i,it)+afld(i,it+1)+afld(i,it+2))/3.
        enddo
      enddo
c write out
      do it=1,192
...
C     averaging.
 ...
 150    CONTINUE
      ...
      OPEN(NU,file=
     2'/cpc/home/wd52pp/utility/intpl/fd102161x81.dat')
      NUP=NU+1
      OPEN(NUP,file=
     2'/cpc/home/wd52pp/utility/intpl/mask_2x2.data'
