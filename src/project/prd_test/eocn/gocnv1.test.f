...
c
c== seasonal mean
c     call season_mean(fld3D,ltime+1,iseason,imax,grid)
      call season_mean(fld3D,ltime+1,iseason,imax,fld2D)
c== anomalies wrt wmo clim for verification later
      call anom_wmo(grid,wmoobs,imax,ltime,ntprd,ndec)
c
      ifld=60
c     call season_anom(fld3D,ltime+1,1,ifld,iseason,imax,grid2)
c filtering with EOF
c     nw=2*ifld+15
c     call eof_flt(grid2,imax,ltime,ifld,mmax,nw,fld2D)
...
c
c     call season_anom(fld3D,ltime+1,1,it,iseason,imax,grid3)
c filtering with EOF
      nw=2*it+15
c     call eof_flt(grid3,imax,ltime,it-1,mmax,nw,fld2D)
c
      do m=1,imax
c
         avg=0
