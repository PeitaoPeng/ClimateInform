...
c have seasonally independent OCN
     ...
     &ntprd,ndec,imax,kp,its,ite,acc,kmax)
      write(6,*)'kmax=',kmax
      write(6,999) acc
      iw=iw+1
      write(60,rec=iw) acc
...
c       kt=k+(id-1)*10
c     do m=1,imax
c       fldprd(m,kt,is)=fldprd(m,kt,is)-cwmo(id,m,is)
c       vrfy(m,kt,is)=vrfy(m,kt,is)-cwmo(id,m,is)
c     enddo
c     enddo
c     enddo
c
c     ks=(ndec-1)*10+1
c     do kt=ks,ntprd !1991-cur (at most 2000)
c     do m=1,imax
c       fldprd(m,kt,is)=fldprd(m,kt,is)-cwmo(ndec,m,is)
c       vrfy(m,kt,is)=vrfy(m,kt,is)-cwmo(ndec,m,is)
c     enddo
c     enddo
c
      do m=1,imax
      do kt=1,13
        fldprd(m,kt,is)=fldprd(m,kt,is)-cwmo(1,m,is)
        vrfy(m,kt,is)=vrfy(m,kt,is)-cwmo(1,m,is)
      enddo
      enddo
c
      do m=1,imax
      do kt=14,23
        fldprd(m,kt,is)=fldprd(m,kt,is)-cwmo(2,m,is)
        vrfy(m,kt,is)=vrfy(m,kt,is)-cwmo(2,m,is)
      enddo
      enddo
c
      do m=1,imax
      do kt=24,33
        fldprd(m,kt,is)=fldprd(m,kt,is)-cwmo(3,m,is)
        vrfy(m,kt,is)=vrfy(m,kt,is)-cwmo(3,m,is)
      enddo
      enddo
