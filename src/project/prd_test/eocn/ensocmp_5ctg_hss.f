...
C==========================================================
      ...
      real grid(imax,ltime),grid2(imax,ltime),grid3(imax,ltime)
      real tdgrd(imax,ltime)
      real wk2d(imax,ltime)
      real fld1(ltime)
      real wk1d1(ntprd),wk1d2(ntprd)
      real wk1d3(ltime),wk1d4(ltime)
      dimension wk1(30),wk2(30)
      dimension obsn(imax,ntprd),prdn(imax,ntprd)
      dimension wk3(imax),wk4(imax)
      dimension iwk3(imax),iwk4(imax)
      dimension cmp1(imax),cmp2(imax),cmp3(imax)
      dimension cmp4(imax),cmp5(imax)
      dimension cmpp(imax)
      dimension clmg(ndec,imax)
      dimension wk22(ndx,ndy)
C
      real fld3D(imax,ltime+1,12)
      real sstin(1,ltime+1,12),sstn(ltime),sst1(ltime),sst2(ltime)
      real sst3(ntprd),tdsst(ltime)
...
c
      ...
      open(unit=60,form='unformatted',access='direct',recl=4*ndx*ndy)
...
c
c== seasonal mean
      call season_mean(fld3D,ltime+1,iseason,imax,grid3)
c== anomalies wrt wmo clim for verification later
      call anom_wmo(grid3,wmoobs,imax,ltime,ntprd,ndec)
c
      call season_anom(fld3D,ltime+1,nys,61,iseason,imax,grid)
      call season_anom(sstin,ltime+1,nys,61,iseason,1,sst1)
C
C== temp composite based on nino34
c
      crt1=-5.0
      crt2=-0.5
      crt3=0.5
...
c
c== normalize nino34
      call normal_1d(sst2,sstn,std,ltime,nys+5,it-6)
c
c== have ENSO composite for temp
      call comp_5ctg(sst2,grid2,crt1,crt2,crt3,crt4,cmp1,cmp2,cmp3,
     &cmp4,cmp5,imax,ltime,nys,it-1)
c
c== write out composite
      call CD102_2_2x2(cmp1,wk22)
      iw=iw+1
      write(60,rec=iw) wk22
      call CD102_2_2x2(cmp2,wk22)
      iw=iw+1
      write(60,rec=iw) wk22
      call CD102_2_2x2(cmp3,wk22)
      iw=iw+1
      write(60,rec=iw) wk22
      call CD102_2_2x2(cmp4,wk22)
      iw=iw+1
      write(60,rec=iw) wk22
      call CD102_2_2x2(cmp5,wk22)
      iw=iw+1
      write(60,rec=iw) wk22
c
c== have SST category and chose comp
      avgsst=0.
      do jt=it-10,it-1
      avgsst=avgsst+sst1(jt)/10.
      enddo
c     ssta=(sst1(it)-avgsst)/std
      ssta=(sst1(it)-avgsst)
      write(6,*) 'avgsst=',avgsst,'  sst1=',sst1(it)
c
      if(ssta.le.crt1) then
        do id=1,imax
          cmpp(id)=cmp1(id)
        enddo
      else if(ssta.gt.crt1.and.ssta.le.crt2) then
        do id=1,imax
          cmpp(id)=cmp2(id)
        enddo
      else if(ssta.gt.crt2.and.ssta.le.crt3) then
        do id=1,imax
          cmpp(id)=cmp3(id)
        enddo
      else if(ssta.gt.crt3.and.ssta.lt.crt4) then
        do id=1,imax
          cmpp(id)=cmp3(id)
        enddo
      else if(ssta.ge.crt4) then
        do id=1,imax
          cmpp(id)=cmp3(id)
        enddo
      endif
c
c== construct forecast
      do id=1,imax
        avgt=0.
        do jt=it-10,it-1
          avgt=avgt+grid(id,jt)/10.
        enddo
c       prd2d(id,it-30)=avgt+cmpp(id)
        prd2d(id,it-30)=cmpp(id)
c       prd2d(id,it-30)=avgt
      enddo
c
      end do  !it
c
...
c
      call season_anom(fld3D,ltime+1,nys,it,iseason,imax,grid)
...
c     ssta=(sst1(it)-avgsst)/std
      ssta=(sst1(it)-avgsst)
c     write(6,*) 'avgsst=',avgsst,'  sst1=',sst1(it)
c
      if(ssta.le.crt1) then
        do id=1,imax
          cmpp(id)=cmp1(id)
...
C  3-category composite based on Nino34 SST
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
      SUBROUTINE comp_5ctg(sst,fld,crt1,crt2,crt3,crt4,cmp1,cmp2,cmp3,
     &cmp4,cmp5,imax,nt,its,ite)
      DIMENSION sst(nt),fld(imax,nt)
      DIMENSION cmp1(imax),cmp2(imax),cmp3(imax)
      DIMENSION cmp4(imax),cmp5(imax)
c
      do id=1,imax
...
c
      ...
      if(si.le.crt1) then
        nc1=nc1+1
        do id=1,imax
        cmp1(id)=cmp1(id)+fld(id,it)
        enddo
      else if(si.gt.crt1.and.si.lt.crt2) then
        nc2=nc2+1
        do id=1,imax
        cmp2(id)=cmp2(id)+fld(id,it)
        enddo
      else if(si.gt.crt2.and.si.lt.crt3) then
        nc3=nc3+1
        do id=1,imax
        cmp3(id)=cmp3(id)+fld(id,it)
        enddo
      else if(si.gt.crt3.and.si.lt.crt4) then
        nc4=nc4+1
        do id=1,imax
        cmp4(id)=cmp4(id)+fld(id,it)
        enddo
      else if(si.gt.crt4) then
        nc5=nc5+1
        do id=1,imax
        cmp5(id)=cmp5(id)+fld(id,it)
        enddo
      endif
c
      enddo  !it
c
      do id=1,imax
        cmp1(id)=cmp1(id)/float(nc1)
        cmp2(id)=cmp2(id)/float(nc2)
        cmp3(id)=cmp3(id)/float(nc3)
        cmp4(id)=cmp4(id)/float(nc4)
        cmp5(id)=cmp5(id)/float(nc5)
      enddo
      write(6,*) 'composite size:',nc1,nc2,nc3,nc4,nc5
c
      return
      end
...
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
      subroutine yr11_flt(fld,out,ncd,nt,its,ite)
...
C
      subroutine anom_wmo2(fldin,fldout,ncd,nt,it,ntprd,ndec)
...
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
C  normalize 1-d data
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
      subroutine normal_1d(a,b,sd,m,its,ite)
      real a(m),b(m)
c
      tt=ite-its+1
      c=0.
