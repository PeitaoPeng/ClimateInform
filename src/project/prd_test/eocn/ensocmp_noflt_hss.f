...
c filtering with EOF
      nw=2*ifld+15
c     call eof_flt(grid2,imax,ltime,ifld,mmax,nw,grid)
C
...
c== have ENSO composite for temp
      call comp_3ctg(sst1,grid2,crt1,crt2,cmp1,cmp2,cmp3,
     &imax,ltime,nys,it-1)
c
...
c
      if(ssta.le.crt1) then
        do id=1,imax
          cmpp(id)=cmp1(id)
        enddo
      else if(ssta.gt.crt1.and.ssta.lt.crt2) then
        do id=1,imax
          cmpp(id)=cmp2(id)
        enddo
      else if(ssta.ge.crt2) then
        do id=1,imax
          cmpp(id)=cmp3(id)
        enddo
      endif
...
c filtering with EOF
      nw=2*it+15
c     call eof_flt(grid2,imax,ltime,it,mmax,nw,grid)
C
c== have ENSO composite for temp
      call comp_3ctg(sst1,grid2,crt1,crt2,cmp1,cmp2,cmp3,
     &imax,ltime,nys,it-1)
...
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
      SUBROUTINE comp_3ctg(sst,fld,crt1,crt2,cmp1,cmp2,cmp3,
     &imax,nt,its,ite)
...
c
      ...
      enddo
      write(6,*) 'composite size:',nc1,nc2,nc3
c
      return
      end
