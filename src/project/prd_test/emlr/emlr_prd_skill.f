      ...
      dimension recons(ltime,lag) !nmode in ssa=lag
      dimension recall(mmax,ltime,lag,12) !recons all rpcs
      dimension out(36,19) !2d out
C
...
c
      ...
      open(unit=63,form='unformatted',access='direct',recl=4*ntout)
...
c
C== From JFM1991 to predict RPCs with SSA
c
      kt=0
...
c
      do m=1,mmax
         ...
         nrt=ifld-lag+1
         call ssa(fld1,recons,ltime,ifld,nrt,lag,lag)

         fldprd(m,kt,is)=recons(ifld,mssa)
c
         do im=1,lag
         recall(m,kt,im,is)=recons(ifld,im)
         enddo
c
         vrfy(m,kt-1,is)=fld2D(m,ifld)
      end do ! mode
      end do ! it
c
...
c
      ...
      write(6,*)'for meof=',meof,' mssa=',mssa, 'skill=',totrms,totacc
...
c anom w.r.t wmo clim
      ...
      do k=1,10
        kt=k+30+(id-1)*10
        fldout(i,kt-60)=fldin(i,kt)-clm(id,i)
      enddo
      enddo
      enddo
c
      ...
      do k=ks,nt !2001-cur (for ndec=6 at most 2020)
        fldout(i,k-60)=fldin(i,k)-clm(ndec,i)
      enddo
      enddo
