...
lcdir=/cpc/home/wd52pp/project/prd_test/pls
tmp=/cpc/home/wd52pp/tmp
sstdir=/cpc/home/wd52pp/data/ca_prd
satdir=/cpc/home/wd52pp/data/LF_prd
#
tgtyr=2017  #target year (NDJ&DJF should be in the same year as JFM, FMA et al)
nyear=87  # number of years of data to be loaded (1931-present)
#
mode=6    # number of modes kept for EOCN
mrot=8    # number of modes for rotation
#
cp $lcdir/CONUS.pls.fcst.f $tmp/CONUS.pls.f
...
#
ntskip=17 #skip 1931-1947
ltime=`expr $nyear - $ntskip`
if [ "$mons" -gt 10 ]; then lt=`expr $nyear - 18`; fi
ntvfc=`expr $lt - 33`  # number of years for verfication
curyr=`expr $tgtyr - 1`
yyp=`expr $tgtyr + 1`
...
cat > parm.h << feof
       ...
       parameter(iss_$var=$mons)
       parameter(iss_$tor=$mons)
       parameter(ndx=36,ndy=19)
       parameter(kp=30)
       parameter(ndec=6)  !# of wmo clim for the whole data length
feof
#
gfortran -o fcst.x reof.s.f CD102_2_2x2.s.f CONUS_pls_fcst.f
echo "done compiling"
#
/bin/rm $tmp/fort.*         
#
 ln -s $satdir/temp_102.txt  fort.10
 ln -s $sstdir/hadoisst.3mon.1948-curr.1x1.gr fort.11
#
 ln -s $datadir/eocnprd_temp.$season.hss_map.gr                 fort.15
 ln -s $datadir/eocnprd_temp.$season.hss_ts.gr                  fort.16
#
fcst.x > $datadir/eocnprd_hss.$season.out
#
