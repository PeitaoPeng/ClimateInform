...
C  skill (cor & rms) of PLS prd based on PC
C==========================================================
      include "parm.h"
      real r1d1(ntprd-30)
...
c
      ...
      open(unit=11,form='unformatted',access='direct',recl=4*im*jm)
...
c
      ...
      enddo
      enddo
c== read in sst data (1948-cur)
      ir=iss_sst
      do ny=1,nyr
        read(11,rec=ir) sst2d
        ir=ir+12
      enddo
c hcst start
      DO ity=1,nyr
        ...
        do i=1,im
        do j=1,jm
          w3d(i,j,it)=sst(i,j,iy)
        enddo
        enddo

100     continue
        enddo !iy loop
c== seasonal anomalies from 1948
      call season_anom(fld3D,nyr,iss_sat,imax,grid)
C== REOF analysis for the 1948-cur period
      ifld=nyr
      nw=2*ifld+15 
      call reof_rpc(grid,imax,ifld,ifld,mrot,mmax,nw,rpc,regr)
c
...
c
      ...
      subroutine season_anom(fld,nyr,mons,nmax,out)
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
C. calculate seasonal anom & clim of 102 cd data
C===========================================================
      real fld(nmax,nyr,12)
      real clim(nmax,12),out(nmax,nyr)
c
      call setzero(clim,nmax,12)
      do i=1,nmax
      do m=1,12
        do ny=1,nyr
          clim(i,m)=clim(i,m)+fld(i,ny,m)/float(nyr)
        enddo
      end do
      end do
c
