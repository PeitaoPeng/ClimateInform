...
c     parameter(ntout=ntprd-30)
      ...
      real pwk1(imax,ntprd,12),pwk2(imax,ntprd,12),pwk3(imax,ntprd,12)
      real vwk1(imax,ntprd,12)
      ...
      dimension wk3(102),wk4(102),wk5(102),wk6(102),wk7(102),wk8(102)
      ...
      dimension kmax(imax,12),kmax_t(imax,12,ntout)
      dimension kmax_t2(imax,12,ntout)
      dimension kmax_t3(imax,12,ntout)
      dimension acc_t(imax,12),acc_s(ntout,12)
      dimension acc_t2(imax,12),acc_s2(ntout,12)
      dimension acc_t3(imax,12),acc_s3(ntout,12)
      dimension rms_t(imax,12),rms_s(ntout,12)
      dimension rms_t2(imax,12),rms_s2(ntout,12)
      dimension rms_t3(imax,12),rms_s3(ntout,12)
      dimension out(36,19)
c
      ...
      open(unit=63,form='unformatted',access='direct',recl=4*36*19)
      open(unit=64,form='unformatted',access='direct',recl=4*imax)
...
c have seasonally independent OCN
     ...
     &ntprd,ndec,imax,kp,its,ite,kmax)
      ...
      do im=1,imax
      do is=1,12
        kmax_t(im,is,ite-60)=kmax(im,is)
      enddo
      enddo
c
 999  format(12I4)
cc write out kmax
      ...
      enddo
      iw=iw+1
      write(64,rec=iw) wk3  !write out kmax of each CD
...
c
      its=61
      ite=ltime
      call rms_skill(fld3D2,awmo,ltime,ntprd,imax,its,ite,
     &kmax_t,rmsm,rms_t,rms_s)

      call acc_skill(fld3D2,pwk1,vwk1,ltime,cwmo,
...
c skill of kmax uniformly =15
      ...
      enddo
      call rms_skill(fld3D2,awmo,ltime,ntprd,imax,its,ite,
     &kmax_t2,rmsm,rms_t2,rms_s2)
      call rms_skill(fld3D2,awmo,ltime,ntprd,imax,its,ite,
     &kmax_t3,rmsm2,rms_t3,rms_s3)

      call acc_skill(fld3D2,pwk2,vwk1,ltime,cwmo,
     &ntprd,ndec,imax,its,ite,kmax_t2,accm,acc_t2,acc_s2)
      call acc_skill(fld3D2,pwk3,vwk1,ltime,cwmo,
     &ntprd,ndec,imax,its,ite,kmax_t3,accm2,acc_t3,acc_s3)
c
      write(6,*) 'skill over 1991-2012 for ocn=15',rmsm,accm
      write(6,*) 'skill over 1991-2012 for ocn=10',rmsm2,accm2
cc write out acc&rmse skill in time domain
      iw=0
      do is=1,12
      do im=1,imax
        wk3(im)=acc_t(im,is)
      ...
      write(61,rec=iw) out
      iw=iw+1
      call CD102_2_2x2(wk6,out)
      write(61,rec=iw) out
      iw=iw+1
      call CD102_2_2x2(wk4,out)
      write(61,rec=iw) out
      iw=iw+1
      call CD102_2_2x2(wk7,out)
      write(61,rec=iw) out
      iw=iw+1
      call CD102_2_2x2(wk5,out)
      write(61,rec=iw) out
      iw=iw+1
      call CD102_2_2x2(wk8,out)
      write(61,rec=iw) out
      enddo
cc write out acc skill in spatial domain
      ...
      do is=1,12
        ...
        xrms3=rms_s3(it,is)
      iw=iw+1
      write(62,rec=iw) xacc1
      iw=iw+1
      write(62,rec=iw) xrms1
      iw=iw+1
      write(62,rec=iw) xacc2
      iw=iw+1
      write(62,rec=iw) xrms2
      iw=iw+1
      write(62,rec=iw) xacc3
      iw=iw+1
      write(62,rec=iw) xrms3
      enddo
      enddo
cc write out temp prediction
      ...
      do i=1,imax
        wk3(i)=pwk1(i,it+30,is)
        wk4(i)=pwk2(i,it+30,is)
        wk5(i)=pwk3(i,it+30,is)
        wk6(i)=vwk1(i,it+30,is)
      enddo
...
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
      subroutine acc1d_t(a,b,n,k,ite,ac)
      real a(n),b(n)
c
      ks=ite-30-k+1
      ke=ite-30
...
c 
      ...
      acc=0
      do j=ks,ke
      sd_a=sd_a+a(j)*a(j)/float(k)
      sd_b=sd_b+b(j)*b(j)/float(k)
      acc=acc+a(j)*b(j)/float(k)
      enddo
c
      sd_a=sqrt(sd_a)
      sd_b=sqrt(sd_b)
...
C  rms in time(n) and space(m) domain for the latest k yrs
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
      subroutine rms3d(a,b,c,m,n,k,ite,rms)
      real a(m,n,12),b(m,n,12)
      real c(m,n,12)
c
      ks=ite-30-k+1
      ...
      do i=1,m
      do is=1,12
      d=d+(a(i,j,is)-b(i,j,is))*(a(i,j,is)-b(i,j,is))/float(m*k*12)
      e=e+c(i,j,is)*c(i,j,is)/float(m*k*12)
      enddo
      enddo
      enddo
c     rms=sqrt(d/e)
...
C  rms in time(n) and space(m) domain for the latest k yrs
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
      subroutine rms1d_t(a,b,n,k,ite,c)
      real a(n),b(n)
c
      ks=ite-30-k+1
      ke=ite-30
...
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
      subroutine mean_stdv(a,n,avg_a,c)
      real a(n)
c
      avg_a=0.
      do i=1,n
...
c
      ...
      dimension nacc(kp)
      dimension kmax(imax,12)
      dimension wk1(ntprd),wk2(ntprd)
...
c
      ...
      do it=1,ntprd
      wk1(it)=fldprd(im,it,is)
      wk2(it)=  vrfy(im,it,is)
      enddo

      call acc1d_t(wk1,wk2,ntprd,myr,ite,ac)
      ...
      call xmaxm(nacc,kp,xac,kmax(im,is))
...
c
      ...
      subroutine acc_skill(fld3D,fldprd,vrfy,ltime,cwmo,
...
C== rms calculation 
      ...
      call rms1d_t(wk1,wk2,ntprd,myr,ite,rm)
      ...
      call xmini(nrms,kp,xms,kmax(im,is))
...
c
      subroutine rms_skill(fld3D,awmo,ltime,ntprd,imax,
     &its,ite,kmax,xrms,rms_t,rms_s)
c
      dimension fld3D(imax,ltime,12)
      dimension awmo(imax,ntprd,12)
      dimension fldprd(imax,ntprd,12),vrfy(imax,ntprd,12)
      dimension kmax(imax,12,ntprd-30)
      dimension rms_t(imax,12)
      dimension rms_s(ntprd-30,12)
c
      DO im=1,imax  ! loop over grid
      DO is=1,12  !season loop
C== prediction with OCN
...
c
      myr=ite-its+1   !with latest ltest years data
      call rms3d(fldprd,vrfy,awmo,imax,ntprd,myr,ite,xrms)
      call rms2d_t(fldprd,vrfy,awmo,imax,ntprd,myr,ite,rms_t)
      call rms2d_s(fldprd,vrfy,awmo,imax,ntprd,myr,ite,rms_s)
