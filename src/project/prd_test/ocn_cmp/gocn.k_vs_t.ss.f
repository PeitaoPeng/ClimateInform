C==========================================================
C  time matching OCN independent of cd
C==========================================================
      include "parm.h"
c     parameter(ntout=ntprd-30)
      ...
      dimension kmax(12),kmax_t(imax,12,ntout)
...
c
     ...
     &kmax_t,rmsm,rms_t,rms_s)
      call acc_skill(fld3D2,pwk,vwk,ltime,cwmo,
     &ntprd,ndec,imax,its,ite,kmax_t,accm,acc_t,acc_s)
      write(6,*) 'skill over 1991-2012 for ss dependent ocn',rmsm,accm
...
cc write out acc skill in spatial domain
      ...
      do is=1,12
        ...
        xrms=rms_s(it,is)
      iw=iw+1
      write(62,rec=iw) xacc
      iw=iw+1
      write(62,rec=iw) xrms
      enddo
      enddo
cc write out temp prediction
      ...
      do i=1,imax
        wk3(i)=pwk(i,it+30,is)
        wk4(i)=vwk(i,it+30,is)
      enddo
...
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
      subroutine acc2d_tcd(a,b,n,imax,k,ite,ac)
      real a(n,imax),b(n,imax)
c
      ks=ite-30-k+1
      ke=ite-30
c have mean
      ...
      do i=1,imax
      do j=ks,ke
c       avg_a=avg_a+a(j,i)/float(k)
c       avg_b=avg_b+b(j,i)/float(k)
      enddo
      enddo
c anom
      do i=1,imax
...
c 
      ...
      do i=1,imax
      do j=ks,ke
      sd_a=sd_a+a(j,i)*a(j,i)/float(imax*k)
      sd_b=sd_b+b(j,i)*b(j,i)/float(imax*k)
      acc=acc+a(j,i)*b(j,i)/float(imax*k)
      enddo
      enddo
c
      sd_a=sqrt(sd_a)
...
C  rms in time(n) and space(m) domain for the latest k yrs
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
      subroutine rms2d_tcd(a,b,n,imax,k,ite,c)
      real a(n,imax),b(n,imax)
c
      ks=ite-30-k+1
      ke=ite-30
      ...
      do j=ks,ke
      do i=1,imax
      c=c+(a(j,i)-b(j,i))*(a(j,i)-b(j,i))/float(k*imax)
      enddo
      enddo
      c=sqrt(c)
c
...
c
      ...
      dimension nacc(kp)
      dimension kmax(12)
      dimension wk1(ntprd,imax),wk2(ntprd,imax)
...
c
      ...
      DO im=1,imax  ! loop over grid
      do it=1,ntprd
      wk1(it,im)=fldprd(im,it,is)
      wk2(it,im)=  vrfy(im,it,is)
      enddo
      ENDDO   !loop over im (grid)
c
      call acc2d_tcd(wk1,wk2,ntprd,imax,myr,ite,ac)
      ...
      call xmaxm(nacc,kp,xac,kmax(is))
...
C== rms calculation 
      ...
      call rms2d_tcd(wk1,wk2,ntprd,imax,myr,ite,rm)
      ...
      call xmini(nrms,kp,xms,kmax(is))
