      ...
      dimension ms(kp),kmax(ntprd,mmax,12),kkk(ntprd,mmax,12)
      dimension xmac(mmax),xac(ntprd,mmax)
      dimension acx(mmax)
      dimension maxk(mmax),xmaxk(mmax)
      dimension stdk(mmax),kk(mmax,12)
...
c
      ...
      open(unit=63,form='unformatted',access='direct',recl=4)
      open(unit=64,form='unformatted',access='direct',recl=4*36*19)
...
c
      do m=1,mmax
      if(irms.eq.1) then
      call opt_k_rms(fld2D,kk(m,is),acx(m),
     &ltime,ntprd,mmax,kp,ifld,m)
      else
      call opt_k_acc(fld2D,kk(m,is),acx(m),
     &ltime,ntprd,mmax,kp,ifld,m)
      endif
c fill the 1st 10 of kamx with kk for later smoothing use
        do it=1,10
        kmax(it,m,is)=kk(m,is)
        enddo
      enddo
...
C
      ...
      do m=1,mmax
         avg=0
         do k=it-kk(m,is),it-1
         avg=avg+fld2D(m,k)/float(kk(m,is))
         end do
         fldprd(m,kt,is)=avg
...
c save ssclm for having totprd later
      do i=1,imax
        sclim(i,kt,is)=ssclm(i)
      enddo
      call reof_rpc(grid3,imax,ltime,ifld,mrot,mmax,nw,fld2D,eof)
      call d2_2_d3(eof,eof_t,imax,mmax,ntprd,kt) !save eofs for later use
      do m=1,mmax
      if(irms.eq.1) then
      call opt_k_rms(fld2D,kmax(it-50,m,is),xac(it-60,m),
     &ltime,ntprd,mmax,kp,ifld,m)
      else
      call opt_k_acc(fld2D,kmax(it-50,m,is),xac(it-60,m),
     &ltime,ntprd,mmax,kp,ifld,m)
      endif
      enddo
c
...
c have k smoothed
c
         kkk(it-60,m,is)=(kmax(it-50,m,is)+kmax(it-51,m,is)+
     &kmax(it-52,m,is)+kmax(it-53,m,is)+kmax(it-54,m,is))/5.
c
c use fcst with smoothed k
c
         do k=it-kkk(it-60,m,is),it-1
         avg=avg+fld2D(m,k)/float(kkk(it-60,m,is))
         end do
c
         fldprd(m,kt,is)=avg
...
c
      ...
      enddo
 999  format(8f7.2)
cc write out temp prediction
      iw=0
      do it=1,ntout
      do is=1,12

      do i=1,imax
        wk3(i)=prd2d(i,it+30,is)
        wk4(i)=awmo(i,it+30,is)
      enddo
