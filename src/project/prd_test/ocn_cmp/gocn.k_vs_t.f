...
c
      ...
      open(unit=65,form='unformatted',access='direct',recl=4*ntout)
      open(unit=70,form='unformatted',access='direct',recl=4*kp)
      open(unit=75,form='unformatted',access='direct',recl=4)
...
c have seasonally independent OCN
      ...
      call rms_k_kmax(fld3D2,ltime,ntprd,imax,kp,its,ite,rms,kmax,rmsm)
      kout(ite-60)=float(kmax)
      sout(ite-60)=rmsm
      write(6,*)'kmax=',kmax
      write(6,999) rms
      iw=iw+1
     ...
     &ntprd,ndec,imax,kp,its,ite,acc,kmax,accm)
      kout(ite-60)=float(kmax)
      sout(ite-60)=accm
      write(6,*)'kmax=',kmax
      write(6,999) acc
...
c
c wmo clim error for period of 1961-ite
      wrm=0.
      nyr=ite-its
      do it=its,ite-1
      do im=1,imax
      do is=1,12
      wrm=wrm+awmo(im,it-30,is)*awmo(im,it-30,is)/float(imax*nyr*12)
      enddo
      enddo
      enddo
      wrm=sqrt(wrm)
...
c
 ...
 999  format(10f7.3)
c
c have acc and rms skill vs K for 1991-cur period
c
      its=31
      ite=ltime
      call rms_vs_k(fld3D2,ltime,ntprd,imax,kp,its,ite,rms,kmax,rmsm)
      call acc_vs_k(fld3D2,ltime,cwmo,
     &ntprd,ndec,imax,kp,its,ite,acc,kmax,accm)
c wmo clim error for period of 1991-ite
      wrm=0.
      nyr=ltime-60
      do it=61,ltime
      do im=1,imax
...
c
      ...
      write(70,rec=3) wrms
c
c write out USAVG of awmo seasonal anomalies
c
      iw=0
      do it=1,ntprd
      do is=1,12
        avg=0.
        do im=1,imax
        avg=avg+awmo(im,it,is)/float(imax)
        enddo
        iw=iw+1
        write(75,rec=iw) avg
      enddo
...
100   format(1h ,'heidke A&B', 4f6.1,3i5,f6.0) 
     ...
     &ntprd,ndec,imax,kp,its,ite,acc,kmax,xac)
c
...
c
      ...
      subroutine acc_vs_k(fld3D,ltime,cwmo,
     &ntprd,ndec,imax,kp,its,ite,acc,kmax,xac)
c
      dimension fld3D(imax,ltime,12)
      dimension fldprd(imax,ntprd,12),vrfy(imax,ntprd,12)
...
C== acc calculation
c
      myr=ite-its-30+1   !with latest years data
      call acc3d(fldprd,vrfy,imax,ntprd,myr,ite,ac)
...
c
      subroutine rms_vs_k(fld3D,ltime,ntprd,imax,kp,
     &its,ite,rms,kmax,xms)
c
      dimension fld3D(imax,ltime,12)
      dimension fldprd(imax,ntprd,12),vrfy(imax,ntprd,12)
...
C== rms calculation 
c
      myr=ite-its-30+1   !with latest ltest years data
      call rms3d(fldprd,vrfy,imax,ntprd,myr,ite,rm)
