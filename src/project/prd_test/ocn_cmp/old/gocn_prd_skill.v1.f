...
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
      subroutine acc2d(a,b,m,n,k,ite,acm)
      real a(m,n),b(m,n),c(m)
      real avg_a(m),avg_b(m)
c
c     ks=ite-30-k+1
c     ke=ite-30
      ks=ite-k+1
      ...
      avg_b(i)=0.
      do j=ks,ke
c       avg_a(i)=avg_a(i)+a(i,j)/float(k)
c       avg_b(i)=avg_b(i)+b(i,j)/float(k)
      enddo
      enddo
c
      do i=1,m
...
c
      ...
      ac=0
      do j=ks,ke
      sd_a=sd_a+a(i,j)*a(i,j)/float(k)
      sd_b=sd_b+b(i,j)*b(i,j)/float(k)
      ac=ac+a(i,j)*b(i,j)/float(k)
      enddo
      sd_a=sqrt(sd_a)
      sd_b=sqrt(sd_b)
      c(i)=ac/(sd_a*sd_b)
