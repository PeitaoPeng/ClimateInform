...
C==========================================================
      ...
      dimension xmms(kp),kmax(ntprd)
      dimension xacc(ntprd)
...
c
      ...
      open(unit=55,form='unformatted',access='direct',recl=4)
...
c
      call opt_k(fld2D,wk2d1,wk2d2,wk1d1,kmax(it-60),xmms(it-60),
     &ltime,ntprd,imax,kp,it-1)
c
      do m=1,imax
         avg=0
         do k=it-kmax(it-60),it-1
         avg=avg+fld2D(m,k)/float(kmax(it-60))
         end do
         prd2d(m,it-30)=avg
      end do  !m
...
c
      ...
      write(55,rec=it) xmaxk
      enddo
c
      do it=1,ntprd-30
      write(6,*) 'it= ',it,'minimum rms= ',xmms(it)
      enddo
...
100   format(1h ,'heidke A&B', 4f6.1,3i5,f6.0) 
      ...
      subroutine opt_k(fld2D,fldprd,vrfy,score,maxk,xmms,
...
c
c     myr=ite-30  !with all available historic data
      myr=30      !with latest 30 yr data
...
c
      ks=30+(ndec-1)*10+1
      do i=1,ncd
      do k=ks,nt !2001-cur (at most 2020 for ndec=6)
...
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
      subroutine xmini(a,n,xout,kout)
      real a(n)
c
      xx=a(1)
      do i=1,n-1
