...
c rotated ssa by rotating eof_t 
ccccccccccccccccccccccccccccccccccccccccccccccccccc
      subroutine rssa(work,work2d,maxt,nd,nt,lag,nmode)
c     parameter(nt=nd-lag+1)
      dimension work(maxt),data(nt,lag),w1d(lag)
      dimension work2(nd),data2(nt,lag)
      dimension work2d(maxt,nmode)
...
c
c     open(20,form='unformatted',access='direct',recl=4*nt)
      iw=0
      do n=1,lag
        do k=1,nt
          wk(k)=u(k,n)
        enddo
        call norm1d(wk,maxt,nt,su(n))
        do k=1,nt
          u(k,n)=wk(k)
        enddo
      iw=iw+1
c     write(20,rec=iw) wk
      enddo
c
c-----write out the EOF modes
c
c     open(30,form='unformatted',access='direct',recl=4*lag)
      iw=0
      do k=1,lag
        do i=1,lag
      ...
      iw=iw+1
c     write(30,rec=iw)w1d
      enddo
c
c-----calculate percentage of variances by each mode
c
c     open(40,form='formatted')
      var=0.0
      do k=1,lag
      var=var+w(k)*w(k)
      enddo
c     write(40,*)'total var=', totalvar, var
c     write(40,*)'mode','         pct variance','      pct acc. var'
      vr=0.0
      do k=1,lag
      vp=w(k)*w(k)/var
      vr=vr+vp
c     write(40,*)'k=', k, '      ',vp, '    ',vr
      enddo
c
c----reconstruct time series and write them and original out
c
c     open(50,form='unformatted',access='direct',recl=4*nd)
c     write(50,rec=1)work  !write out original
c
      iw=1
      do nm=1,nmode
...
c
      ...
      work2d(i+j-1,nm)=data2(i,j)
      enddo
      enddo
c
      iw=iw+1
c     write(50,rec=iw)work2

      enddo  !loop nmode
c
c----write out reconstructed in time for easy compare with moving avg
c
c     open(60,form='unformatted',access='direct',recl=4)
      iw=0
      do it=1,nd
        iw=iw+1
c       write(60,rec=iw) work(it)
        do nm=1,8
        iw=iw+1
c       write(60,rec=iw) work2d(it,nm)
        enddo
      enddo
c
      return
...
c     write(6,*)'Increase memory in Subroutine ordersvd'
      stop 1200
      endif
c
...
c     write(6,*)'check first 10 values'
c
      do i=1,100
      aa(i)=0.0
      enddo
...
c
      ...
      enddo
      do k=1,mode
c     write(6,*)'var=',evar(k),' w2=',w(k)*w(k)
      enddo
      return
      end
c
