...
C       This program computes the AR model.
      ...
      PARAMETER (IO1=0,IO2=0,IO3=2,IO4=2,IO5=2)
      PARAMETER (NDT=8500,NPRD=24)
...
C         TS(IR) = RN(IR)*SQRT(SIG) - SIN(PI*I*100/PI)
        END DO
        DO I=1,NDT
          TS(I) = TS(I+NDT)
        END DO
        open(unit=7,form='unformatted',access='direct',recl=4)
        do I=1,NDT
        WRITE(7,rec=I) TS(I)
        enddo
...
C ------- Choose an order
20    CONTINUE
c     PRINT *, '  Type the order of AR model for a spectral density.'
c     READ(5,*,ERR=200) IP
      IP=2
      SIG = ERR(IP)
      A(0) = 1.
...
C95    PRINT *, '  Type the peak frequency number.'
C     READ(5,*,ERR=130)  IPK
      IPK=1
...
c     PRINT *, '  Type how many points you want to predict.'
c     READ *, NPRD
