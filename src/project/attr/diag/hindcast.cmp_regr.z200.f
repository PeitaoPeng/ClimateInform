...
C
      ...
      real bord(imx,jmx,2)
c
      real fld3d2(imx,jmx,nyr-1),fldn(imx,jmx,nyr-1)
...
C
C== Time loop from 1948 to curr to have forecast for each year
C
      iw=0
      DO it=1,ite

      ssttgt=ssti(it)  !nino34 in tgt year

c== fill data to t=1->ite-1 arrays
      kt=0
      do jt=1,ite
      if(jt.eq.it) go to 555 
      kt=kt+1
      do i=1,imx
      do j=1,jmx
...
C== composite based on nino34
      nyr2=nyr-1
      nyr3=ite-1
      call comp_5ctg(ssti2,fld3d2,imx,jmx,nyr2,nyr3,ssttgt,cmp,frq,
     &cmpp,freq,sst2,cmpn,fldn,cfdn,nc,bord,wk1,wk2)
c
      call regr_nino(ssti2,fld3d2,imx,jmx,nyr2,nyr3,ssttgt,regr,corr,
     &cons)
      do i=1,imx
      do j=1,jmx
...
c
      do i=1,imx
      do j=1,jmx
        if(abs(fld3d(i,j,1)).lt.99999) then
        do it=1,ite
        fld1d(it)=fcst(i,j,it)
        fld1d2(it)=fcst2(i,j,it)
        fld1d3(it)=fld3d(i,j,it)
        enddo
        call acc_s(fld1d,fld1d3,nyr,ite,acsp(i,j))
        call acc_s(fld1d2,fld1d3,nyr,ite,acsp2(i,j))
        else
        acsp(i,j)=-9999.0
        acsp2(i,j)=-9999.0
        endif
...
c     call normal(sst,nt,nt2,std)
      ...
      if(abs(fld(i,j,1)).lt.99999) then
