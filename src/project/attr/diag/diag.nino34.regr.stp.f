...
c
      ...
      enddo
c
      IF(abs(f2(1)).lt.undef) then
      v1=0.
      v2=0.
      do it=1,ite  !ite means the data used include target season
      v1=v1+f1(it)/float(ite)
      v2=v2+f2(it)/float(ite)
      enddo
      ...
      sd2=0.
      do it=1,ite !ite means the data used include target season 
         cor=cor+(f1(it)-v1)*(f2(it)-v2)/float(ite)
         sd1=sd1+(f1(it)-v1)*(f1(it)-v1)/float(ite)
         sd2=sd2+(f2(it)-v2)*(f2(it)-v2)/float(ite)
      enddo
c
      sd1=sd1**0.5
      sd2=sd2**0.5
      ...
      cons(i,j)=f1(ite)*cor/(sd1)  !constructed anom
      write(6,*) 'current Nino34 SST=',f1(ite)
      sstnm=f1(ite)/sd1
      write(6,*) 'normalized sst=',sstnm

c== 1981-2010 clim
      vc=0.
      do it=34,63 !1981-2010
c     do it=34,58 !1981-2005
      vc=vc+f2(it)/30.
c     vc=vc+f2(it)/25.
      enddo
      obs(i,j)=f2(ite)-vc
