'reinit'
'run /cpc/home/wd52pp/bin/white.gs'
'run /cpc/home/wd52pp/bin/rgbset.gs'

mtx=var
eof_range=tp
ts=rsd_rpc

'open /cpc/home/wd52pp/data/attr/telcon/cmap_20.'ts'.djf.contrib.ctl'

*---------------------------string/caption
 'set string 1 tc 5 0'
 'set strsiz 0.12 0.12'
 'draw string 4.25 10.75 Reconstruction of Prate for 2019/20 DJF'
 'set strsiz 0.12 0.12'
*---------------------------set dimsnesion, page size and style
*
nframe=8
nframe2=4

xmin0=1.2;  xlen=3.25;  xgap=-0.3
ymax0=10.25; ylen=-1.5; ygap=-0.3
*
'define p1=obs'
'define p2=n34'
'define p3=c1'
'define p4=c2'
'define p5=c3'
'define p6=trd'
'define p7=n34+trd+c2'
'define p8=p7+c1+c3+c4'

'set t 1'
*
iframe=1
ieof=1
ipc=1
while ( iframe <= nframe )
  icx=1; if (iframe > nframe2); icx=2; endif
  xmin=xmin0+(icx-1)*(xlen+xgap)
  xmax=xmin+xlen
  icy=iframe; if (iframe > nframe2); icy=iframe-nframe2; endif
  if(iframe >  4); ylen=-1.5; endif
  if(iframe >  4); xlen=3.25; endif
  if(iframe >  4); ygap=-0.3; endif
  if(iframe >  4); ymax0=10.25; endif
  ymax=ymax0+(icy-1)*(ylen+ygap)
* if(iframe = 7); ymax=ymax0+(icy-1)*(ylen+ygap)-0.1; endif
  ymin=ymax+ylen
* if(iframe = 6); titly=ymax+0.1; endif
* if(iframe = 7); titly=ymax+0.1; endif
  xstr=xmin + 0.25
  ystr=ymax + 0.15
* say xmin; say xmax; say ymin; say ymax
  'set vpage 0 8.5 0 11'
  'set parea 'xmin' 'xmax' 'ymin' 'ymax
*
'set grads off'
*if(iframe < 6);
*'set mproj scaled'
'set lat 20. 70'
'set ylint 10'
'set lon 190 300'
'set xlint 20'
'set grid on'
*'set poli on'
*'set mpdset mres'
*'set gxout grfill'
'set xlab off'
'set ylab off'
*if(iframe<= 5);'set ylab on';endif
if(iframe< 5);'set ylab on';endif
if(iframe=4);'set xlab on';endif
if(iframe=8);'set xlab on';endif
*
'set gxout shaded'
*'set clevs  -0.3 0.3'
*'set ccols  22 0 22'
*'d c'%ieof
*'set gxout contour'
*'set cint 5'
'set clevs  -2.0 -1.6 -1.2 -0.8 -0.4 -0.1 0.1 0.4 0.8 1.2 1.6 2'
'set ccols  79 77 76 75 74 72 0 32 34 35 36 37 39'
'd p'%ieof
*'run /cpc/home/wd52pp/bin/dline.gs 120 0 360 0'
*'run /cpc/home/wd52pp/bin/dline.gs 180 0 180 90'
'set string 1 tl 5'
'set strsiz 0.11 0.11'
if(iframe = 1);'draw string 'xstr' 'ystr' a)OBS'; endif
if(iframe = 2);'draw string 'xstr' 'ystr' b)Nino3.4'; endif
if(iframe = 3);'draw string 'xstr' 'ystr' c)RPC1'; endif
if(iframe = 4);'draw string 'xstr' 'ystr' d)RPC2'; endif
if(iframe = 5);'draw string 'xstr' 'ystr' e)RPC3'; endif
if(iframe = 6);'draw string 'xstr' 'ystr' f)Trend'; endif
if(iframe = 7);'draw string 'xstr' 'ystr' g)Nino34+Trend+RPC2'; endif
if(iframe = 8);'draw string 'xstr' 'ystr' h)Nino34+trend+RPC1-4'; endif
*endif
*----------
'set string 1 tl 5'
if(iframe = 8);'run /cpc/home/wd52pp/bin/cbarn.gs 0.9 0 4.25 2.75';endif
*

iframe=iframe+1
ieof=ieof+1
ipc=iframe-4

endwhile
*
'printim cmap.'ts'.contrib.20.na.png gif x1200 y1600'
