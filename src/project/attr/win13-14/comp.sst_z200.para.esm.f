...
C===========================================================
      ...
      real z4d(imx,jmx,5,nyr),t4d(imx,jmx,5,nyr)
      real z3d(imx,jmx,nyr),t3d(imx,jmx,nyr)
      real cr1d(500)
C
...
c
      ...
      open(60,form='unformatted',access='direct',recl=4*imx*jmx) 
...
c read in nmon-month sst data
      ...
      do iy=1,nyr
        ...
        do i=1,imx
        do j=1,jmx
          t4d(i,j,im,iy)=w2d(i,j)
        enddo
        enddo
       enddo !im loop
      enddo ! iy loop
c average djfm data for both z200 and sst
      do iy=1,nyr
      ...
      call setzero(w2d2,imx,jmx)
        ...
        do j=1,jmx
          ...
          w2d2(i,j)=w2d2(i,j)+t4d(i,j,im,iy)/4.
          enddo
          z3d(i,j,iy)=w2d(i,j)
          t3d(i,j,iy)=w2d2(i,j)
        enddo
...
c composite for Z200 and SST
      call setzero(w2d,imx,jmx)
      call setzero(w2d2,imx,jmx)
      call setzero(w2d3,imx,jmx)
      call setzero(w2d4,imx,jmx)
      crt=0.5 !criterion of cor
      crtn=-1*crt
      npc=0
      nnc=0
      do iy=1,nyr-1
