...
if [ " ${ptest}" = " ${gtest}" ]; then
   echo " Half-month data 102 forecast divs computed  , JOB "${pgm}" STATUS = 0" >>${logdir}/${pid}status.txt
   echo " Half-month data 102 forecast divs computed  , JOB "${pgm}" STATUS = 0"
else
   echo " Half-month data update for 102 forecast divs failed , JOB "${pgm}" STATUS = "${ptest} >>${logdir}/${pid}status.txt 
fi
