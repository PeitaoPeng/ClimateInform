...
# ACCOUNT  dunger.tst                                                 #
#---------------------------------------------------------------------#
# INITIATION METHOD:  Called from Parent Script NCDCprelim.ksh or     #
#    NCDCfinal.ksh.  Can be run independently from parent scripts too.#
#---------------------------------------------------------------------#
# PURPOSE: Reformats the by-month climate division dataset to a by-   #
#      division dataset                                               #
#---------------------------------------------------------------------#
# USAGE:   ./mkcdlong.ksh $1 $2 $3                                    #
#---------------------------------------------------------------------#
...
if [ " ${ptest}" = " ${gtest}" ]; then
   echo ${3}".long files updated  , JOB "${pgm}" STATUS "${3}" = 0" >>${logdir}/${pid}status.txt
   echo ${3}".long files updated  , JOB "${pgm}" STATUS "${3}" = 0" 
else
   echo ${3}" .long update failed , JOB "${pgm}" STATUS "${3}" = "${ptest} >>${logdir}/${pid}status.txt 
fi
