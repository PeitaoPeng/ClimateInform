...
!  FT 12   CALIBRATION CONSTANTS CORRECTING NCDC GRED TO NCDC STATION  *
...
USE NcdcDiv
      ...
      INTEGER nu,nfile,kvn,nuts,nups,nut,nup,nerr,jyncdc1,jys,jms
...
!       Copy data through the month before we start to adjust
      CALL CPYCDDATA(14,jyncdc1,jm1,jym1,jmm1,1,nerr,ndcd)
      CALL CPYCDDATA(16,jyncdc1,jm1,jym1,jmm1,2,nerr,ndcd)
...
!            Read, adjust, and write data to the cumulative dataset
            CALL RDCDDATA(18,cdt,jyr,jm,jh,kyymm,1,jflagt,nerr,ndcd)
             IF(nerr /= 0) THEN
                 PRINT *,'Error reading cpc temps ',nuct,jyr,jm,nerr
                 STOP
             ENDIF
             CALL ADJCD(cdt,cdts,jm,1)
             CALL RDCDDATA(19,cdp,jyr,jm,jh,kyymm,2,jflagp,nerr,ndcd)
             IF(nerr /= 0) THEN
                 PRINT *,'Error reading cpc temps ',nuct,jyr,jm,nerr
                 STOP
             ENDIF
             CALL ADJCD(cdp,cdps,jm,2)
            CALL WRTCDDATA(15,cdts,jyr,jm,jh,1,ndcd,jflagt,nerr,ndcd)
            CALL WRTCDDATA(17,cdps,jyr,jm,jh,2,ndcd,jflagp,nerr,ndcd)
         END DO
      END DO
      IF( nerr == 0) THEN 
        print *,"PROGRAM COMPLETE STATUS = 0"
      ELSE
