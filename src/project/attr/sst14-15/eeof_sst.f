...
c     PARAMETER (nfld=lreal-ml+1,nw=2*nfld+15,nmod=10)
      PARAMETER (nw=2*nfld+15,nmod=10)
c     PARAMETER (lats=35,late=75,lons=60,lone=150) !Pan Pacific: 20S-60N, 120E-60W
      PARAMETER (lats=15,late=75,lons=1,lone=180) !global SST: 20S-60N, 120E-60W
c     PARAMETER (ngrd=2996) !pan Pacific 
      PARAMETER (ngrd=8281) !global ocean 
      real fld3d(imx,jmx,lreal)
      real fld4d(imx,jmx,ml,nfld)
      real reg4d(imx,jmx,ml,nmod)
      real fldin(imx,jmx)
...
c
      ...
      do m=1,nmod
        ...
        con4d2(i,j,mt,it)=con4d(i,j,mt,it)+rcoef(m,it)*reg4d(i,j,mt,m)
      enddo
      else
