...
C     DVGSET=0  no anom div globally
C           =1  have anom div globally
...
C
c     read(21,rec=4)FORBLK !djf1982/83
c     read(21,rec=16)FORBLK !regression to SST PCs
      CALL norsou(FORBLK,EDIVG,IX,IY)  ! EDIVG: anomaly year total field
C----------------------------------------------
...
c     ADIVG(i,j)=1*EDIVG(i,j)*0.000001
      end do
      end do
C --------------------------------
C   if need to prescibe idealized div
C --------------------------------
c     CALL IDEAFRC(FORBLK2,24,19,4.,2.,-3.)
c     CALL IDEAFRC(FORBLK3,35,21,6.,2.,4.)
c simulate dec2013
