...
C
      ...
      real freq(imx,jmx),cmpp(imx,jmx)
...
c==make composite for each category
      ...
      do it=1,ite-1 !use previous data for comp
c
      si=sst(it)
      if(si.gt.crt1) then
        nc(1)=nc(1)+1
...
c
      ...
      if(cmpn(i,j,k).ge.aa)then

      do it=1,nc(k)
        if(cfdn(i,j,it,k).ge.aa) then
        frq(i,j,k)=frq(i,j,k)+100./float(nc(k))
        endif
      enddo

      else if(cmpn(i,j,k).lt.aa.and.cmpn(i,j,k).gt.bb) then

      do it=1,nc(k)
        if(cfdn(i,j,it,k).lt.aa.and.cfdn(i,j,it,k).gt.bb) then
        frq(i,j,k)=frq(i,j,k)+100./float(nc(k))
        endif
      enddo

      else if(cmpn(i,j,k).le.bb) then

      do it=1,nc(k)
        if(cfdn(i,j,it,k).lt.bb) then
        frq(i,j,k)=frq(i,j,k)+100./float(nc(k))
        endif
      enddo
