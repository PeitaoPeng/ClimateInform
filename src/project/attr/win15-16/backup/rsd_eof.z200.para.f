...
C===========================================================
      ...
      real w3d(imx,jmx,ltime,nesm+1),w3d2(imx,jmx,ltime)
      real w2d(imx,jmx),w2d2(imx,jmx)
      real aa(imx,jmx),bb(imx,jmx)
      real regr(imx,jmx)
      real tcof(nmod+1,ltime+1)
      real cof(nmod+1,ltime+1)
C
...
C
      open(21,form='unformatted',access='direct',recl=4)
c
...
C==read in obs
      ...
      do it=1,ltime
        ...
        do i=1,imx
        do j=1,jmx
          w3d(i,j,it,ie)=fld(i,j) !for z200
        enddo
        enddo
      itr=itr+1
      enddo
...
c
      do it=1,ltime
        itr=iskip_idx+it
      read(21,rec=itr) sst(it)
      enddo
      call anom(sst,ltime)
      write(6,*) 'nino34=', sst
c
c== put non-normalized nino34 aside for later us
c
      do m=1,nmod+1
      do it=1,ltime+1
        tcof(m,it)=-9999.0
...
c
      ...
      do it=1,ltime
      ts1(it)=w3d(i,j,it,ie)
      enddo
      call anom(ts1,ltime)
      do it=1,ltime
      w3d(i,j,it,ie)=ts1(it)
      enddo
...
c
      do it=1,ltime
        do i=1,imx
        do j=1,jmx
          w3d2(i,j,it)=w3d(i,j,it,ie)-ts1(it)*regr(i,j)
        enddo
        enddo
      enddo
c
...
c
      do m=1,nmod
      do it=1,ltime
        cof(m+1,it)=coef(m,it)
c       tcof(m+1,it)=rcoef(m,it)
      enddo
      enddo
c
