...
C parameters need to be changed are: nws, nkeeps, nkeepe
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
...
c     parameter(imp=72,jmp=24)   ! grid size of prcp
      ...
      parameter(nws=25,nps=6)   ! 25 winters
...
c     open(unit=40,form='formatted')     !corr skill
      ...
      open(unit=60,form='unformatted',access='direct',recl=imp*jmp) !real prd
c*************************************************
c     if (natura.eq.1)write(6,307)
c     if (natura.eq.0)write(6,308)
c*************************************************
c*
      do j=1,jmp
...
c* keep the data for prd test
        ...
        nkeeps=(iw-1)*12+9    !1st used mon of current year
        nkeepe=iw*12+2        !mon of next year
c       nkeeps=(iw-1)*12+7    !1st used mon of current year
c       nkeepe=(iw-1)*12+12   !last used mon of current year
c* nkeeps and nkeepe need to be changed each time
        np=1
      ...
      ENDDO
        icur=nkeeps+13   !IC field
        read(10,rec=icur) fldin
        do i=1,imp
        do j=1,jmp
          obscur(i,j)=fldin(i,j+24)
        enddo
        enddo
...
c
        ...
        do nmode=1,modemax
          call inprod4(e,obscur,imp,jmp,cosl,nmode,modemax,anorm)
          eobscur(nmode)=anorm
        enddo
c
c* cross validated CA hind_cast begins
c
...
c
 ...
 555    continue
        ...
        enddo
        write(6,*) 'alpha square=',alphas

 222    continue

        do nmode=1,modemax
c         etgt3d(nmode,ntgtp,ntgtw)=0.
        enddo

        do nmode=1,modemax
        do ntime=1,nhs
c       etgt3d(nmode,ntgtp,ntgtw)=etgt3d(nmode,ntgtp,ntgtw)+
c    &           ana(nmode,ntime)*alpha3d(ntime,ntgtp,ntgtw)
        enddo
        enddo
...
c* EOF filtering for data used to construct prd 
      do it=1,nhs
        ...
        do j=1,jmp
        do i=1,imp
c         aaa2(id,it)=fld3d(i,j,it)*sqrt(COSL(j))
          id=id+1
        end do
        end do
      enddo
...
c* pattern ac
       ...
       DO ip=1,npp
         ...
         enddo
         if(ldpen.eq.1) then
c          write(20) obs     !write out obs
         endif
         write(20+ldpen) prd !write out prd
c
...
c
c* save stdv and skill for write out later
c
      IF(ldpen.gt.1) then
      ...
      do ip=1,npp
        do i=1,imp
        do j=1,jmp
          stdvo(i,j,iprd)=stdvo(i,j,iprd)+
     &                     fld4d(i,j,ip+ldpen-1,iw)**2/ttl
          stdvp(i,j,iprd)=stdvp(i,j,iprd)+
     &                     fld4d2(i,j,ip,iw)**2/ttl
          acprd(i,j,iprd)=fld2d(i,j)
        enddo
        enddo
      enddo
      enddo
      ...
      do i=1,imp
      do j=1,jmp
        stdvo(i,j,iprd)=sqrt(stdvo(i,j,iprd))
        stdvp(i,j,iprd)=sqrt(stdvp(i,j,iprd))
      enddo
      enddo
      ...
      ENDDO  !loop for hind_cast ldpen
c
c* CA for real time prd begins
c
        ...
        do ip=1,npp
          do nmode=1,modemax
            ana2(nmode,it)=efld3d(nmode,ip,iw)
          enddo
        it=it+1
        enddo
        enddo  
        ...
        do nmode=1,modemax
        ana2(nmode,npp*nws+1)=eobscur(nmode)
        enddo

        call getalpha(ana2,alpha2,modemax,nws*npp,fak,ridge)
      write(6,*) 'alpha2='
      write(6,*) alpha2

      DO ldpen=1,3  !1: 0 mon lead; 2: 1 mon lead; ...

        it=1
        do iw=1,nws       !loop for the historical winter
        do ip=1,npp
          do i=1,imp
          do j=1,jmp
            fld3d(i,j,it)=fld4d(i,j,ip+ldpen,iw)
          enddo
          enddo
        it=it+1
        enddo
        ...
        do nt=1,nws*npp
        fld2d(i,j)=fld2d(i,j)+
     &           fld3d(i,j,nt)*alpha2(nt)
        enddo
        enddo
        enddo
        write(60) fld2d
        write(60) obscur  !serves as persistent prd

        do i=1,imp
        do j=1,jmp
          fld2d(i,j)=stdvo(i,j,ldpen)
          obs(i,j)=stdvp(i,j,ldpen)
          obs2(i,j)=acprd(i,j,ldpen)
        enddo
        enddo

        write(60) fld2d   !stdv of obs
        write(60) obs     !stdv of prd
        write(60) obs2    !temporal acc of prd

      ENDDO   !loop for real time prd
