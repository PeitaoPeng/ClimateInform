...
#======================================
# have SST IC
# update HAD-OI SST first in ../data_proc, then do hte following
#======================================
icmon_beg=aug2015
icmon_mid=sep2015
icmon_end=oct2015
...
cat >sstic.gs<<EOF
'reinit'
'open $datadir/hadoisst.mon.1948-curr.ctl'
'set x 1 180'
'set y 1  89'
'set gxout fwrite'
'set fwrite $outdata/$outfile.gr'
'd ave(sst,time=$icmon_beg,time=$icmon_end)'
'c'
EOF
grads -l <havesstic
...
#
ln -s $outdata/real_ca_prd.${sst_analysis}.${eof_range}.$modemax.3mon.gr fort.85
#
ca.x > $outdata/realtime.${sst_analysis}.${eof_range}.$modemax.out
#ca.x 
#
...
EOF
#
cat>$outdata/real_ca_prd.${sst_analysis}.${eof_range}.$modemax.3mon.ctl<<EOF
dset ^real_ca_prd.${sst_analysis}.${eof_range}.$modemax.3mon.gr
undef -9.99E+33
title EXP1
xdef  180 linear   0. 2.
ydef   89 linear -88. 2.
