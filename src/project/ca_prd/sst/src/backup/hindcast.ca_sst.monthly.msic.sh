...
nrskip=0
if [ $sst_analysis = 'hadoisst' ]; then  nrskip=1; fi #because hadoisst is from jan1949
#
...
cp $lcdir/hindcast.ca_sst.monthly.msic.f $tmp/sst_hcst.f
...
ln -s $datadir/${sst_analysis}.mon.1949-curr.1x1.gr fort.12
#
ln -s $outdata/eof.${sst_analysis}.$icseason.${mseason}ics.${eof_range}.gr fort.70
#
ln -s $outdata/ca_hcst.skill.${sst_analysis}.$modemax.$icmon.${mseason}ics.${eof_range}.mon.gr fort.85
ln -s $outdata/ca_hcst.${sst_analysis}.$modemax.$icmon.${mseason}ics.${eof_range}.mon.gr fort.86
#
...
EOF
#
cat>$outdata/ca_hcst.skill.${sst_analysis}.$modemax.$icmon.${mseason}ics.${eof_range}.mon.ctl<<EOF
dset ^ca_hcst.skill.${sst_analysis}.$modemax.$icmon.${mseason}ics.${eof_range}.mon.gr
undef -9.99E+8
title EXP1
...
EOF
#
cat>$outdata/ca_hcst.${sst_analysis}.$modemax.$icmon.${mseason}ics.${eof_range}.mon.ctl<<EOF
dset ^ca_hcst.${sst_analysis}.$modemax.$icmon.${mseason}ics.${eof_range}.mon.gr
undef -9.99E+8
title EXP1
XDEF 360 LINEAR    0.5  1.0
YDEF 180 LINEAR  -89.5  1.0
