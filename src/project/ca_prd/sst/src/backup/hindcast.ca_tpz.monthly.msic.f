...
c
      ...
      dimension tgttpz4d(ims,jms,npp,nseason)
...
C
      open(unit=11,form='unformatted',access='direct',recl=4*ims*jms) !input historical ss sst
      open(unit=12,form='unformatted',access='direct',recl=4*ims*jms) !input historical mon sst
c
...
c* input data for EOF analysis
      ...
      enddo
c     write(6,*) 'length of input data to EOF calculation=',it
c     write(6,*) 'grid # of SST ngrd=',id
c       
...
c for sst
     ...
     &           sstana(i,j,ntime)*alpha3d(ntime,1,ntgts)
      ...
      if(abs(tpz4d2(i,j,npp,nseason)).lt.999) then
      do ntime=1,nhs
...
c for sst
     ...
     &*tpzstdo(i,j,ldpen)/tpzstdf(i,j,ldpen)
      enddo
      else
