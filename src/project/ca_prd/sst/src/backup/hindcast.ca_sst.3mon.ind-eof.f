...
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
      parameter(ngrd=5647)
c     parameter(ngrd=4935)
c range for sst pat cor (ac_1d)
      parameter(imss=76,imse=141)   ! 150->280
      parameter(jmss=1,jmse=26)     ! 25S->25N
      parameter(kpdf=100)   ! # of pdf bin
c
      parameter(ntt=nseason*npp,nhs=(nseason-3)*npp)
      parameter(ntt2=(nseason-32)*npp)
c
      dimension ts1(ntt2),ts2(ntt2)
      dimension fin(nseason-2),vfy(nseason-32),ocn(nseason-32)
      dimension ts3(mldp),ts4(mldp)
      dimension ts5(mldp),ts6(mldp)
      dimension ts7(nseason-2),ts8(nseason-2)
      dimension ac1d(mldp,ntt2),rms1d(mldp,ntt2)
      dimension sstac(imp,jmp,mldp),sstrms(imp,jmp,mldp)
      dimension fldin(ims,jms),fldin2(ims,jms)
      dimension xlat(jmp),fldp(imp,jmp)
      dimension alpha(nhs),alpha2(nhs),corr(imp,jmp)
      dimension alpha3d(nhs,npp,nseason)
      dimension obssst(imp,jmp),sstvfy4d(imp,jmp,npp,nseason-32,mldp)
      dimension prdsst(imp,jmp),sstprd(imp,jmp,mldp)
      dimension sstprd4d(imp,jmp,npp,nseason,mldp)
      dimension fld2d(imp,jmp),fldic(imp,jmp)
      dimension wk2d(imp,jmp),wk2d2(imp,jmp)
      dimension wk2d3(imp,jmp),wk2d4(imp,jmp)
      dimension allsst(ims,jms,12+20,nyear)
      dimension fld4d(imp,jmp,nps,nseason)
      dimension fld4d2(imp,jmp,nps,nseason-3)
      dimension sstana(imp,jmp,nhs+1)
      dimension tgt2d(imp,jmp),tgt3d(imp,jmp,npp) 
      dimension tgtsst4d(imp,jmp,npp,nseason)
      dimension sstic(imp,jmp)
      dimension aaa(ngrd,ntt)
      dimension bbb(ngrd,nhs)
      dimension efld3d(modemax,nps,nseason-3)
      dimension efldic(modemax)
...
C
      dimension sststdo(imp,jmp,mldp)
      dimension sststdf(imp,jmp,mldp)
c
...
c
      open(unit=60,form='unformatted',access='direct',recl=4*imp*jmp) !EOFs for proj use
      open(unit=61,form='unformatted',access='direct',recl=4*imp*jmp) !chose ind-eof
...
c
      open(unit=85,form='unformatted',access='direct',recl=4*imp*jmp) !skill
      open(unit=86,form='unformatted',access='direct',recl=4*imp*jmp) !obs&prd
c*************************************************
c
      PI=4.*atan(1.)
      CONV=PI/180.
...
c
       ...
       Enddo
c
c prepare data for EOF analysis
c
      iws=itgtm 
      iwe=itgtm+nps-1
...
c       write(6,*)m1,anorm
        ...
        write(60,rec=m1) fld2d ! write out EOFs
        enddo  !loop for m1
...
c
c* input data for independent EOF analysis
c
      IF(ldpen.gt.1) go to 333
      is2=0
      it=0
      do is=1,nseason
       ...
       do ip=1,npp
        ...
        do i=1,imp
          if(abs(fld4d(i,j,npp,nseason)).lt.999) then
          id=id+1
          bbb(id,it)=fld4d(i,j,ip,is)*sqrt(cosl(j))
          endif
        end do
        end do
       enddo !ip
c* have fld4d2 for later use
        is2=is2+1
        do j=1,jmp
        do i=1,imp
        do ip=1,nps
          fld4d2(i,j,ip,is2)=fld4d(i,j,ip,is)
        end do
        end do
        end do
  555 continue
      enddo !is
c       
      call eofs_4_ca(bbb,ngrd,nhs,nhs,eval2,evec2,pc2,wk2,0)
c
      write(6,*)'ntgts=',ntgts
      write(6,*)'eval2='
      write(6,400) eval2
c
c* have 2D independent EOF 
c
        do nm=1,modemax
        ng=0
        do j=1,jmp
...
c
c* standardize spatial patterns to unity for indp-eof
c
      do m1=1,modemax
...
c       write(6,*)m1,anorm
      ...
      if(ntgts.eq.5) then
        ...
        write(61,rec=m1) fld2d ! write out indp-EOFs for chosed target season
      endif
      enddo  !loop for m1
c
C* project grid data onto indp-EOFs*******
c
c for historical data
        do is=1,nseason-3
        do ip=1,npp
        call f4d_2_f2d(imp,jmp,nps,nseason-3,ip,is,fld2d,fld4d2)
        do nmode=1,modemax
...
c
      DO ntgtp=1,npp
c for target year ic
        do i=1,imp
        do j=1,jmp
          fldic(i,j)=fld4d(i,j,ntgtp,ntgts)
        end do
        end do
        do nmode=1,modemax
          call inprod4(eof,fldic,imp,jmp,cosl,nmode,modemax,anorm)
          efldic(nmode)=anorm
        enddo 
c
        it=0
...
c put prd of each ldpen into 4d array for later analysis
      ...
      do i=1,imp
      do j=1,jmp
        sstprd4d(i,j,ip,is,ldpen)=tgtsst4d(i,j,ip,is)
      enddo
      enddo
      ENDDO
      ENDDO
...
c for sst
      ...
      do ip=1,npp
        ...
        ts2(it)=sstprd4d(i,j,ip,is,ldpen)
      enddo
      enddo
...
c
      write(6,*) 'adjust the amplitude of forcast'
c for sst
      do i=1,imp
      do j=1,jmp
      if(abs(fld4d(i,j,npp,nseason)).lt.999) then
      do is=1,nseason
      do ip=1,npp
        sstprd4d(i,j,ip,is,ldpen)=sstprd4d(i,j,ip,is,ldpen)
     &*sststdo(i,j,ldpen)/sststdf(i,j,ldpen)
      enddo
      enddo
      else
        sstprd4d(i,j,ip,is,ldpen)=undef2
      endif
      enddo
      enddo
c
cc have verification
      do i=1,imp
      do j=1,jmp
      if(abs(fld4d(i,j,npp,nseason)).lt.999) then
      do is=33,nseason
      do ip=1,npp
        sstvfy4d(i,j,ip,is-32,ldpen)=fld4d(i,j,ip+ldpen-1,is)
      enddo
      enddo
      else
      do is=33,nseason
      do ip=1,npp
        sstvfy4d(i,j,ip,is-32,ldpen)=undef2
      enddo
      enddo
      endif
      enddo
...
c for sst
      write(6,*) 'for sst data'
       ...
       np=np+1
         do i=1,imp
         do j=1,jmp
           obssst(i,j)=sstvfy4d(i,j,ip,is-32,ldpen)
           prdsst(i,j)=sstprd4d(i,j,ip,is,ldpen)
         enddo
         enddo
c
      call rmsd1(obssst,prdsst,imp,jmp,imss,imse,jmss,jmee,cosl,
...
c* for sst
      ...
      if(abs(fld4d(i,j,npp,nseason)).lt.999) then
        do is=1,nseason-32
        do ip=1,npp
          ts1(is)=fld4d(i,j,ip+ldpen-1,is+32)
          ts2(is)=sstprd4d(i,j,ip,is+32,ldpen)
        enddo
        enddo
...
c write out hindcast and obs
      ...
      do i=1,imp
      do j=1,jmp
        wk2d(i,j)=sstvfy4d(i,j,ip,is-32,ldpen)
        wk2d2(i,j)=sstprd4d(i,j,ip,is,ldpen)
      enddo
      enddo
