...
tmp=/cpc/home/wd52pp/tmp/test2
#tmp=/cpc/home/wd52pp/tmp
if [ ! -d $tmp ] ; then
  mkdir -p $tmp
fi
datadir=/cpc/home/wd52pp/data/ca_prd
outdata=/cpc/consistency/id/ca_hcst/monthly
#
cd $tmp
#
for icseason in jfm fma mam amj mjj jja jas aso son ond ndj djf; do
#
if [ $icseason = jfm ]; then ic_midmon=feb; icmon=mar; ic_ss=1; fi
if [ $icseason = fma ]; then ic_midmon=mar; icmon=apr; ic_ss=2; fi
if [ $icseason = mam ]; then ic_midmon=apr; icmon=may; ic_ss=3; fi
if [ $icseason = amj ]; then ic_midmon=may; icmon=jun; ic_ss=4; fi
if [ $icseason = mjj ]; then ic_midmon=jun; icmon=jul; ic_ss=5; fi
if [ $icseason = jja ]; then ic_midmon=jul; icmon=aug; ic_ss=6; fi
if [ $icseason = jas ]; then ic_midmon=aug; icmon=sep; ic_ss=7; fi
if [ $icseason = aso ]; then ic_midmon=sep; icmon=oct; ic_ss=8; fi
if [ $icseason = son ]; then ic_midmon=oct; icmon=nov; ic_ss=9; fi
if [ $icseason = ond ]; then ic_midmon=nov; icmon=dec; ic_ss=10; fi
if [ $icseason = ndj ]; then ic_midmon=dec; icmon=jan; ic_ss=11; fi
if [ $icseason = djf ]; then ic_midmon=jan; icmon=feb; ic_ss=12; fi
#
...
cp $lcdir/hindcast.ca_tpz.monthly.msic.f $tmp/sst_hcst.f
...
ln -s $datadir/$var.1949_cur.mon.1x1.gr         fort.12
#
ln -s $outdata/ca_hcst_skill.$var.${sst_analysis}.$modemax.$icmon.${mseason}ics.${eof_range}.mon.gr fort.85
ln -s $outdata/ca_hcst.$var.${sst_analysis}.$modemax.$icmon.${mseason}ics.${eof_range}.mon.gr fort.86
#
#ca.x > $outdata/ca_hcst.$var.${sst_analysis}.$modemax.$icmon.${mseason}ics.${eof_range}.mon.out
ca.x 
#
undef_data=-9.99E+8
cat>$outdata/ca_hcst_skill.$var.${sst_analysis}.$modemax.$icmon.${mseason}ics.${eof_range}.mon.ctl<<EOF
dset ^ca_hcst_skill.$var.${sst_analysis}.$modemax.$icmon.${mseason}ics.${eof_range}.mon.gr
undef $undef_data
title EXP1
...
EOF
#
cat>$outdata/ca_hcst.$var.${sst_analysis}.$modemax.$icmon.${mseason}ics.${eof_range}.mon.ctl<<EOF
dset ^ca_hcst.$var.${sst_analysis}.$modemax.$icmon.${mseason}ics.${eof_range}.mon.gr
undef $undef_data
title EXP1
XDEF 360 LINEAR    0.5  1.0
YDEF 180 LINEAR  -89.5  1.0
