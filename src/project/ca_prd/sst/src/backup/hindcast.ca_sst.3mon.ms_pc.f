...
c
      dimension aaa2(icttm,ntt)
      dimension eval2(ntt),evec2(icttm,ntt),pc2(ntt,ntt)
      dimension eof2(modemax,modemax,mseason)
      dimension efld2d(modemax,nseason)
      dimension fld2d2(modemax,mseason)
      dimension WK2(ntt,icttm)
      dimension cosl3(mseason)
C
...
c
c* for EOF analysis appplied to efld3d
      it=0
      do is=1,nseason
      it=it+1
        id=0
        ...
        do j=1,mseason
          id=id+1
          aaa2(id,it)=efld3d(i,j,is)
        end do
        end do
      enddo
c       
      if(icttm.ge.ntt) then
      call eofs_4_ca(aaa2,icttm,ntt,ntt,eval2,evec2,pc2,wk2,0)
      else
      call eofs_4_ca(aaa2,icttm,ntt,icttm,eval2,evec2,pc2,wk2,0)
      endif
c
      write(6,*)'eval2='
      write(6,400) eval2
...
c
      ...
      END DO
        ...
        call inprod1(modemax,mseason,modemax,eof2,cosl3,m1,m1,anorm)
        write(6,*) 'anorm=',anorm
        do i=1,modemax
        do j=1,mseason
          eof2(m1,i,j)=eof2(m1,i,j)/sqrt(anorm)
        end do
        end do

        end do
c
C* project efld3d data onto EOFs*******
c
c for historical data
        do is=1,nseason
        call f3d_2_f2d(modemax,mseason,nseason,is,fld2d2,efld3d)
        do nm=1,modemax
        call inprod4(eof2,fld2d2,modemax,mseason,cosl3,nm,modemax,anorm)
          efld2d(nm,is)=anorm
        enddo
        enddo 
      write(6,*)'eof2='
