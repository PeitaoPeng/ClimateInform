...
c
      read(10,rec=1) fldin
        ...
        sstic(i,j)=fldin(i+lons-1,j+lats-1)  
        else
        sstic(i,j)=undef2
        endif
        enddo
...
c
      ...
      is=is+1
        ...
        np=np+1
          ...
          enddo
          enddo
c full field data for later use
          do j=1,jms
          do i=1,ims
...
c add sstic to aaa
        ...
        do j=1,jmp
        do i=1,imp
          if(abs(sstic(i,j)).lt.999) then
          id=id+1
          aaa(id,it)=sstic(i,j)*sqrt(cosl(j))
          endif
        end do
        end do
c
      write(6,*) 'length of input data for EOF calculation=',it
...
c for IC data
        do nmode=1,modemax
          call inprod4(eof,sstic,imp,jmp,cosl,nmode,modemax,anorm)
          efld1d(nmode)=anorm
        enddo
        do k=1,modemax
        write(6,*) 'mode=',k,'IC projection=',efld1d(k)
        enddo
c
c* realtime forecast
c
...
c
          do ip=1,npp
            ...
            do i=1,ims
            do j=1,jms
              sstana2(i,j,it)=fld4d2(i,j,ip+ldpen-1,is)
            enddo
            enddo
          enddo   
        enddo  
...
c
c amplitude adjusted for preal fcst
      write(6,*) 'amplitude adjustment for real prd'
c for sst
      DO ldpen=1,mldp
c
      do i=1,ims
