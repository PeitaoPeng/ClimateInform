...
mldp=`expr $mlead + 1` 
#
for modemax in  15 20 25 30; do
#for modemax in  20; do
...
c
      ...
      parameter(eoflats=-29,lats=31,late=77)  !lat range for EOFs analysis (29S-65N)
c     parameter(eoflats=-45,lats=31,late=68)  !lat range for EOFs analysis (29S-45N)
...
ln -s $outdata/eof_indp.sst.$icseason.gr fort.61
#
ln -s $outdata/ca_hcst.sst.skill.indp-eof.$modemax.$icseason.gr fort.85
ln -s $outdata/ca_hcst.sst.$modemax.$icseason.gr fort.86
#
...
EOF
#
cat>$outdata/eof_indp.sst.$icseason.ctl<<EOF
dset ^eof_indp.sst.$icseason.gr
undef -9.99E+33
title EXP1
xdef  180 linear   0. 2.
ydef   47 linear -29. 2.
...
EOF
#
cat>$outdata/ca_hcst.sst.skill.indp-eof.$modemax.$icseason.ctl<<EOF
dset ^ca_hcst.sst.skill.indp-eof.$modemax.$icseason.gr
undef -9.99E+33
title EXP1
xdef  180 linear   0. 2.
ydef   47 linear -29. 2.
...
EOF
#
cat>$outdata/ca_hcst.sst.indp-eof.$modemax.$icseason.ctl<<EOF
dset ^ca_hcst.sst.indp-eof.$modemax.$icseason.gr
undef -9.99E+33
title EXP1
xdef  180 linear   0. 2.
ydef   47 linear -29. 2.
