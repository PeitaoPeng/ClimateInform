...
datadir0=/export/cpc-lw-mlheureux/wd52ml/enso/oni.v4/data/ersst.v4
...
if [ $curmo = 02 ]; then cmon=2; icmoe=1 ; icmon_mid=dec; icssnmb=11; fi #icssnmb: 1st mon of icss
if [ $curmo = 03 ]; then cmon=3; icmoe=2 ; icmon_mid=jan; icssnmb=12 ; fi
if [ $curmo = 04 ]; then cmon=4; icmoe=3 ; icmon_mid=feb; icssnmb=1 ; fi
if [ $curmo = 05 ]; then cmon=5; icmoe=4 ; icmon_mid=mar; icssnmb=2; fi
if [ $curmo = 06 ]; then cmon=6; icmoe=5 ; icmon_mid=apr; icssnmb=3; fi
if [ $curmo = 07 ]; then cmon=7; icmoe=6 ; icmon_mid=may; icssnmb=4; fi
if [ $curmo = 08 ]; then cmon=8; icmoe=7 ; icmon_mid=jun; icssnmb=5; fi
if [ $curmo = 09 ]; then cmon=9; icmoe=8 ; icmon_mid=jul; icssnmb=6; fi
if [ $curmo = 10 ]; then cmon=10; icmoe=9 ; icmon_mid=aug; icssnmb=7; fi
...
#
outd=/cpc/home/wd52pp/data/season_fcst/ca/$curyr
outdata=${outd}/$curmo
if [ ! -d $outd ] ; then
  mkdir -p $outd
fi
...
cat >avg.gs<<EOF
'reinit'
'open $datadir0/ersst.v4.1854.pres.ctl'
'set x 1 180'
'set y 1  89'
'set t 1 12'
