...
c
      ...
      dimension obssst(imp,jmp),sstvfy4d(imp,jmp,nseason-32,mldp)
      dimension prdsst(imp,jmp),sstprd(imp,jmp,mldp)
      dimension sstprd4d(imp,jmp,nseason,mldp)
...
c
          do ip=1,npp
            ...
            do i=1,imp
            do j=1,jmp
              sstana(i,j,it)=fld4d(i,j,ip+ldpen-1,is)
            enddo
            enddo
...
cc have verification
      ...
      if(abs(fld4d(i,j,npp,nseason)).lt.999) then
      do is=33,nseason
        sstvfy4d(i,j,is-32,ldpen)=fld4d(i,j,ldpen,is)
      enddo
      else
      do is=33,nseason
        sstvfy4d(i,j,is-32,ldpen)=undef2
      enddo
      endif
      enddo
      enddo
...
c for sst
      write(6,*) 'for sst data'
       ...
       np=np+1
         do i=1,imp
         do j=1,jmp
           obssst(i,j)=sstvfy4d(i,j,is-32,ldpen)
...
c* for sst
      ...
      if(abs(fld4d(i,j,npp,nseason)).lt.999) then
        do is=1,nseason-32
c         ts1(is)=sstvfy4d(i,j,is,ldpen)
          ts1(is)=fld4d(i,j,ldpen,is+32)
...
c write out hindcast and obs
      ...
      do i=1,imp
      do j=1,jmp
        wk2d(i,j)=sstvfy4d(i,j,is-32,ldpen)
