...
mseason=4  # # of seasons together in CA
...
cp $lcdir/hindcast.ca_sst.3mon.ms.f $tmp/sst_hcst.f
...
ln -s $outdata/ca_hcst.${sst_analysis}.$icseason.${mseason}ics.${eof_range}.gr fort.86
#
...
EOF
#
cat>$outdata/ca_hcst.${sst_analysis}.$icseason.${mseason}ics.${eof_range}.ctl<<EOF
dset ^ca_hcst.${sst_analysis}.$icseason.${mseason}ics.${eof_range}.gr
undef -9.99E+33
title EXP1
xdef  180 linear   0. 2.
ydef   89 linear -88. 2.
