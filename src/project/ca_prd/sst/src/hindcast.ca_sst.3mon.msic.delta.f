...
c
      ...
      parameter(ntt2=(nseason-nskip)*npp)
c
      dimension ts1(ntt2),ts2(ntt2)
      dimension ts3(ntt2),ts4(ntt2)
      dimension ts5(ntt2),ts6(ntt2)
      dimension ts7(mldp),ts8(mldp)
      dimension ac1d(mldp,ntt2),rms1d(mldp,ntt2)
      dimension sstac1(ims,jms,mldp),sstrms1(ims,jms,mldp)
      dimension sstac2(ims,jms,mldp),sstrms2(ims,jms,mldp)
      dimension cov1(ims,jms,mldp),cov2(ims,jms,mldp)
      dimension cov3(ims,jms,mldp),cov4(ims,jms,mldp)
      dimension cov5(ims,jms,mldp),cov6(ims,jms,mldp)
      dimension cov7(ims,jms,mldp),cov8(ims,jms,mldp)
      dimension fldin(ims,jms),fldin2(ims,jms)
      dimension xlat(jmp),cosl(jmp)
      dimension xlat2(jms),cosl2(jms)
      dimension alpha(nhs),alpha2(nhs)
      ...
      dimension wk2d3(ims,jms),wk2d4(ims,jms),wk2d5(ims,jms)
      dimension wk2d6(ims,jms),wk2d7(ims,jms),wk2d8(ims,jms)
      dimension wk2d9(ims,jms),wk2d10(ims,jms),wk2d11(ims,jms)
      dimension wk2d12(ims,jms),wk2d13(ims,jms),wk2d14(ims,jms)
      dimension wk2d15(ims,jms),wk2d16(ims,jms),wk2d17(ims,jms)
      dimension allsst(ims,jms,12+26,nyear)
      dimension allsst2(ims,jms,12+26,nyear)
      dimension fld4d(imp,jmp,nps,nseason)
      dimension fldms(imp,jmp,mseason,nseason)
      dimension fld4d2(ims,jms,nps,nseason)
      dimension fld4d3(ims,jms,nps,nseason)
      dimension sstana(ims,jms,nhs+1)
      dimension tgtsst4d(ims,jms,npp,nseason)
      dimension aaa(ngrd,ntt)
      dimension bbb(ngrd,nhs)
...
C
      ...
      dimension stdodel(ims,jms,mldp)
      dimension stdfdel(ims,jms,mldp)
      dimension stdotot(ims,jms,mldp)
      dimension stdftot(ims,jms,mldp)
c
      dimension nts4(nseason*npp),nts5(nseason*npp)
C
      open(unit=11,form='unformatted',access='direct',recl=4*ims*jms) !input historical delta sst
      open(unit=12,form='unformatted',access='direct',recl=4*ims*jms) !input historical total sst
c
...
c
      open(unit=85,form='unformatted',access='direct',recl=4*ims*jms) !skill
      open(unit=86,form='unformatted',access='direct',recl=4*ims*jms) !obs&prd
c*************************************************
c
      PI=4.*atan(1.)
      CONV=PI/180.
...
c
        ...
        allsst2(i,j,iw+nwextb,iy)=fldin2(i,j)
        enddo
        enddo
c
      Enddo
...
c
        ...
        allsst2(i,j,iw,iy)=allsst2(i,j,12+iw,iy-1)
        enddo
        enddo
...
c
        ...
        allsst2(i,j,12+nwextb+iw,iy)=allsst2(i,j,nwextb+iw,iy+1)
        enddo
        enddo
c
        Enddo
...
c
        ...
        allsst2(i,j,12+nwextb+12+iw,iy)=allsst2(i,j,nwextb+iw,iy+2)
        enddo
        enddo
c
        Enddo
...
c have whole field for later CA use
          ...
          do i=1,ims
            ...
            fld4d3(i,j,np,is)=allsst2(i,j,nwextb+iw,iy)  
          enddo
          enddo
        enddo
...
c
 ...
 555    continue
        enddo             !loop for the non-target year
...
c put prd of each ldpen into 4d array for later analysis
      DO is=1,nseason
      do i=1,ims
      do j=1,jms
...
c
       DO ldpen=1,mldp
c* have std for delta obs and prd
      do i=1,ims
      do j=1,jms
      if(abs(fld4d2(i,j,npp,nseason)).lt.999) then
      it=0
      do is=nskip+1,nseason !from 1981 to 2013
        it=it+1
      ...
      sststdo(i,j,ldpen)=undef1
      sststdf(i,j,ldpen)=undef1
      endif
      enddo
      enddo
c
...
c       sstprd4d(i,j,is,ldpen)=fld4d2(i,j,ldpen,is)
      enddo
      else
        sstprd4d(i,j,is,ldpen)=undef1
      endif
      enddo
      enddo
c
...
c* have spatial corr
       ...
       np=np+1
         do i=1,ims
         do j=1,jms
           obssst(i,j)=fld4d2(i,j,ldpen,is)+fld4d3(i,j,ldpen,is)
           prdsst(i,j)=sstprd4d(i,j,is,ldpen)+fld4d3(i,j,ldpen,is)
         enddo
         enddo
c
...
c* for sst
      ...
      if(abs(fld4d2(i,j,npp,nseason)).lt.999) then
        do is=nskip+1,nseason
          ts1(is-nskip)=fld4d2(i,j,ldpen,is)
          ts2(is-nskip)=sstprd4d(i,j,is,ldpen)
          ts3(is-nskip)=fld4d3(i,j,ldpen,is)
          ts4(is-nskip)=fld4d2(i,j,ldpen,is)+fld4d3(i,j,ldpen,is)
          ts5(is-nskip)=sstprd4d(i,j,is,ldpen)+fld4d3(i,j,ldpen,is)
        enddo
        call cov_1d(ntt2,ts1,ts2,cov1(i,j,ldpen))
        call cov_1d(ntt2,ts4,ts5,cov2(i,j,ldpen))
        call cov_1d_shift(ntt2,ts4,ts5,cov3(i,j,ldpen))
        call cov_1d_shift(ntt2,ts5,ts4,cov4(i,j,ldpen))
        call corr_1d(ntt2,ts1,ts2,sstac1(i,j,ldpen))
        call corr_1d(ntt2,ts4,ts5,sstac2(i,j,ldpen))
        call rms_1d(ntt2,ts1,ts2,sstrms1(i,j,ldpen))
        call rms_1d(ntt2,ts4,ts5,sstrms2(i,j,ldpen))
        call std_1d(ntt2,ts1,stdodel(i,j,ldpen))
        call std_1d(ntt2,ts2,stdfdel(i,j,ldpen))
        call std_1d(ntt2,ts4,stdotot(i,j,ldpen))
        call std_1d(ntt2,ts5,stdftot(i,j,ldpen))
        call cov_1d(ntt2,ts1,ts3,cov5(i,j,ldpen))
        call cov_1d(ntt2,ts2,ts3,cov6(i,j,ldpen))
        call cov_1d(ntt2,ts3,ts3,cov7(i,j,ldpen))
      else
      cov1(i,j,ldpen)=undef1
      cov2(i,j,ldpen)=undef1
      cov3(i,j,ldpen)=undef1
      cov4(i,j,ldpen)=undef1
      cov5(i,j,ldpen)=undef1
      cov6(i,j,ldpen)=undef1
      cov7(i,j,ldpen)=undef1
      sstac1(i,j,ldpen)=undef1
      sstac2(i,j,ldpen)=undef1
      sstrms1(i,j,ldpen)=undef1
      sstrms2(i,j,ldpen)=undef1
      stdodel(i,j,ldpen)=undef1
      stdfdel(i,j,ldpen)=undef1
      stdotot(i,j,ldpen)=undef1
      stdftot(i,j,ldpen)=undef1
      endif
      enddo
      enddo
c
...
c
       ...
       do ldpen=1,mldp
       it=it+1
           ts7(it)=ac1d(ldpen,it)
           ts8(it)=rms1d(ldpen,it)
       end do
         iwo=iwo+1
c        write(71,rec=iwo) ts5
         iwo=iwo+1
c        write(71,rec=iwo) ts6
       end do
c
c write out skill
      iwr=0
      ...
      do i=1,ims
      do j=1,jms
        wk2d(i,j) =sstrms1(i,j,ldpen)
        wk2d2(i,j)=sstrms2(i,j,ldpen)
        wk2d3(i,j)=sstac1(i,j,ldpen)
        wk2d4(i,j)=sstac2(i,j,ldpen)
        wk2d5(i,j)=cov1(i,j,ldpen)
        wk2d6(i,j)=cov2(i,j,ldpen)
        wk2d7(i,j)=cov3(i,j,ldpen)
        wk2d8(i,j)=cov4(i,j,ldpen)
        wk2d9(i,j)=cov5(i,j,ldpen)
        wk2d10(i,j)=cov6(i,j,ldpen)
        wk2d11(i,j)=cov7(i,j,ldpen)
        wk2d12(i,j)=stdodel(i,j,ldpen)
        wk2d13(i,j)=stdfdel(i,j,ldpen)
        wk2d14(i,j)=stdotot(i,j,ldpen)
        wk2d15(i,j)=stdftot(i,j,ldpen)
      enddo
      enddo
      iwr=iwr+1
      write(85,rec=iwr) wk2d
      iwr=iwr+1
      write(85,rec=iwr) wk2d2
      iwr=iwr+1
      write(85,rec=iwr) wk2d3
      iwr=iwr+1
      write(85,rec=iwr) wk2d4
      iwr=iwr+1
      write(85,rec=iwr) wk2d5
      iwr=iwr+1
      write(85,rec=iwr) wk2d6
      iwr=iwr+1
      write(85,rec=iwr) wk2d7
      iwr=iwr+1
      write(85,rec=iwr) wk2d8
      iwr=iwr+1
      write(85,rec=iwr) wk2d9
      iwr=iwr+1
      write(85,rec=iwr) wk2d10
      iwr=iwr+1
      write(85,rec=iwr) wk2d11
      iwr=iwr+1
      write(85,rec=iwr) wk2d12
      iwr=iwr+1
      write(85,rec=iwr) wk2d13
      iwr=iwr+1
      write(85,rec=iwr) wk2d14
      iwr=iwr+1
      write(85,rec=iwr) wk2d15

      ENDDO  !loop for ldpen
c
c write out hindcast and obs
      iwr=0
      DO is=nskip+1,nseason
      DO ldpen=1,mldp
      ...
      do i=1,ims
      do j=1,jms
      if(abs(fld4d2(i,j,ldpen,is)).lt.999) then
        wk2d(i,j)=fld4d2(i,j,ldpen,is)+fld4d3(i,j,ldpen,is)
        wk2d2(i,j)=sstprd4d(i,j,is,ldpen)+fld4d3(i,j,ldpen,is)
c       wk2d(i,j)=fld4d2(i,j,ldpen,is)
c       wk2d2(i,j)=sstprd4d(i,j,is,ldpen)
      else
        wk2d(i,j)=undef1
        wk2d2(i,j)=undef1
      endif
      ...
      iwr=iwr+1
      write(86,rec=iwr) wk2d
      iwr=iwr+1
      write(86,rec=iwr) wk2d2
...
c anomalize first
      ...
      av2=0
      do i=1,n
c       av1=av1+ts1(i)/float(n)
c       av2=av2+ts2(i)/float(n)
      enddo
      do i=1,n
c       ts1(i)=ts1(i)-av1
c       ts2(i)=ts2(i)-av2
...
c
      subroutine cov_1d(n,ts1,ts2,cov)
      dimension ts1(n),ts2(n)
      xy=0.
      do it=1,n
      xy=xy+ts1(it)*ts2(it)
      ...
      subroutine cov_1d_shift(n,ts1,ts2,cov)
      dimension ts1(n),ts2(n)
      xy=0.
      do it=1,n-1
      xy=xy+ts1(it)*ts2(it+1)
      enddo
      cov=xy/float(n-1)
      return
      end
