...
cd $tmp
#
for esmcat in all ersst hadoisst 1sic 2sic 3sic 4sic; do
for var in sst; do
...
mld=17  # lead 0-13
#
cp $lcdir/ca_sst_skill.esm.f $tmp/ca_skill.f
#
cat > parm.h << eof
...
/bin/rm $tmp/fort.*
#
ln -s $datadir/cahcst.$var.${icss}_ic.${esmcat}_esm.gr fort.11
#
ln -s $outdata/ac_t.cahcst.$var.$icss.${esmcat}_esm.gr fort.85
#
skill.x 
#
cat>$outdata/ac_t.cahcst.$var.$icss.${esmcat}_esm.ctl<<EOF
dset ^ac_t.cahcst.$var.$icss.${esmcat}_esm.gr
undef -9.99E+8
title EXP1
xdef  360 linear   0.  1.
