...
c
      parameter(ntt=nseason*npp,nhs=(nseason-1)*npp)
...
c
      ...
      dimension sstac(ims,jms),sstrms(ims,jms)
      ...
      dimension allsst(ims,jms,12+16,nyear)
...
c
      dimension hur0(4,nyear),hur(nyear)
      dimension hurana(nhs+1),tgtyhrcn(nseason)
      dimension hurprd(nseason)
C
      open(unit=11,form='unformatted',access='direct',recl=4*ims*jms) !input historical sst
      open(unit=12,form='formatted') !input historical tpz
c
      open(unit=85,form='unformatted',access='direct',recl=4*ims*jms) !skill
      open(unit=86,form='unformatted',access='direct',recl=4) !obs&prd
c*************************************************
c
      PI=4.*atan(1.)
      CONV=PI/180.
...
c
      ...
      write(6,*) 'hur=',hur
c
c have hur anom wrt two clim
      hclm1=0
      do it=1,47
        hclm1=hclm1+hur(it)/47.
      enddo
      do it=1,47
c       hur(it)=hur(it)-hclm1
      enddo
c
      hclm2=0
      do it=48,nyear
        hclm2=hclm2+hur(it)/float(nyear-47)
      enddo
      do it=48,nyear
c       hur(it)=hur(it)-hclm2
      enddo
      write(6,*) 'hclm1&hclm2=',hclm1,hclm2
      call anom(hur,nyear,avg)
c
...
c       ntgtsm=ntgts-1
c       ntgtsp=ntgts+1
c       if(ntgts.eq.1) ntgtsm=3
c       if(ntgts.eq.nseason) ntgtsp=nseason-2
...
c
c       if(is.eq.ntgtsm) go to 555
        if(is.eq.ntgts)  go to 555
c       if(is.eq.ntgtsp) go to 555
c
            it=it+1
...
c for hurrican
      do ntime=1,nhs
      tgtyhrcn(ntgts)=tgtyhrcn(ntgts)+
     &           hurana(ntime)*alpha3d(ntime,1,ntgts)
      enddo
c
...
c put prd of each ldpen into 4d array for later analysis
      ...
      do i=1,ims
      do j=1,jms
        sstprd4d(i,j,is,1)=tgtsst4d(i,j,1,is)
      enddo
      enddo
        hurprd(is)=tgtyhrcn(is)
      ENDDO
c
c* processing historical forecast
c     write(6,*) 'processing historical forecast'
c
c* have std for obs and prd
c
c for hrcn
      it=0
      do is=33,nseason !from 1981 to 2014
        it=it+1
        ts1(it)=hur(is)
        ts2(it)=hurprd(is)
      enddo
      call have_std(ts1,ntt2,hstdo)
      call have_std(ts2,ntt2,hstdf)
c
      do is=1,46
      hurprd(is)=hurprd(is)
      enddo
      do is=47,nseason
      hurprd(is)=hurprd(is)+hclm2-hclm1
      enddo
c
c
c* skill calculation
...
c
      if(abs(fld4d2(i,j,npp,nseason)).lt.999) then
        do is=1,nseason-nskip
          ts1(is)=fld4d2(i,j,1,is+nskip)
          ts2(is)=sstprd4d(i,j,is+nskip,1)
        enddo
        call corr_1d(ntt2,ts1,ts2,sstac(i,j))
        call rms_1d(ntt2,ts1,ts2,sstrms(i,j))
      else
      sstac(i,j)=undef1
      sstrms(i,j)=undef1
      endif
c
      enddo
      enddo
c write out 2-d skill
      iwr=0
      do i=1,ims
      do j=1,jms
        wk2d(i,j)=sstac(i,j)
        wk2d2(i,j)=sstrms(i,j)
      enddo
      enddo
      iwr=iwr+1
...
c hrcn ac/rms skill
        do is=1,nseason-nskip
          ts1(is)=hur(is+nskip)
          ts2(is)=hurprd(is+nskip)
        enddo
        call corr_1d(ntt2,ts1,ts2,hurac)
        call rms_1d(ntt2,ts1,ts2,hurrms)
      write(6,*) 'hurac&rms=',hurac, hurrms
c
c
c write out hindcast and obs
      ...
      iwr=iwr+1
      write(86,rec=iwr) hur(is)
      iwr=iwr+1
      write(86,rec=iwr) hurprd(is)
