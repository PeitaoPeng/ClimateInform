...
c range for sst pat cor (ac_1d)
      ...
      parameter(inas=462,inae=600)   ! 230-300 for CONUS
      parameter(jnas=230,jnae=280)   ! 25N->75N
...
c
      ...
      dimension fldin(ims,jms),fldin2(imx,jmx)
...
c
      dimension alltpz(imx,jmx,12+26,nyear)
      dimension tpz4d(imx,jmx,nps,nseason)
      dimension tpz4d2(imx,jmx,nps,nseason)
      dimension tpzana(imx,jmx,nhs+1)
      dimension tpzana2(imx,jmx,ntt)
      dimension tpzprd(imx,jmx,mldp),tpz2d(imx,jmx)
      dimension tpzprd4d(imx,jmx,nseason,mldp)
      dimension tgttpz4d(imx,jmx,1,nseason)
      dimension tpzac(imx,jmx,mldp),tpzrms(imx,jmx,mldp)
      dimension tpzstdf(imx,jmx,mldp)
      dimension tpzstdo(imx,jmx,mldp)
      dimension obstpz(imx,jmx)
      dimension prdtpz(imx,jmx)
      dimension wk2d(imx,jmx),wk2d2(imx,jmx)
      dimension wk2d3(imx,jmx),wk2d4(imx,jmx)
      dimension wk2d5(imx,jmx),wk2d6(imx,jmx)
C
...
c
      open(unit=12,form='unformatted',access='direct',recl=4*imx*jmx) !input pre/temp/hgt
c
...
c
      open(unit=85,form='unformatted',access='direct',recl=4*imx*jmx) !real forecast
c*************************************************
c
      PI=4.*atan(1.)
      CONV=PI/180.
...
c
      ...
      enddo
c
cc read in all historical data
c
      iu1=11
      iu2=12
c
...
c
        do i=1,imx
        do j=1,jmx
           if(abs(tpz2d(i,j)).lt.999) then
           tpzprd(i,j,ldpen)=tpz2d(i,j)
           else
           tpzprd(i,j,ldpen)=undef1
           endif
        enddo
        enddo
c
