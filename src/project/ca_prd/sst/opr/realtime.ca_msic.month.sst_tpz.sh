...
#
cp $lcdir/realtime.ca_msic.month.sst_tpz.f $tmp/sst_prd.f
...
c
      ...
      parameter(ngrd=$ngrd)
      parameter(msic=$msic)
      parameter(nskip=12) ! skip from jfm1948 to jfm1949
eof
#
...
#
ln -s $outdata/real_ca_prd.$var.${sst_analysis}.${eof_range}.$modemax.${msic}ics.mon.gr fort.85
#
...
EOF
#
cat>$outdata/real_ca_prd.$var.${sst_analysis}.${eof_range}.$modemax.${msic}ics.mon.ctl<<EOF
dset ^real_ca_prd.$var.${sst_analysis}.${eof_range}.$modemax.${msic}ics.mon.gr
undef $undef_data
title EXP1
XDEF 360 LINEAR    0.5  1.0
YDEF 180 LINEAR  -89.5  1.0
