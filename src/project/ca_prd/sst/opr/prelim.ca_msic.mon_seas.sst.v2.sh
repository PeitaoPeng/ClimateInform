...
#
if [ $curmo = 01 ]; then cmon=1; icmon_end=jan; icmoe=1; icmon_mid=dec; icssnmb=11; fi #icmon_mid: mid mon of icss
if [ $curmo = 02 ]; then cmon=2; icmon_end=feb; icmoe=2 ; icmon_mid=jan; icssnmb=12; fi #icssnmb: 1st mon of icss; considered curr mon added
if [ $curmo = 03 ]; then cmon=3; icmon_end=mar; icmoe=3 ; icmon_mid=feb; icssnmb=1 ; fi
if [ $curmo = 04 ]; then cmon=4; icmon_end=apr; icmoe=4 ; icmon_mid=mar; icssnmb=2 ; fi
if [ $curmo = 05 ]; then cmon=5; icmon_end=may; icmoe=5 ; icmon_mid=apr; icssnmb=3; fi
if [ $curmo = 06 ]; then cmon=6; icmon_end=jun; icmoe=6 ; icmon_mid=may; icssnmb=4; fi
if [ $curmo = 07 ]; then cmon=7; icmon_end=jul; icmoe=7 ; icmon_mid=jun; icssnmb=5; fi
if [ $curmo = 08 ]; then cmon=8; icmon_end=aug; icmoe=8 ; icmon_mid=jul; icssnmb=6; fi
if [ $curmo = 09 ]; then cmon=9; icmon_end=sep; icmoe=9 ; icmon_mid=aug; icssnmb=7; fi
if [ $curmo = 10 ]; then cmon=10; icmon_end=oct; icmoe=10; icmon_mid=sep; icssnmb=8; fi
if [ $curmo = 11 ]; then cmon=11; icmon_end=nov; icmoe=11; icmon_mid=oct; icssnmb=9; fi
if [ $curmo = 12 ]; then cmon=12; icmon_end=dec; icmoe=12; icmon_mid=nov; icssnmb=10; fi
#
yyyy=$curyr
yyym=`expr $curyr - 1`
#
...
/usr/local/bin/grads -bl <readsst
...
EOF
#=======================================
# have historical monthly anomalies
#=======================================
...
#
cat sstanom.gr sst.curmon.anom.gr > $outfile2.gr
#
...
#
cp $lcdir/prelim.ca_msic.mon_seas.sst.v2.f $tmp/sst_prd.f
...
ln -s $datadir/${sst_analysis}.mon.1948-curr.gr fort.12
#
...
ln -s $outdata/prelim_ca_prd.sst.${eof_range}.$modemax.${msic}ics.mon.gr fort.87
...
EOF
#
cat>$outdata/prelim_ca_prd.sst.${eof_range}.$modemax.${msic}ics.mon.ctl<<EOF
dset ^prelim_ca_prd.sst.${eof_range}.$modemax.${msic}ics.mon.gr
undef -9.99E+33
title EXP1
XDEF 360 LINEAR    0.5  1.0
YDEF 180 LINEAR  -89.5  1.0
