...
datadir0=/export/cpc-lw7-mlheureux/wd52ml/enso/oni.v5/data/ersst.v5
...
#
yrn2=`expr $curyr - 1949` # total full years,=67 for 1949-2015
if [ $cmon = 1 ]; then yrn2=`expr $yyym - 1949`; fi
mmn2=`expr $yrn2 \* 12`
#
tmax=`expr $mmn2 + $icmoe` # 816=dec2015
#
nyear=`expr $curyr - 1949`  # total full year data used for CA, 67 for 1949-2015
if [ $cmon -le 2 ]; then nyear=`expr $curyr - 1949 - 1`; fi # for having 12 3-mon avg data for past year
#
icyr=$curyr
...
#
outfile=${sst_analysis}.3mon.1949-curr
cat >sst3monavg<<EOF
run avg.gs
EOF
cat >avg.gs<<EOF
'reinit'
'open $datadir0/ersst.v5.1854.pres.ctl'
'set x 1 180'
'set y 1  89'
'set t 1 12'
...
EOF
#
/usr/local/bin/grads -bl <sst3monavg
#
...
EOF
#
outfile3=${sst_analysis}.mon.1949-curr
cat >sstmon<<EOF
run sst_pick.gs
EOF
cat >sst_pick.gs<<EOF
...
'modify clm seasonal'
'set gxout fwrite'
'set fwrite $datadir/$outfile3.gr'
'set t $nts $nte'
'd sst-clm'
'c'
EOF
#
/usr/local/bin/grads -bl <sstmon
#
...
fEOF
#
cat >regrid2.gs<<gsEOF
'reinit'
'open $datadir/$outfile3.ctl'
'open /cpc/home/wd52pp/data/obs/sst/grid.1x1.ctl'
'set gxout fwrite'
'set fwrite $datadir/$outfile3.1x1.gr'
nt=1
while ( nt <= $ntend)
...
/usr/local/bin/grads -pb <int2

cat>$datadir/$outfile3.1x1.ctl<<EOF
dset ^$outfile3.1x1.gr
undef -9.99E+8
*
options little_endian
*
...
#
cp $lcdir/realtime.ca_msic.month.sst.f $tmp/sst_prd.f
...
ln -s $datadir/${sst_analysis}.3mon.1949-curr.1x1.gr fort.11
ln -s $datadir/$outfile3.1x1.gr fort.12
#
ln -s $outdata/eof.${sst_analysis}.${eof_range}.${msic}ics.3mon.gr fort.70
#
ln -s $outdata/real_ca_prd.sst.${sst_analysis}.${eof_range}.$modemax.${msic}ics.mon.gr fort.85
ln -s $outdata/real_ca_weights.${sst_analysis}.${eof_range}.$modemax.${msic}ics.mon.gr fort.86
#
./ca.x > $outdata/realtime.${sst_analysis}.${eof_range}.$modemax.${msic}ics.mon.out
#ca.x 
#
...
EOF
#
cat>$outdata/real_ca_prd.sst.${sst_analysis}.${eof_range}.$modemax.${msic}ics.mon.ctl<<EOF
dset ^real_ca_prd.sst.${sst_analysis}.${eof_range}.$modemax.${msic}ics.mon.gr
undef -9.99E+8
title EXP1
XDEF 360 LINEAR    0.5  1.0
YDEF 180 LINEAR  -89.5  1.0
...
vars  8
sst   1 99 mon_sst prd
sdo   1 99 std of obs mon_sst
sdf   1 99 std of prd mon_sst
ac    1 99 ac skill since 1981
rms   1 99 rms skill since 1981
ssst  1 99 3mon_avg_sst prd
sac   1 99 ac skill of 3mon_sst
srms  1 99 rms skill of 3mon_sst
endvars
EOF
#
#
cat>$outdata/real_ca_weights.${sst_analysis}.${eof_range}.$modemax.${msic}ics.mon.ctl<<EOF
dset ^real_ca_weights.${sst_analysis}.${eof_range}.$modemax.${msic}ics.mon.gr
undef -9.99E+8
title EXP1
