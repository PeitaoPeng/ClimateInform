...
datadir=/cpc/home/wd52pp/data/season_fcst/ca/$icyr/$icmoe
#
cd $tmp
#
...
#
ln -s $datadir/real_ca_prd.sst.${sst_analysis}.${eof_range}.$modemax.${msic}ics.mon.gr fort.$ifile
#
ifile=`expr $ifile + 1` 
#
done  # maxmode
...
#
cat>$datadir/ca.ersst.nino34.mon.ctl<<EOF
dset ^ca.ersst.nino34.mon.gr
undef -9.99E+33
title EXP1
XDEF  1 LINEAR    0  1.0
...
#
EOF
cat>$datadir/ca.hadoisst.nino34.mon.ctl<<EOF
dset ^ca.hadoisst.nino34.mon.gr
undef -9.99E+33
title EXP1
XDEF  1 LINEAR    0  1.0
YDEF  1 LINEAR  -89.5  1.0
...
#
EOF
cat>$datadir/ersst.clim.mon.ctl<<EOF
dset ^ersst.clim.gr
undef -9.99E+08
title EXP1
...
#
EOF
cat>$datadir/hadoisst.clim.mon.ctl<<EOF
