...
outfile=$var.1949_cur.3mon.0.5x0.5
imx=720
jmx=360
#
#=======================================
...
#
cp $lcdir/realtime.ca_msic.season.sst_p.0.5x0.5.f $tmp/sst_prd.f
...
#
ln -s $outdata/real_ca_prd.$var.${sst_analysis}.${eof_range}.$modemax.${msic}ics.3mon.0.5x0.5.gr fort.85
#
...
EOF
#
cat>$outdata/real_ca_prd.$var.${sst_analysis}.${eof_range}.$modemax.${msic}ics.3mon.0.5x0.5.ctl<<EOF
dset ^real_ca_prd.$var.${sst_analysis}.${eof_range}.$modemax.${msic}ics.3mon.0.5x0.5.gr
undef $undef_data
title EXP1
XDEF $imx LINEAR     0.25  0.5
YDEF $jmx LINEAR   -89.75  0.5
