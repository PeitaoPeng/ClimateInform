...
c
      parameter(ntt=nseasonp,nhs=nseason-3)
...
c
      ...
      dimension prdsst(ims,jms),prdsst2(ims,jms)
      dimension sstprd(ims,jms,mldp),sstprd2(ims,jms,mldp)
      ...
      dimension fld4d(imp,jmp,nps,nseasonp)
      dimension fldms(imp,jmp,msic,nseasonp)
      dimension fld4d2(ims,jms,nps,nseasonp)
      dimension fld4d3(ims,jms,nps,nseasonp)
      dimension sstana(ims,jms,nhs+1)
      dimension sstana2(ims,jms,ntt),sstana3(ims,jms,ntt)
      dimension tgt2d(ims,jms),tgt2d2(ims,jms)
      dimension tgtsst4d(ims,jms,1,nseason)
      dimension sstic(imp,jmp,msic)
      dimension aaa(ngrd,ntt+1)
      dimension efld2d(modemax,msic)
      dimension efld3d(modemax,msic,nseasonp)
...
c
      dimension xn34(msic,nseasonp),xn34ic(msic)
C
      ...
      open(unit=12,form='unformatted',access='direct',recl=4*ims*jms) !input historical sst
c
      open(unit=70,form='unformatted',access='direct',recl=4*imp*jmp) !EOFs for proj use
c
      open(unit=85,form='unformatted',access='direct',recl=4*ims*jms) !real 3-mon forecast
      open(unit=86,form='unformatted',access='direct',recl=4*nseasonp) !weights
      open(unit=87,form='unformatted',access='direct',recl=4*ims*jms) !real monhtly forecast
c*************************************************
c
      PI=4.*atan(1.)
      CONV=PI/180.
...
c
      ...
      it=it+1
        ...
        read(iu2,rec=it+2) fldin2 !monthly data
c
        do j=1,jms
        do i=1,ims
        allsst(i,j,iw+nwextb,iy)=fldin(i,j)
        allsst2(i,j,iw+nwextb,iy)=fldin2(i,j) !monthly
        enddo
        enddo
c
      Enddo
...
c
      enddo
      call nino34(fldms,xn34,imp,jmp,msic,nseasonp,undef2)
...
c for historical data
        do is=1,nseasonp
        call f4d_2_f2d(imp,jmp,msic,nseasonp,ic,is,fld2d,fldms)
        do nmode=1,modemax-1
          call inprod4(eof,fld2d,imp,jmp,cosl,nmode,modemax,anorm)
          efld3d(nmode,ic,is)=anorm
        enddo
          efld3d(modemax,ic,is)=40*xn34(ic,is) ! n34 as last mode
        enddo 
c for IC data
        do i=1,imp
        do j=1,jmp
...
c
      DO ldpen=1,mldp  !1:IC; 2:0 mon1; 3: mon2; 4: mon3; 5: mon4; 6: mon5 ...
        it=0
        do is=1,nseasonp      !loop for the non-target winter
c
            it=it+1
            ...
            do j=1,jms
              ...
              sstana3(i,j,it)=fld4d3(i,j,ldpen,is)
            enddo
            enddo
        enddo  
c
...
c for sst
     ...
     &           sstana2(i,j,ntime)*alpha2(ntime)
        tgt2d2(i,j)=tgt2d2(i,j)+
     &           sstana3(i,j,ntime)*alpha2(ntime)
        enddo
        else
        tgt2d(i,j)=undef2
        tgt2d2(i,j)=undef2
...
c
      ...
      do i=1,ims
      do j=1,jms
        prdsst(i,j)=sstprd2(i,j,ldpen)
      enddo
      enddo
      iwr2=iwr2+1
      write(87,rec=iwr2) prdsst
      ENDDO  !loop for ldpen
c
C
C*********************************************
