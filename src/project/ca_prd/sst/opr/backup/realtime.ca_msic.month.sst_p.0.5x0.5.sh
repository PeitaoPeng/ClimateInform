...
outfile=$var.1949_cur.mon.0.5x0.5
imx=720
jmx=360
#
#=======================================
...
dset $outfile.gr
undef $undef_data
ydef $jmx linear -89.75 0.5
xdef $imx linear 0.25 0.5
...
nseason=`expr $nyear - 1` #for eof and ca analysis
#
...
#
cp $lcdir/realtime.ca_msic.month.sst_p.0.5x0.5.f $tmp/sst_prd.f
...
c
      ...
      parameter(imx=$imx,jmx=$jmx)   !input prec dimension
c
...
ln -s $datadir/$outfile.gr fort.12
#
ln -s $outdata/eof.${sst_analysis}.${eof_range}.${msic}ics.3mon.gr fort.70
#
ln -s $outdata/real_ca_prd.$var.${sst_analysis}.${eof_range}.$modemax.${msic}ics.mon.0.5x0.5.gr fort.85
#
#ca.x > $outdata/realtime.$var.${sst_analysis}.${eof_range}.$modemax.${msic}ics.mon.out
./ca.x 
#
...
EOF
#
cat>$outdata/real_ca_prd.$var.${sst_analysis}.${eof_range}.$modemax.${msic}ics.mon.0.5x0.5.ctl<<EOF
dset ^real_ca_prd.$var.${sst_analysis}.${eof_range}.$modemax.${msic}ics.mon.0.5x0.5.gr
undef $undef_data
title EXP1
XDEF $imx LINEAR     0.25  0.5
YDEF $jmx LINEAR   -89.75  0.5
