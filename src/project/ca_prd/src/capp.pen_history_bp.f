...
C historical data used for CA are filtered
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
      parameter(natura=1)! natura=1 means natural analogue
      ...
      parameter(imp=72,jmp=26)   ! grid size of prcp
...
c     parameter(ims=9,ime=60)   ! grid to have 1d skill
      ...
      dimension fld2d(imp,jmp),fld2d2(imp,jmp)
      dimension fld4d(imp,jmp,nps,nws),fld4d2(imp,jmp,nps,nws)
      dimension tgt4d(imp,jmp,npp,nws)
      dimension clim(imp,jmp),aaa(ngrd,ntt)
      dimension efld3d(modemax,nps,nws),efld3d2(modemax,nps,nws)
...
C
      ...
      open(unit=11,form='unformatted',access='direct',recl=72*72) !pent data
      ...
      open(unit=25,form='unformatted',access='direct',recl=72*jmp) !pent data
      open(unit=26,form='unformatted',access='direct',recl=72*jmp) !pent data
      open(unit=27,form='unformatted',access='direct',recl=72*jmp) !pent data
...
c*
      do j=1,jmp
c     xlat(j)=-28.75+(j-1)*2.5
      xlat(j)=-31.25+(j-1)*2.5
      enddo
c
      PI=4.*atan(1.)
      CONV=PI/180.
...
c* read in data and save the tropics
      do it=1,18   ! skip Jan-Mar of 1979
        read(10,rec=it) fldin
        read(11,rec=it) fldin
      enddo
      DO iw=1,nws              !ws -> winter seasons

        nskips=(iw-1)*73+19    !first of Apr
        nskipe=(iw-1)*73+67    !Last  of Nov
        do it=nskips,nskipe    ! skip data of Apr-Nov
          read(10,rec=it) fldin
          read(11,rec=it) fldin
        enddo

        nkeeps=(iw-1)*73+68    !first of Apr
        nkeepe=iw*73+18        !Last  of Nov
        np=1
        do it=nkeeps,nkeepe    !read DJFM data
          read(10,rec=it) fldin
          read(11,rec=it) fldin2
            do i=1,imp
            do j=1,jmp
            fld4d(i,j,np,iw)=fldin(i,j+24)
            fld4d2(i,j,np,iw)=fldin2(i,j+24)
...
C* project grid data onto eof's*******
        ...
        call f4d_2_f2d(imp,jmp,nps,nws,ip,iw,fld2d2,fld4d2)
        do nmode=1,modemax
          call inprod4(e,fld2d,imp,jmp,cosl,nmode,modemax,anorm)
          efld3d(nmode,ip,iw)=anorm
          call inprod4(e,fld2d2,imp,jmp,cosl,nmode,modemax,anorm)
          efld3d2(nmode,ip,iw)=anorm
        enddo
        enddo
        enddo 
...
c
      DO ldpen=1,7  !1:IC; 2:0 pent lead; 3: 1pent lead; ...
 ...
 555    continue
        ...
        do nmode=1,modemax
c       ana(nmode,nhs+1)=efld3d2(nmode,ntgtp,ntgtw)
...
c
      ...
      DO ip=1,npp  
        ...
        do j=1,jmp
        do nmode=1,modemax
          tgt4d(i,j,ip,iw)=tgt4d(i,j,ip,iw)+
     &          etgt3d(nmode,ip,iw)*e(nmode,i,j)
        enddo
        enddo
        enddo
...
c* pattern ac
       ...
       DO ip=1,npp
         do i=1,imp
         do j=1,jmp
           obs(i,j)=fld4d2(i,j,ip+ldpen-1,iw)
           obs2(i,j)=fld4d2(i,j,ip,iw) !as persistency prd
...
c* temporal ac
      ...
      do j=1,jmp
        ...
        do iw=1,nws
        do ip=1,npp
          ts1(it)=fld4d2(i,j,ip+ldpen-1,iw)
          ts2(it)=tgt4d(i,j,ip,iw)
          ts3(it)=fld4d2(i,j,ip,iw)
        it=it+1
        enddo
        enddo
