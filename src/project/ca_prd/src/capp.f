...
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
      ...
      parameter(ims=20,ime=60)   ! grid to have skill
c     parameter(ims=1,ime=imp)   ! grid to have skill
      parameter(ntt=24,nhs=ntt-3,modemax=8)!choice # of modes and ntt
      parameter(ngrd=imp*jmp)
      dimension xlat(jmp),fldp(imp,jmp)
      dimension t3(imp,jmp),t3c(imp,jmp)
      ...
      dimension obs3d(imp,jmp,ntt-2),obs3d2(imp,jmp,ntt-2)
      dimension prd3d(imp,jmp,ntt-2),ca3d(imp,jmp,ntt-2)
      dimension prd3d2(imp,jmp,ntt-2)
      dimension fld2d(imp,jmp)
...
C     open(unit=20,form='unformatted',access='direct',recl=imz*jmz) !z500
      ...
      open(unit=45,form='unformatted',access='direct',recl=imp*jmp) !ca_prd
      open(unit=46,form='unformatted',access='direct',recl=imp*jmp) !ca_sst_prd
...
c
      ...
      do ntgt=2,ntt-1  !loop over target years
        ...
        call f2d_2_f3d(imp,jmp,ntt-2,no,obs,obs3d)
c* get data for the non-target years
        ...
        call f2d_2_f3d(imp,jmp,nhs,ny,fld2d,fld3d)
 555    continue
        enddo
        write(6,*) 'non-target years#=',ny
        call anom_clim(imp,jmp,nhs,ny,fld3d,fld2d)
...
c* input data for EOF analysis
        do it=1,ny
          ...
          do i=1,imp
            id=id+1
            aaa(id,it)=fld3d(i,j,it)*sqrt(COSL(j))
          end do
          end do
        enddo
c       
...
C* project "historical data" onto current eof's*******
        do nt=1,ny
        call f3d_2_f2d(imp,jmp,ny,nt,fld2d,fld3d)
        do nmode=1,modemax
...
c
        call f3d_2_f2d(imp,jmp,ntt-2,no,fld2d,obs3d)
        do nmode=1,modemax
          call inprod4(e,fld2d,imp,jmp,cosl,nmode,modemax,anorm)
...
C
       ...
       enddo
       enddo
       call f3d_2_f2d(imp,jmp,ntt-2,no,obs,obs3d) !for persistent prd
c      call rmsd1(obs,t3c,imp,jmp,ims,ime,cosl,cr1d(no))
c
       write(40) t3c    ! write IC-error for grads1
       call f2d_2_f3d(imp,jmp,ntt-2,no,t3c,ca3d)
c* construct prediction
       write(6,*)'start ca_prd_spc'
       call ca_prd_spc(15,ntt,ntgt,nhs,imp,jmp,alpha,t3,prd
     &                 ,cosl,ngrd,modemax)
c      call ca_prd_grd(15,ntt,ntgt,nhs,imp,jmp,alpha,t3,prd)
       call f2d_2_f3d(imp,jmp,ntt-2,no,prd,prd3d)
       call f2d_2_f3d(imp,jmp,ntt-2,no,t3,obs3d2)
       call rmsd1(t3,prd,imp,jmp,ims,ime,cosl,cr1d2(no))
       call rmsd1(t3,obs,imp,jmp,ims,ime,cosl,cr1d3(no)) !persistent prd
       write(45) prd
...
c
      ...
      write(50) corr   !ca prd
      call corr_2d(imp,jmp,ntt-2,obs3d,ca3d,corr)
      write(50) corr   !ca for IC month
      call corr_2d(imp,jmp,ntt-2,obs3d2,obs3d,corr)  !persistent prd
      write(50) corr   !persistent prd
c* read in the SST based ca prd
       do it=1,ntt-2
        read(46,rec=it) fld2d
        call f2d_2_f3d(imp,jmp,ntt-2,it,fld2d,prd3d2)
       enddo
c* mixing the SST based prd with OPI based prd
       do it=1,ntt-2
       do i=1,imp
       do j=1,jmp
         prd3d2(i,j,it)=(prd3d2(i,j,it)+prd3d(i,j,it)+
     &                   obs3d(i,j,it))/3.
       enddo
       enddo
       enddo
      call corr_2d(imp,jmp,ntt-2,obs3d2,prd3d2,corr)  !ensemble prd
      write(50) corr   !2-prd ensemble
c
...
c*
      subroutine ca_prd_spc(iu,ntt,ntgt,nhs,im,jm,alpha,obs,prd
     &                      ,cosl,ngrd,modemax)
      dimension aaa(ngrd,nhs),WK(nhs,ngrd)
      dimension ana(modemax,nhs+1),anac(modemax)
      dimension EVAL(nhs),EVEC(ngrd,nhs),PC(nhs,nhs)
      dimension e(modemax,im,jm),cosl(jm)
 ...
 666      continue
      enddo
        call anom_clim(im,jm,nhs,nhs,fld3d,fld2d)
        read(iu,rec=ntgt) obs
        do i=1,im
        do j=1,jm
...
C* standardize spatial patterns to unity****
        do m1=1,modemax
        call inprod1(im,jm,modemax,e,cosl,m1,m1,anorm)
c       write(6,*)m1,anorm
        do i=1,im
        do j=1,jm
          e(m1,i,j)=e(m1,i,j)/sqrt(anorm)
...
C* project "historical data" onto current eof's*******
        ...
        do nmode=1,modemax
          call inprod4(e,fld2d,im,jm,cosl,nmode,modemax,anorm)
          ana(nmode,nt)=anorm
        enddo
c       write(6,100)nj,(ana(nmode,nj),nmode=1,10)
        enddo
