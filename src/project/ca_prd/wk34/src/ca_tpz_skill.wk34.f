...
c range for sst pat cor (ac_1d)
      parameter(imss=93,imse=121)   ! 230-300 for CONUS
      parameter(jmss=47,jmse=57)    ! 25N-50N
      parameter(nt=nseason*13)   
c
      dimension ts1(nt),ts2(nt)
      dimension fld(ims,jms)
      dimension xlat(jms),cosl(jms)
C
      dimension o3d(ims,jms,nt)
c
      dimension p5d(ims,jms,nt,neof,nic)
      dimension p4d(ims,jms,nt,neof)
C    
      dimension ac3d(ims,jms,neof)
      dimension ac4d(ims,jms,neof,nic)
C 
      dimension avgac1d(neof)
      dimension avgac2d(neof,nic)

C
      open(unit=10,form='unformatted',access='direct',recl=4*ims*jms) !obs
c
      open(unit=11,form='unformatted',access='direct',recl=4*ims*jms) !prd
      open(unit=12,form='unformatted',access='direct',recl=4*ims*jms) !prd
      open(unit=13,form='unformatted',access='direct',recl=4*ims*jms) !prd
      open(unit=14,form='unformatted',access='direct',recl=4*ims*jms) !prd
      open(unit=15,form='unformatted',access='direct',recl=4*ims*jms) !prd
      open(unit=16,form='unformatted',access='direct',recl=4*ims*jms) !prd
      open(unit=17,form='unformatted',access='direct',recl=4*ims*jms) !prd
      open(unit=18,form='unformatted',access='direct',recl=4*ims*jms) !prd
      open(unit=19,form='unformatted',access='direct',recl=4*ims*jms) !prd
      open(unit=20,form='unformatted',access='direct',recl=4*ims*jms) !prd
      open(unit=21,form='unformatted',access='direct',recl=4*ims*jms) !prd
      open(unit=22,form='unformatted',access='direct',recl=4*ims*jms) !prd
      open(unit=23,form='unformatted',access='direct',recl=4*ims*jms) !prd
      open(unit=24,form='unformatted',access='direct',recl=4*ims*jms) !prd
      open(unit=25,form='unformatted',access='direct',recl=4*ims*jms) !prd
      open(unit=26,form='unformatted',access='direct',recl=4*ims*jms) !prd
c
      open(unit=70,form='unformatted',access='direct',recl=4*ims*jms) !2dskill
      open(unit=75,form='unformatted',access='direct',recl=4*ims*jms) !2dskill
c************************************************
c
      PI=4.*atan(1.)
      CONV=PI/180.
      ...
      END DO
c
cc read in historical obs fld
c
      iu=10
      Do it=1,nt
      read(iu,rec=it) fld
      call f2d_2_f3d(ims,jms,nt,it,fld,o3d)
      Enddo
      write(6,*) 'finished reading model data'
c
c
cc read in all historical prd fld
c
      ...
      it=0
      Do is=1,nseason
      Do lp=1,13 ! wk# in each season
      it=it+1
      read(iu,rec=it) fld
      call f2d_2_f5d(fld,p5d,ims,jms,nt,neof,nic,it,ie,ic)
      Enddo
      Enddo
      Enddo
      Enddo
...
c
      ...
      Do j=1,jms
       p4d(i,j,it,ie)=0.
       do ic=1,nic  !for ICs of 1,2,3,4
       p4d(i,j,it,ie)=p4d(i,j,it,ie)+p5d(i,j,it,ie,ic)
     &/float(nic)
       enddo
      Enddo
      Enddo
...
c for ac3d
      ...
      do i=1,ims
      do j=1,jms
      if(abs(o3d(i,j,1)).lt.999) then
        do it=1,nt
          ts1(it)=o3d(i,j,it)
          ts2(it)=p4d(i,j,it,ie)
        enddo
        call corr_1d(nseason,ts1,ts2,ac3d(i,j,ie))
      else
        ac3d(i,j,ie)=undef1
      endif
      enddo
...
c for ac4d
      ...
      if(abs(o3d(i,j,1)).lt.999) then
        do it=1,nt
          ts1(it)=o3d(i,j,it)
          ts2(it)=p5d(i,j,it,ie,ic)
        enddo
        call corr_1d(nseason,ts1,ts2,ac4d(i,j,ie,ic))
      else
        ac4d(i,j,ie,ic)=undef1
      endif
      enddo
      enddo
      enddo
...
c for ac3d
      ...
      call ac2davg(fld,avgac1d(ie),ims,jms,imss,imse,
     &jmss,jmse,cosl)
...
c for ac4d
      ...
      do i=1,ims
      do j=1,jms
        fld(i,j)=ac4d(i,j,ie,ic)
      enddo
      enddo
      
      call ac2davg(fld,avgac2d(ie,ic),ims,jms,imss,imse,
     &jmss,jmse,cosl)

      ENDDO
      ENDDO
c write out skill
c for ac of ensemble mean
      iwr=0
      DO ie=1,neof
      do i=1,ims
      do j=1,jms
...
c for avgac2d
      do ic=1,nic
      do ie=1,neof
        avgac1d(ie)=avgac2d(ie,ic)
      enddo
      write(6,*) 'ic=',ic,avgac1d
      enddo
      write(75,rec=1) avgac2d
c
      STOP
      END
c*
...
c
      subroutine f2d_2_f5d(wk1,wk2,n1,n2,n3,n4,n5,k,m,n)
      dimension wk1(n1,n2),wk2(n1,n2,n3,n4,n5)
      do j=1,n2
      do i=1,n1
      wk2(i,j,k,m,n)=wk1(i,j)
      enddo
      enddo
      return
      end
