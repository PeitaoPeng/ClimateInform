...
c     parameter(imss=77,imse=121)   ! 190-300 for NA
c     parameter(jmss=41,jmse=65)    ! 10N-70N
...
c
      open(unit=85,form='unformatted',access='direct',recl=4*nic*neof) !2dskill
c************************************************
c
      PI=4.*atan(1.)
      CONV=PI/180.
...
c
      ...
      it=it+1
      read(iu,rec=it) fld
      call f2d_2_5d(fld,ac5d,ims,jms,nld,neof,nic,ld,ie,ic)
      it=it+1
      read(iu,rec=it) fld
      Enddo
      Enddo
...
c for avgac1d
      ...
      enddo
      iwr=iwr+1
c     write(87,rec=iwr) f1d
      ENDDO
c
      iwr=iwr+1
c     write(87,rec=iwr) avgac1d
c
...
c
      subroutine f2d_2_5d(wk1,wk2,n1,n2,n3,n4,n5,k,l,m)
      dimension wk1(n1,n2),wk2(n1,n2,n3,n4,n5)
      do j=1,n2
      do i=1,n1
      wk2(i,j,k,l,m)=wk1(i,j)
      enddo
      enddo
      return
      end
