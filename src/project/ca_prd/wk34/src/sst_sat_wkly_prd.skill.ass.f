...
c range for sst pat cor (ac_1d)
      parameter(ims=1,ime=180)   ! 0->360
      parameter(jms=1,jme=jmp)   ! 30S->70N
...
C
C
      open(unit=11,form='unformatted',access='direct',recl=4*imin*jmin) !input sst
...
c
      ...
      open(unit=61,form='unformatted',access='direct',recl=4*imp*jmp) !mon1_prd
      open(unit=62,form='unformatted',access='direct',recl=4*imp*jmp) !mon2_prd
      open(unit=63,form='unformatted',access='direct',recl=4*imp*jmp) !mon3_prd
c
...
c
      ...
      DO is=1,nseason          ! number of seasons used to for CA
        ...
        np=np+1
          ...
          itt=it+52*3
          read(iu2,rec=itt) t2d
            do j=1,jmp
            do i=1,imp
              fld4d(i,j,np,is)=fldin(i+lons-1,j+lats-1)  
              if(abs(fldin(i+lons-1,j+lats-1)).gt.9999999) then
c               write(6,*) 'undef in is=',is,'np=',np,'i,j=',i,j
c               fld4d(i,j,np,is)=0.
              endif
            enddo
            enddo
...
c* input data for EOF analysis
      ...
      do is=1,nseason
      do ip=1,npp
c     do ip=1,nps  !need to compare if use npp or nps
      it=it+1
        id=0
        do j=1,jmp
        do i=1,imp
          if(abs(fld4d(i,j,ip,is)).LT.50) then
          id=id+1
...
c
c temporal corr for predictand grids
c
      do i=1,imp
      do j=1,jmp
        it=1
        ...
        do ip=1,npp
          ...
          if(ldpen.eq.6) then
          ts1(it)=0.5*(fld4d(i,j,ip+ldpen-2,is)+
     &fld4d(i,j,ip+ldpen-1,is))
          endif
