...
c
      ...
      dimension allsat(ims,jms,52+nps,nyear)
...
C
C
      open(unit=11,form='unformatted',access='direct',recl=4*ims*jms) !input psi
...
c
      ...
      ridge=0.05
c 
cc read in all psi and sat data
c
      iu1=11
      iu2=12
c
...
c
      ...
      Enddo
c
cc test forecast from wk1 of the nyear-th year
c
c     Do 1000 itgtw=1,52          !loop over target week for test
      itgtw=13          !loop over target week for test
        do j=1,jmp
        do i=1,imp
        wk2d(i,j)=allpsi(i+lons-1,j+lats-1,itgtw+nwextb,nyear)  !IC for CA prd
        enddo
        enddo
c
...
c for IC data
        do nmode=1,modemax
          call inprod4(eof,wk2d,imp,jmp,cosl,nmode,modemax,anorm)
          efld1d(nmode)=anorm
        enddo
...
c* spatial cor for test run
         ...
         do j=1,jmp
         if(ldpen.lt.6) then
         obs(i,j)=allpsi(i+lons-1,j+lats-1,itgtw+nwextb+ldpen-1,nyear)
         endif
         if(ldpen.eq.6) then
         obs(i,j)=
     &(allpsi(i+lons-1,j+lats-1,itgtw+nwextb+ldpen-3,nyear)+
     &allpsi(i+lons-1,j+lats-1,itgtw+nwextb+ldpen-2,nyear))/2.
           endif
           prd(i,j)=tgt2d(i,j)
         enddo
         enddo
c for sat
         ...
         do j=1,jmt
         if(ldpen.lt.6) then
           obssat(i,j)=allsat(i,j,itgtw+nwextb+ldpen-1,nyear)
         endif
           if(ldpen.eq.6) then
           obssat(i,j)=0.5*(allsat(i,j,itgtw+nwextb+ldpen-3,nyear)+
     &allsat(i,j,itgtw+nwextb+ldpen-2,nyear))
           endif
           prdsat(i,j)=tgtsat2d(i,j) !Sat prd
         enddo
         enddo
      iwr=iwr+1
      write(85,rec=iwr) obs
      call changeundef(obssat,twk2d,undef1,undef2,imt,jmt)
      write(86,rec=iwr) twk2d
      iwr=iwr+1
      write(85,rec=iwr) prd
      write(86,rec=iwr) prdsat
c* calculate spatial cor
      call rmsd1(obs,prd,imp,jmp,imss,imse,jmss,jmse,cosl,
     &sprmspsi(ldpen),spacpsi(ldpen))
c
      call rmsd1(obssat,prdsat,imt,jmt,imts,imte,jmts,jmte,coslsat,
     &sprmssat(ldpen),spacsat(ldpen))

      ENDDO  !loop for ldpen
c
      write(6,*) 'test psi rms=',sprmspsi
      write(6,*) 'test psi ac==',spacpsi
      write(6,*) 'test sat rms=',sprmssat
      write(6,*) 'test sat ac==',spacsat
