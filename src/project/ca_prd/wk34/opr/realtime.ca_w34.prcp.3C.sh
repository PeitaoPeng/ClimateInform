...
cp $lcdir/realtime.ca_w34.p.3C.f $tmpp/prcp_prd.f
cp $lcdir/eof_4_ca.s.f $tmpp/eof_4_ca.s.f
...
ln -s $tmpdir/real_prd_psi200_prcp_wkly.bin fort.86
ln -s out.2.5x2.5.bin fort.87
#
./ca.x > $lcdir/realtime.w34.ca.out
#
cat>$tmpdir/real_prd_psi200_prcp_wkly.ctl<<EOF
dset ^real_prd_psi200_prcp_wkly.bin
undef -9.99E+33
title EXP1
...
cat>out.2.5x2.5.ctl<<EOF
dset ^out.2.5x2.5.bin
undef -9.99E+33
title prcp unit=0.1mm/day
xdef  144 linear   0 2.5
ydef   73 linear  -90 2.5
zdef  01 levels 1
tdef  12 linear jan1950 1mon
vars  1
f     1 99 psi based wk34
endvars
EOF
#=======================================
# regrid to 1x1
...
cat >regrid.gs<<gsEOF
'reinit'
'open out.2.5x2.5.ctl'
'open /cpc/home/wd52pp/data/obs/sst/grid.1x1.ctl'
'set gxout fwrite'
'set fwrite $tmpdir/ca_prcp_wk34.bin'
nt=1
while ( nt <= 12)
...
cat>$tmpdir/ca_prcp_wk34.ctl<<EOF
dset ^ca_prcp_wk34.bin
undef -9.99E+8
title prcp unit=0.1mm/day
...
cp $tmpdir/ca_prcp_wk34.ctl /cpc/home/wd52pp/data/wk34_fcst/ca/$fy4/$fm2/$fd2
cp $tmpdir/ca_prcp_wk34.bin /cpc/home/wd52pp/data/wk34_fcst/ca/$fy4/$fm2/$fd2
