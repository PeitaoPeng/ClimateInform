...
c
     ...
     &psiclm(i+lons-1,j+lats-1,itgtw)
      enddo
      enddo
      write(6,*) 'psiic=',psiic(1,1),psiic(imp,jmp)
      do i=1,ims
      do j=1,jms
...
c for historical data
        ...
        enddo
        enddo 
c       write(6,*) 'efld3d=',efld3d(modemax,nps,nseason)
c for IC data
        do nmode=1,modemax
          call inprod4(eof,psiic,imp,jmp,cosl,nmode,modemax,anorm)
          efld1d(nmode)=anorm
        enddo
        write(6,*) 'efld1d=',efld1d(1),efld1d(modemax)
c
c* realtime forecast
c
...
c past kocn year avg
      ...
      enddo
      enddo
c ocn prd(past kocn avg minus WMO 30-yr clim)
      do i=1,imt
      do j=1,jmt
        if(abs(t2d(i,j)).lt.999) then
...
c       twk2d2(i,j)=twk2d(i,j)/w34std(i,j) ! looks too strong
      enddo
      enddo
...
c have 3-class obs and prd
      do i=1,imt
      do j=1,jmt
      if(abs(w34std(i,j)).lt.999) then
      do is=1,nseason
      do iw=1,npp
      obssatw34(i,j,iw,is)=obssatw34(i,j,iw,is)/obsstd(i,j)
...
c
      ...
      if(abs(t2d(i,j)).lt.999) then
        ...
        do iw=1,npp
        it=it+1
        odata=obssatw34(i,j,iw,is)
        pdata=prdsatw34(i,j,iw,is)
        ...
        enddo
        enddo
        call hss3c_t(nts4,nts5,hs(i,j),npp*30,npp*nseason,npp*nseason)
c       call hss3c_t(nts4,nts5,hs(i,j),1,npp*nseason,npp*nseason)
      else
        hs(i,j)=t2d(i,j)
      endif
      enddo
