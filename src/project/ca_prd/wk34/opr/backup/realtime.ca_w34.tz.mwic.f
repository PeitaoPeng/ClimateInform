...
C CA prediction for weeks 2,3,4 3-4 sat and H500
C IC are multiweek
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
...
c
      ...
      dimension fldin(ims,jms),fldin2(ims,jms),fldin3(ims,jms)
      ...
      dimension allpsi(ims,jms,52+nps+mwic-1,nyear)
      dimension allsat(imt,jmt,52+nps,nyear)
      dimension fld4d(imp,jmp,nps+mwic-1,nseason)
      dimension tgt2d(imp,jmp),tgt3d(imp,jmp,npp) 
      dimension tgt4d(imp,jmp,npp,nseason)
      dimension psiclm(ims,jms,52+mwic-1),psiic(imp,jmp,mwic)
      dimension aaa(ngrd,ntt+mwic)
      dimension efld2d(modemax,mwic)
      ...
      dimension WK(ntt+mwic,ngrd)
      dimension ana(icttm,nhs+1),ana2(icttm,ntt+1)
      dimension eval(ntt+mwic),evec(ngrd,ntt+mwic)
      dimension pc(ntt+mwic,ntt+mwic)
...
C
      dimension twk_2(imt,jmt),twk_3(imt,jmt)
      dimension twk_4(imt,jmt),twk_34(imt,jmt)
C
C
...
c
      ...
      ridge=0.01
c
cc have psi clim data
c
      do it=1,52
      read(13,rec=it) fldin
      do i=1,ims
      do j=1,jms
        psiclm(i,j,it+mwic-1)=fldin(i,j)
      enddo
      enddo
      enddo
c fill the first mwic-1 space
      do it=1,mwic-1
      do i=1,ims
      do j=1,jms
        psiclm(i,j,it)=psiclm(i,j,52+it)
      enddo
      enddo
      enddo
cc have psi IC data, the 1st read is the latest one
      do ic=1,mwic
      read(10,rec=ic) fldin2
c
      do i=1,imp
      do j=1,jmp
        psiic(i,j,ic)=fldin2(i+lons-1,j+lats-1)-
     &psiclm(i+lons-1,j+lats-1,itgtw+mwic-ic)
      enddo
      enddo
c
      if(ic.eq.1) then
      do i=1,ims
      do j=1,jms
        fldin3(i,j)=fldin2(i,j)-psiclm(i,j,itgtw+mwic-1)
      enddo
      enddo
      endif
c
...
c
        do j=1,jms
        do i=1,ims
        allpsi(i,j,iw+nwextb+mwic-1,iy)=fldin(i,j) !mwic-1 for later use
        enddo
        enddo
c
        do j=1,jmt
...
c  for ext in the beginning
       Do iw=1,nwextb+mwic-1 ! have space for mwic
        do j=1,jms
        do i=1,ims
...
c
        do j=1,jms
        do i=1,ims
        allpsi(i,j,52+nwextb+mwic-1+iw,iy)=
     &allpsi(i,j,nwextb+mwic-1+iw,iy+1)
        enddo
        enddo
c
        do j=1,jmt
...
c
      ...
      is=is+1
        ...
        np=np+1
            do j=1,jmp
            do i=1,imp
              fld4d(i,j,np+mwic-1,is)=
     &allpsi(i+lons-1,j+lats-1,iw+mwic-1,iy)  
            ...
            enddo
        enddo
      write(6,*) 'time length of psi data =',irec
      ENDDO
c
c fill in the first mwic-1 space of fld4d
      if (mwic.gt.1) then
c for gerneral situation 
      is=0
      DO iy=2,nyear-1 
      is=is+1
        do iw=1,mwic-1  
            do j=1,jmp
            do i=1,imp
      fld4d(i,j,mwic-iw,is)=
     &allpsi(i+lons-1,j+lats-1,itgtw+mwic-1-iw,iy)  
              if(abs(fldin(i+lons-1,j+lats-1)).gt.999999999) then
                write(6,*) 'undef in iy=',iy,'np=',np,'i,j=',i,j
                fld4d(i,j,np,is)=0.
              endif
...
c               write(6,*) 'sat undef in iy=',iy,'np=',np,'i,j=',i,j
        ...
        enddo
c
      write(6,*) 'time length of sat data =',irec
      ENDDO
c
c* input data for EOF analysis
      ...
      it=it+1
        ...
        do i=1,imp
          id=id+1
          aaa(id,it)=fld4d(i,j,ip+mwic-1,is)*sqrt(cosl(j))
        end do
        end do
      enddo
      enddo
c add psiic to aaa
      write(6,*) 'for aaa, it=',it
      do ic=1,mwic
      it=it+1
        id=0
        do j=1,jmp
        do i=1,imp
          id=id+1
          aaa(id,it)=psiic(i,j,ic)*sqrt(cosl(j))
        end do
        end do
      end do
...
c       
      call eofs_4_ca(aaa,ngrd,ntt+mwic,ntt+mwic,eval,evec,pc,wk,0)
c
      write(6,*)'eval='
      write(6,400) eval
 400  format(6f8.4)
...
c for IC data
        do i=1,imp
        do j=1,jmp
          fldp(i,j)=psiic(i,j,ic)
        enddo
        enddo
        write(6,*) 'fldp=',fldp(1,1),fldp(imp,jmp)
        do nmode=1,modemax
          call inprod4(eof,fldp,imp,jmp,cosl,nmode,modemax,anorm)
          efld2d(nmode,ic)=anorm 
        enddo
        write(6,*) 'efld2d=',efld2d(1,1),efld2d(modemax,1)
...
c
          do ip=1,npp
            ...
            do ic=1,mwic
            mm=mm+1
              ana2(mm,it)=efld4d(nmode,ip+ldpen-1,ic,is)
            enddo
            enddo
c
            do i=1,imt
...
c add IC to ana2
        ...
        enddo
        enddo
        write(6,*) 'ana2=',ana2(1,ntt+1),ana2(modemax,ntt+1)
...
c psi forecast associated with the latest IC
        im=0
...
c construct sat prd
     ...
     &           satana2(i,j,ntime)*alpha2(ntime)
        ...
        enddo
c
c* PC to grid for constructed psi prd
c
        do i=1,imp
        do j=1,jmp
          tgt2d(i,j)=0.
     ...
     &          etgt1d(nmode)*eof(nmode,i,j)
        ...
        enddo
c
c* have wk2,3,4 & 34 for sat and psi
c
      if(ldpen.eq.4) then
...
c save wk2,3,4
      if(ldpen.eq.3) then
      call pass_2d(tgtsat2d,twk_2,imt,jmt)
      endif
      if(ldpen.eq.4) then
      call pass_2d(tgtsat2d,twk_3,imt,jmt)
      endif
      if(ldpen.eq.5) then
      call pass_2d(tgtsat2d,twk_4,imt,jmt)
      endif
...
c
cc statistics(mean and stdv) of wk34 prd & OBS 
c
...
c       twk2d(i,j)=prdsat(i,j)+satocn(i,j)
        ...
        twk_2(i,j)=twk_2(i,j)+sattrd(i,j,nseason)
        twk_3(i,j)=twk_3(i,j)+sattrd(i,j,nseason)
        twk_4(i,j)=twk_4(i,j)+sattrd(i,j,nseason)
        twk2d2(i,j)=twk2d(i,j)/obsstd(i,j)
      enddo
      enddo
c have prob forecast for wk34
      xdel=10./float(kpdf)
...
c
      ...
      do j=1,jmt
        ...
        if(abs(t2d(i,j)).lt.999) then
        call prob(twk2d2(i,j),kpdf,xbin,xdel,ypdf,pp,pn)
        if(twk2d2(i,j).gt.0) then 
        prbprd(i,j)=pp
        else
        prbprd(i,j)=-pn
        endif
...
c
      ...
      write(87,rec=3) twk_2
      write(87,rec=4) twk_3
      write(87,rec=5) twk_4
      write(87,rec=6) twk2d !wk34
      write(87,rec=7) prbprd
      write(87,rec=8) bb
...
c
      ...
      enddo
      write(87,rec=9) hs
      write(87,rec=10) fldin3
      write(87,rec=11) obsstd
      write(87,rec=12) w34std
C*********************************************
