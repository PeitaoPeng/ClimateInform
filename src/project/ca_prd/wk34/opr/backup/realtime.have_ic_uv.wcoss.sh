...
module load ics hpss GrADS ibmpe
ulimit -c 0

export GADDIR=/usrx/local/GrADS/2.0.2/lib
export GASCRP="/u/Mingyue.Chen/grads/scripts"
export GAUDFT="/u/Mingyue.Chen/grads/udf/udf.txt"

lcdir=/sss/cpc/save/Peitao.Peng/project/ca_prd/wk34
tmpdir=/ptmpp1/Peitao.Peng
if [ ! -d $tmpdir ] ; then
  mkdir -p $tmpdir
fi
datadir=/sss/cpc/shared/Peitao.Peng/wk34
outdata=/sss/cpc/shared/Peitao.Peng/wk34
bindir=/sss/cpc/save/Peitao.Peng/bin
cp $datadir/fort.10 $tmpdir/.
#
#================================================
...
/u/Wanqiu.Wang/bin/wgrib pgb.ave | egrep -e ":(UGRD|VGRD):" | grep -e ":200 mb:" | /u/Wanqiu.Wang/bin/wgrib pgb.ave -i -grib -o uv200.grb
/u/Mingyue.Chen/bin/grib2ctl.pl uv200.grb > uv200.ctl
set +e
/u/Wanqiu.Wang/bin/gribmap -0 -i uv200.ctl
set -e
#
...
'c'
EOF
/usrx/local/GrADS/2.0.2/bin/grads -bl <readuv
#
# from uv to psi
