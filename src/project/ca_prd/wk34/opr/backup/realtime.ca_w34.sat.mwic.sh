...
#for modemax in 35; do
#
cp $lcdir/realtime.ca_w34.tz.mwic.f $tmpdir/sat_prd.f
...
ln -s $tmpdir/real_prd_psi200_sat_wkly.${mwic}ics.$modemax.bin fort.86
ln -s $tmpdir/ca_sat_wk34.${mwic}ics.$modemax.bin fort.87
#
./ca.x > $lcdir/realtime.w34.ca.${mwic}ics.$modemax.out
#
cat>$tmpdir/real_prd_psi200_sat_wkly.${mwic}ics.$modemax.ctl<<EOF
dset ^real_prd_psi200_sat_wkly.${mwic}ics.$modemax.bin
undef -9.99E+33
title EXP1
...
cat>$tmpdir/ca_sat_wk34.${mwic}ics.$modemax.ctl<<EOF
dset ^ca_sat_wk34.${mwic}ics.$modemax.bin
undef -9.99E+33
title EXP1
xdef  144 linear   0 2.5
ydef   73 linear  -90 2.5
