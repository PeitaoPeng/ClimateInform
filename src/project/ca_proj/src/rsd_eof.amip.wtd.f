...
c     PARAMETER (ngrd=(late-lats+1)*(lone-lons+1))
      ...
      real rcof(nmod+2,ltime)
      real pc1(ltime),pc2(ltime)
      real regr(imx,jmx),corr(imx,jmx)
      real regr2(imx,jmx),corr2(imx,jmx)
...
c have anom in the period
      ...
      enddo
c
c== read in 1st and 2nd rpc of SST
c
      ir=0
      do m=1,2
      do it=1,ltime
      ir=ir+1
      read(11,rec=ir) rcof(m,it)
      enddo
      enddo
c
c== detrend data
...
c write out of trend data of all years
      ...
      enddo
c
c== regr sst rpc1&2 to undetrended data
      do it=1,ltime
        ts1(it)=rcof(1,it)
        ts2(it)=rcof(2,it)
      enddo
      ...
      call regr_t(ts1,ts3,ltime,corr(i,j),regr(i,j))
      call regr_t(ts2,ts3,ltime,corr2(i,j),regr2(i,j))
      else
...
c         w3d2(i,j,it)=afld(i,j,it)-xn34(it)*regr(i,j)
          w3d2(i,j,it)=w3d(i,j,it)-rcof(1,it)*regr(i,j)-
     &    rcof(2,it)*regr2(i,j)
        else
...
c
      ...
      call regr_t(pc1,ts2,ltime,rcorr(i,j),rregr(i,j))
      else
