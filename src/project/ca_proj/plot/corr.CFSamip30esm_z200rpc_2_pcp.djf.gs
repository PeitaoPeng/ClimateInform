'reinit'
'run /cpc/home/wd52pp/bin/white.gs'
'run /cpc/home/wd52pp/bin/rgbset.gs'

'open /cpc/home/wd52pp/data/ca_proj/corr.CFSz200rpc_vs_pcp.djf.yb1957.ctl'

*---------------------------string/caption
 'set string 1 tc 4'
 'set strsiz 0.15 0.15'
 'draw string 5.5 8.5 Regr of Precip to RPC of Z200 (CFS_AMIP30_esm,1957-2021)'
 'set strsiz 0.12 0.12'
*---------------------------set dimsnesion, page size and style
*
nframe=6
nframe2=3

xmin0=0.75;  xlen=4.25;  xgap=0.5
ymax0=8.0; ylen=-2.35;  ygap=-0.1
*
iframe=1
ieof=1
ipc=1
while ( iframe <= nframe )
  icx=1; if (iframe > nframe2); icx=2; endif
  xmin=xmin0+(icx-1)*(xlen+xgap)
  xmax=xmin+xlen
  icy=iframe; if (iframe > nframe2); icy=iframe-nframe2; endif
  if(iframe >= 3); ylen=-2.35; endif
  if(iframe >= 3); ygap=-0.1; endif
  ymax=ymax0+(icy-1)*(ylen+ygap)
* if(iframe = 3); ymax=ymax0+(icy-1)*(ylen+ygap)-0.1; endif
  ymin=ymax+ylen
  titlx=xmin+0.5
  titly=ymax-1.8
* if(iframe = 6); titly=ymax+0.2; endif
* if(iframe = 7); titly=ymax+0.1; endif
* barx=xmin+4.1
  barx=5.25
  bary=ymin-0.45
* say xmin; say xmax; say ymin; say ymax
  'set vpage 0 11 0 8.5'
  'set parea 'xmin' 'xmax' 'ymin' 'ymax
*
'set lat -89.5 89.5'
'set ylint 30'
'set lon 0 360'
'set xlint 60'
'set grads off'
'set grid on'
'set poli on'
'set xlab off'
if(iframe = 3); 'set xlab on'; endif
if(iframe = 6); 'set xlab on'; endif
*'set mpdset mres'
*'set gxout grfill'
*
'set gxout shaded'
'set clevs  -2.5 -2.0 -1.5 -1.0 -0.5 -0.2 -0.1 0.1 0.2 0.5 1. 1.5 2 2.5'
'set ccols  79 78 77 75 74 73 72 0 32 33 34 35 37 38 39'
'set t '%ieof 
if(iframe = 1); 'd -reg'; endif
if(iframe = 2); 'd reg'; endif
if(iframe = 3); 'd reg'; endif
if(iframe > 3); 'd -reg'; endif
'set gxout contour'
'set cint 0.2'
*'d -reg'
'run /cpc/home/wd52pp/bin/dline.gs 0 0 360 0'
'run /cpc/home/wd52pp/bin/dline.gs 180 -90 180 90'
 'set string 1 tl 4'
 'set strsiz 0.12 0.12'
*----------
if(iframe = 1); 'draw string 'titlx' 'titly' RPC1'; endif
if(iframe = 2); 'draw string 'titlx' 'titly' RPC2'; endif
if(iframe = 3); 'draw string 'titlx' 'titly' RPC3'; endif
if(iframe = 4); 'draw string 'titlx' 'titly' RPC4'; endif
if(iframe = 5); 'draw string 'titlx' 'titly' RPC5'; endif
if(iframe = 6); 'draw string 'titlx' 'titly' RPC6'; endif
*
*
if(iframe = 3); 'run /cpc/home/wd52pp/bin/cbarn.gs 0.75 0 'barx' 'bary''; endif

iframe=iframe+1
ieof=ieof+1
ipc=iframe-3

endwhile

'printim corr.CFSamip30esm_z200rpc_2_pcp.djf.png gif x1600 y1200'
