...
#=========================================================
# MLR correction for NMME forecasted T&P 
#=========================================================
...
c      parameter(ism=1,iem=360,jsm=71,jem=161) !8280
       parameter(ism=1,iem=360,jsm=71,jem=111) !3780
c      parameter(ism=1,iem=360,jsm=111,jem=161) !4680
...
c
      ...
      real reval2(nmod),revec2(ngrd,nmod),rcoef2(nmod,nt)
...
C select grid data
      ...
      do i=ism,iem,2
      do j=jsm,jem,2
#       if(land(i,j).lt.1.or.w3dm(i,j,it).lt.-1000) then
        ig=ig+1
...
c write varm paterns

      enddo ! loop m
c
c EOF analysis for OBS data
c
c
C grid data of obs
      DO it=1,nt
...
c skill of each rpc
      do m=1,nmod
        do it=1,nt
          ...
          ts2(it)=rpcf2(m,it)
        enddo
        call acrms_t(ts1,ts2,corfo(m),rmsfo(m),nt)
        print *, 'm=',m,'cor&rms=',corfo(m),rmsfo(m)
        ncor(m)=1.
        if(corfo(m).lt.0) ncor(m)=0
      enddo ! loop m
        print *, 'ncor=',ncor
c
c forecasted var on grids
      ...
      if(land(i,j).gt.0) then
        do m=1,nmod
          prd(i,j,it)=prd(i,j,it)+ncor(m)*rpcf2(m,it)*regro(i,j,m)
        enddo
      else
          prd(i,j,it)=undef
      endif
