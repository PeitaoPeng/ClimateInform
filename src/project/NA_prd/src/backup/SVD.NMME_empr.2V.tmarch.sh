...
#=========================================================
# time marching SVD forecast using NMME forecasted SST
#=========================================================
...
c for var1
      ...
      call regr_t(ts1,ts3,nt,mt,corr11(i,j,m),regr11(i,j,m))
      call regr_t(ts2,ts3,nt,mt,corr21(i,j,m),regr21(i,j,m))
...
c for var2
      ...
      call regr_t(ts1,ts3,nt,mt,corr12(i,j,m),regr12(i,j,m))
      call regr_t(ts2,ts3,nt,mt,corr22(i,j,m),regr22(i,j,m))
      ...
      enddo !m loop
c     write(6,*)' after corr '
c
c forecast
C
        do i=1,imx
        do j=1,jmx
c         wic(i,j)=w3dv1(i,j,mt+1)
          wic(i,j)=w3dvm(i,j,mt-32)
        enddo
        enddo
...
c         if(w3dv1(i,j,1).gt.-1000) then
          ...
          ngd3=ngd3+1
          if(id.eq.1) then
c         aic(ngd3)=wic(i,j)*cosr(j)/std1(i,j)
...
c forecasted var2 on grids
      ...
      if(abs(w3dv2(i,j,1)).lt.999) then
        do m=1,nmod
          prd(i,j,itp)=prd(i,j,itp)+cic(m)*regr12(i,j,m)
        enddo
      else
          prd(i,j,itp)=undef
      endif
