...
#byear=1978
byear=1949
nyrp=39  #hcst period 1982->2020
nyro=72  #obs period 1949->2020
nyrt=52  #training data 1949-??
nyrv=`expr $nyro - $nyrt`  # verification period long ??-2020
#
imx=360
jmx=180
#ngrdv1=5745 # sst/or SST_t2m
ngrdv1=2798 # sst/or SST_t2m
#ngrdv1=2718 # sst/or SST_t2m
ngrdv2=1690  # prec
#
...
#=======================================
infile=$datadir3/NMME.$varm.ss.${micm}_ic.1982-2020.ld1-7.esm.anom
outfile=NMME.$varm.ss.${micm}_ic
#ntend=`expr $nyr \* 7`
ntend=273
cat >int<<fEOF
reinit
...
cat > svd.f << EOF
      ...
      dimension w2dm(imx,jmx)
      dimension wic(imx,jmx),aic(ng1),cic(nmod,ntp)
      dimension w3dv1(imx,jmx,nt),w3dv2(imx,jmx,nt)
      dimension w3dvm(imx,jmx,ntp)
      dimension prd(imx,jmx,ntp)

      dimension tso(ntv),tsf(ntv),tsv(ntv)
      dimension ts1(ntt),ts2(ntt)
      dimension ts3(ntt),ts4(ntt)
      ...
      logical matu,matv
c
      real cof1(nmod,ntt),cof2(nmod,ntt)
...
c
      ...
      open(unit=25,form='unformatted',access='direct',recl=4*imx*jmx)
...
c
      ...
      do j=1,jmx
        ...
        enddo
        std1(i,j)=sqrt(std1(i,j)/float(ntt))
        endif
      enddo
      enddo
c
      ...
      do j=1,jmx
        ...
        enddo
        std2(i,j)=sqrt(std2(i,j)/float(ntt))
        endif
      enddo
      enddo
c
      ...
      do j=1,jmx
        ...
        stdm(i,j)=0.
        do it=1,ntp
        stdm(i,j)=stdm(i,j)+w3dvm(i,j,it)*w3dvm(i,j,it)
        enddo
        stdm(i,j)=sqrt(stdm(i,j)/float(ntp))
        endif
      enddo
      enddo
...
c
      ...
      ngd1=0
        do i=isv1,iev1,2
        do j=jsv1,jev1,2
c         if(w3dv1(i,j,1).gt.-1000) then
          if(w3dvm(i,j,1).gt.-1000.and.w3dv1(i,j,1).gt.-1000) then
          ngd1=ngd1+1
          if(id.eq.1) then
...
c
      ...
      a(i,j)=0.
      do k=1,ntt
      a(i,j)=a(i,j)+aleft(i,k)*aright(j,k)/float(ntt)
      enddo
...
c for var1
      ...
      call regr_t(ts1,ts3,ntt,corr11(i,j,m),regr11(i,j,m))
      call regr_t(ts2,ts3,ntt,corr21(i,j,m),regr21(i,j,m))
...
c for var2
      ...
      call regr_t(ts1,ts3,ntt,corr12(i,j,m),regr12(i,j,m))
      call regr_t(ts2,ts3,ntt,corr22(i,j,m),regr22(i,j,m))
...
c CV-1 for forecast
C
      call setzero_3d(prd,imx,jmx,ntp)

      DO itgt=1,ntp  !loop over target yr

        do i=1,imx
        do j=1,jmx
          wic(i,j)=w3dvm(i,j,itgt)
c         wic(i,j)=w3dv1(i,j,itgt+33)
        enddo
        enddo
...
c       write(6,*) 'it1=',it1,' it2=',it2
        ...
        enddo
c       write(6,*) 'it1=',it1,' it2=',it2
        call acrms_t(tsf,tso,corp(i,j),rmsp(i,j),ntv)
      else
        corp(i,j)=undef
        rmsp(i,j)=undef
      endif
...
#
 ...
 ln -s $outfile.360x180.gr  fort.25
#
