...
c     Calculate unrotated EOFs based on 1950-2000 monthly means
c 
c     ************************************************************************
c     **                                                                    **
c     **          *****    *   *           ***** ***** *****                **
...
c     **                                                                    **
c     **  program   :                                                       **
c     **                 eof_unrotated_500hpa_calculate.f                   **
c     **                                                                    **
c     **  objective :                                                       **
c     **                 Calculate the unrotated eofs                       **
c     **                                                                    **
c     **  input     :                                                       **
c     **                 ntime  ==  number of time steps                    **
c     **                                                                    **
c     **                 ngpt ==  number of space grid boxes                **
c     **                                                                    **
c     **                 data   ==  anomaly data                            **
c     **                                                                    **
c     **  output    :                                                       **
c     **                 eval   ==  eign values                             **
c     **                                                                    **
c     **                 space  ==  space loading function                  **
c     **                                                                    **
c     **                 pc     ==  principal components                    **
c     **                                                                    **
c     ************************************************************************
c 
      parameter(iyrbeg=1949,iyrbeg2=1950,iyrend=2000,nmo=12,
     &nyr=iyrend-iyrbeg2+1,ntime=nyr*3,nmode=nyr*3,ngpt=144*28,neof=10)  
c
      dimension  data(ngpt,ntime), covar(ntime,ntime),      
     #           eval0(nmode),evec0(ntime,ntime),pcout(nyr), 
     #           eval(nmode), evaln(nmode,nmo),arrout(ngpt),  
     #           pc(ntime,nmode),    space(ngpt,ntime),   
     #           work(ntime*(ntime+1)/2+ntime)
c
      dimension  buff(ntime),arr(ngpt)  
c
      character  cfile10*65, cfile51*67, cfile52*64, cfile53*65     
c
c Open data file containing monthly standardized anomalies 1949-2001.
c Data is created using monthly_tele_indices_1950-pres.gs  
c 
c     input data file 
      cfile10 ( 1: 28) = '/cpc/home/wd52gb/eof_new/'
      cfile10 (29: 65) = 'monthly_500hpa_stdanoms_1949-2001.dat'
c  
c     output spatial loading functions  (unrotated modes)  
      cfile51 ( 1: 28) = '/cpc/home/wd52gb/eof_new/'
      cfile51 (29: 67) = 'eof_unrotated_500hpa_loadingpatterns.bi'
c 
c     time series of unrotated PCs 
      cfile52 ( 1: 28) = '/cpc/home/wd52gb/eof_new/'
      cfile52 (29: 64) = 'eof_unrotated_500hpa_pctimeseries.bi'
c 
c     total explained variance of for each of 165 unrotated modes (%) 
      cfile53 ( 1: 28) = '/cpc/home/wd52gb/eof_new/'
      cfile53 (29: 65) = 'eof_unrotated_500hpa_explvariance.dat'
c
      open(unit=10,file=cfile10,access='direct',recl=ngpt*4) 
      open(unit=51,file=cfile51,access='direct',recl=ngpt*4) 
      open(unit=52,file=cfile52,access='direct',recl=nyr*4) 
      open(unit=53,file=cfile53,access='sequential',form='formatted')   
      print *,'finished openning files' 
c
c     Read in the standardized anomalies (sqrt cosine weighted)
c     for each 3-month running period.
c     Data is read in from Dec. 1949 - Jan 2001, which yields a
c     complete set of monthly grids for all seasons from 1950-2000
c     in order to calculate the leading EOFs.
c
      do 10 imo=1,nmo 
 ...
 45      continue
      ...
      do 20 iyr=iyrbeg2,iyrend 
            ...
            if(imo.eq.1) then
              ...
              irec1=(iyr1-iyrbeg)*12 + imo1 
              read(10,rec=irec1) arr 
            do 25 igpt = 1, ngpt  
 25         data (igpt,icnt) = arr(igpt) 
c          print *,iyr1,imo1,irec1,icnt
              
              icnt = icnt + 1  
              irec2=(iyr-iyrbeg)*12 + imo 
            read(10,rec=irec2) arr 
            do 30 igpt = 1, ngpt  
 30         data (igpt,icnt) = arr(igpt) 
c           print *, iyr,imo,irec2,icnt
            ...
            if(imo.eq.12) then
               imo1=1
               iyr1=iyr+1
               irec3=(iyr1-iyrbeg)*12 + imo1 
            else
               irec3=(iyr-iyrbeg)*12 + imo1             
            end if
            read(10,rec=irec3) arr 
            do 35 igpt = 1, ngpt  
 35         data (igpt,icnt) = arr(igpt) 
...
c 
 ...
 2501   covar(kt1,kt2) = covar(kt1,kt2) + data(ks,kt1)*data(ks,kt2) 
        covar(kt1,kt2) = covar(kt1,kt2)/float(ngpt) 
 2502 continue 
      print *,'finished calculating co-variance matrix' 
c
c     Calculate the eigenvalues 
c
      call  eigrs (covar, ntime, 12, eval0, evec0, ntime, work, ier) 
c
      evalt = 0.0 
 ...
 3003 continue 
      ...
      do 3004 imode = 1,nmode
        evaln(imode,imo) = 100.*eval(imode)/evalt 
 3004 continue
      print *,'finished calculating eignvalues' 
c 
...
c
 ...
 105  arrout(igpt)=space(igpt,imode)
      irec= (imo-1)*neof+imode
      write(51,rec=irec) arrout
 100  continue
c
      do 110 imode=1,neof
      do 115 iyr=1,nyr
 115  pcout(iyr)=pc(iyr,imode) 
      irec=(imo-1)*neof+imode
      write(52,rec=irec) pcout 
 110  continue
      print *,'finished writing loading patterns for month ',imo
 
 10    continue
  
c
c     Output the explained variance of the total 165 modes 
c     for each month
c
      do 8003 imode = 1,nmode 
 8003 write  (53,8051) imode,(evaln(imode,imo),imo=1,nmo)
 8051 format  (i3,1x,12f6.2)
      print *,'finished writing out explained variance '
