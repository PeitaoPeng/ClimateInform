...
c read in time values produced by Grads script
      ...
      open (2,file='/cpc/home/wd52pp/project/modes/gb/daymon.dat',
     &status='unknown')
c
...
c 1950-2020
      data iposn/-2,  -4,  5,-10,  3, -9,  6, -7,  0,  0,
     &           -1,  -6, -3,  9,  4, -8,  5,  7,-10,  0,
     &            2,   5, -4,-10,  3, -7, -8, -6,  9,  0,
     &           -2,   8,  3,-10,  4, -6,  7,  0,  9,  0,
     &           -3,  -4, -8,  7,  2, -5,  9,  0,  8,  0,
     &           -2,   6,  5,-10, -8,  7,  4,  0,  9,  0,
     &           -2,  -3,  7,  9,  5, -4,  8,  0, 10, -6,
     &           -6,   4, -7,  4, -2,  9, 10,  0, -8, -5,
     &            6,  -9,-10,  3, -4, -7, -8,  0,  0, -5,
     &           -4,  -9,-10,  3, -7,  2,  6,  0, -8,  5,
     &           -2,  -8, -7, -5, -4, -3, -6,  0,-10,  9,
     &           -2,  -8, -5,  6, -4, 10,  7,  9,  3,  0/  
c 1950-2000
      data iposn/-3,  -5, -1, -9, -2,  7, 10,  8, -6,  0,
     &            2,   5,  1,  9,  3, -4,  7,  8,-10,  0,
     &            2,  -4, -1,  9,  3,  0,  7, -8,-10,  0,
     &           -3,   8, -4,-10,  2, -6,  5,  0,  0,  0,
     &           -5,  -7, -4, 10, -2, -8,  9,  0,  0,  0,
     &           -2,  -6, -7,  5, -9,  8,  0,  0,  0,  0,
     &            4,  -5, -7, -6, -2,-10,  0,  0,  0,  9,
     &           -7,   4,  5, -3, -2, -8, -10, 0,  0, -6,
     &            5,  -9, -3, -4, -2,  8,  7,  0,  0,  6,
     &            3,   9, -7, -8, -6, -4,  5,  0,  0,  2,
     &            4,   5,  8, -9,  2,  5,  3, -7,  0,-10,
     &           -3,  -6, -4,  9, -2, 10, -8,  7,  0,  0/  

c Input standardized anomalies--arranged south-to-north
      open (1,
     &file='monthly_tele_indices_1950-pres.in',
     &status='old',form='unformatted',access='direct',recl=ngpt)
c
c Input rotated loading patterns--arranged south-to-north
c   
      open (10,
     &file='500z.loading.patterns_little_endian',
     &status='old',form='unformatted',access='direct',recl=ngpt)
c
c Input un-rotated loading patterns---arranged south-to-north
c   
      open (11,
     &file='eof_unrotated_500hpa_loadingpatterns_little_endian.bi',
     &status='old',form='unformatted',access='direct',recl=ngpt)
c
...
c     
      open (12,
     &file='eofmon_meansd.dat',
     &form='formatted',status='unknown')

c Output files for standardized EOF amplitudes

      open (22,
     &file='eofmonrot_new.tim',status='unknown')
      open (23,
     &file='eofmonftp_new.tim',status='unknown')

c Grads files for non-standardized and standardized EOF amplitudes

      open (24,
     &file='monthly_raw_tele_indices.bi',
     &form='unformatted',access='direct',status='unknown',recl=neof1)     
      open (26,
     &file='monthly_raw_tele_indices.ctl',status='unknown')           

      open (25,
     &file='monthly_tele_indices.bi',
     &form='unformatted',access='direct',status='unknown',recl=neof1)     
      open (27,
     &file='monthly_tele_indices.ctl',status='unknown')           
c**************************************************************   
      do bmi=1,12
...
c
      ...
      if(irotat .eq. 1) call amplit(z,neweofs,ieofcnt,ipos2,ampl)
...
c
       ...
       write(26,300)
 300  format('DSET /cpc/home/wd52gb/eof_new/monthly_raw_tele_
     &indices.bi')
       write(27,301)
c23456789012345678901234567890123456789012345678901234567890123456789012
 301  format('DSET /cpc/home/wd52gb/eof_new/monthly_tele_indi
     &ces.bi')
       write(26,302) 
       write(27,302)
 302   format('*') 
...
c
      ...
      real aa(numeofs,numeofs),zproj(numeofs),sum(nsn,numeofs),pi,rad
c
...
c
      ...
      zproj(i)=0.
      do 30 j=1,ngpt
 30   zproj(i)=zproj(i)+zz(j)*aeofs(i,j)/sum(mon,i)
 25   continue
c
c put teleconnection indices in array "ampltud" 
c back into the proper columns. 
...
c
 ...
 615  format('NAO  EA   WP  EP/NP PNA EA/WR SCA TNH   POL  PT ')
      do imo=1,nsn
c      write(12,620) imo,(zsqr(imo,j),j=1,neof)
