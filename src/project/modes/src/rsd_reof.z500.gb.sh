...
cp $lcdir/rsd_reof.z500.gb.f $tmp/eof.f
...
if [ $mon == Jun ]; then imon=18; imm=17; imp=19; ina=2; ipna=5; fi
if [ $mon == Jul ]; then imon=19; imm=18; imp=20; ina=2; ipna=5; fi
if [ $mon == Aug ]; then imon=20; imm=19; imp=21; ina=2; ipna=4; fi
if [ $mon == Sep ]; then imon=21; imm=20; imp=22; ina=2; ipna=2; fi
if [ $mon == Oct ]; then imon=22; imm=21; imp=23; fi
if [ $mon == Nov ]; then imon=23; imm=22; imp=24; fi
if [ $mon == Dec ]; then imon=24; imm=23; imp=25; fi
#
#infile=R1.z500.jan1949-dec2021.std_cos
 infile1=R1.z500.jan1949-dec2001.std_cos
 
 infile2=pc.NA.R1_$var.$mon.mtx$mtx.nmod5.yre${end_yr}
 infile3=rpc.PNA.R1_$var.$mon.mtx$mtx.nmod5.yre${end_yr}
#
 outfile1=rsd_reof.${eof_area}.R1_$var.$mon.mtx$mtx.nmod$nmode.yre${end_yr}
 outfile2=rsd_reof.${eof_area}.R1_$var.$mon.mtx$mtx.nmod$nmode.yre${end_yr}.glb
 outfile3=rsd_rpc.${eof_area}.R1_$var.$mon.mtx$mtx.nmod$nmode.yre${end_yr}
#
cat > parm.h << eof
       ...
       parameter(ina=$ina, ipna=$ipna)
eof
gfortran -o eof.x reof.s.f eof.f
echo "done compiling"

#
 ln -s $datadir/$infile1.gr     fort.11
 ln -s $datadir/$infile2.gr     fort.12
 ln -s $datadir/$infile3.gr     fort.13
#
...
if(iframe = 6);'draw string 'xstr' 'ystr' e)REOF5'; endif
*endif
*----------
'set string 1 tl 5 0'
