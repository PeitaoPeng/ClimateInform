...
C===========================================================
      ...
      PARAMETER (nmode=ifld,neof=nmod)
      ...
      real f3d(imx,jmx,ny),f4d(imx2,jmx2,ny,3),f4d2(imx,jmx,ny,3)
      real clim(imx,jmx),stdv(imx,jmx)
      real varp(imx,jmx),var3m(imx,jmx,3),vartot(imx,jmx)
      real ts1(ifld),ts2(ifld)
      real tsna(ifld),tspna(ifld)
      real RPC2(neof,ny),ts3(ifld)
      real afld(imx2,jmx2,ifld)
      real arsd(imx2,jmx2,ifld)
      real corr(imx2,jmx2),regr(imx2,jmx2)
      real rna(imx2,jmx2),rpna(imx2,jmx2)
      real tcof(nmod,ifld)
      real eout(ngrd)
      real eout2(ngrd)
     ...
     &,EVCT(ngrd,ifld)
C
      open(11,form='unformatted',access='direct',recl=4*imx2*jmx2) !obs
      open(12,form='unformatted',access='direct',recl=4*ifld) !NA index
      open(13,form='unformatted',access='direct',recl=4*ifld) !PNA index
C
      open(51,form='unformatted',access='direct',recl=4*imx*jmx)
      open(52,form='unformatted',access='direct',recl=4*imx2*jmx2)
      open(53,form='unformatted',access='direct',recl=4)
c
C==read in data
c
      read(12,rec=ina) tsna
      read(13,rec=ipna) tspna
c
      do ms=1,3
        if(ms.eq.1) ims=imm 
        if(ms.eq.2) ims=mons 
...
ccc take off ina and ipna related variability 
      call normal(tsna,ifld) 
      call normal(tspna,ifld) 
      do i=1,imx2
      do j=1,jmx2
        do it=1,ifld
        ts1(it)=afld(i,j,it)
        enddo
        call regr_t(ts1,tsna,ifld,corr(i,j),rna(i,j),undef)
        call regr_t(ts1,tspna,ifld,corr(i,j),rpna(i,j),undef)
      enddo
      enddo
c
      do it=1,ifld
        do i=1,imx2
        do j=1,jmx2
          arsd(i,j,it)=afld(i,j,it)-tsna(it)*rna(i,j)
     &-tspna(it)*rpna(i,j)
        enddo
        enddo
      enddo
...
c     DO 208 J=1,neof
      DO 208 J=2,neof+1
c       RWORK2(I,J)= EVCT(I,J)*SQRT(eval(J))
        RWORK2(I,J-1)= EVCT(I,J)*SQRT(eval(J))
...
c
      ...
      DO 206 J=1,ifld
         RPC(I,J)=0.
         DO 206 K=1,neof
 206     RPC(I,J)=RPC(I,J)+T(K,I)*PC(K+1,J)
      PRINT *, '...... OK ROTATION'
C
...
c     for the leading rotated EOFs
c
c output global regr pattern
      do 403 km = 1, neof
...
c         ts1(it)=PC(km,it)
          ts2(it)=afld(i,j,it)
        enddo
        call regr_t(ts1,ts2,ifld,corr(i,j),regr(i,j),undef)

        enddo
        enddo
      write(52,rec=km) regr
 403  continue
c
c output lat(20-87.5)regr pattern
      ...
      call noreof(eout2,ngrd)
      irec=irec+1
      write(51,rec=irec) eout
      irec=irec+1
      write(51,rec=irec) eout2
...
c write % expalined by each mode
 ...
 6002 continue
c
c pick & write out rpc for target month
c fill RPC2 with undef
      do m=1,neof
        do i=1,ny
          ts3(i)=RPC(m,ny+i)
        enddo
