...
cp $lcdir/eof.z500.gb.local.f $tmp/eof.f
...
#infile=R1.z500.jan1949-dec2021.std_cos
 ...
 outfile1=eof.${eof_area}.R1_$var.$mon.mtx$mtx.nmod$nmode.yre${end_yr}
 outfile2=eof.${eof_area}.R1_$var.$mon.mtx$mtx.nmod$nmode.yre${end_yr}.glb
 outfile3=pc.${eof_area}.R1_$var.$mon.mtx$mtx.nmod$nmode.yre${end_yr}
#
cat > parm.h << eof
...
if(iframe = 1);'draw string 'xstr' 'ystr' a)EOF1'; endif
if(iframe = 2);'draw string 'xstr' 'ystr' b)EOF2'; endif
if(iframe = 3);'draw string 'xstr' 'ystr' c)EOF3'; endif
if(iframe = 4);'draw string 'xstr' 'ystr' d)EOF4'; endif
if(iframe = 5);'draw string 'xstr' 'ystr' e)EOF5'; endif
*endif
*----------
'set string 1 tl 5 0'
