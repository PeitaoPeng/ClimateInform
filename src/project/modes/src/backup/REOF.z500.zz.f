...
ccc feed matrixd data
      ...
      ir=ir+1
        ...
        do i=is,ie
          ng=ng+1
          data(ng,ir)=cosr(j)*f4d(i,j,it,ms)
          draw(ng,ir)=f4d(i,j,it,ms)
        end do
        end do
      end do
      end do
...
C
      CALL ofrota(RWORK2,ngrd,ngrd,neof,norm,0,60,1.,eps,delta,
...
c       call regr_t(ts1,ts2,ifld,REVCT(ks,km),reg,undef)
        enddo
 403  continue
c
      do 100 imode=1,neof
      do 105 igpt=1,ngrd
 105  arrout(igpt)=REVCT(igpt,imode)
      irec= imode
