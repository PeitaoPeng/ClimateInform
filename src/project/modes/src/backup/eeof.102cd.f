      ...
      parameter(ngrd=102)
      parameter(nspc=ngrd*lag)
      dimension grid3(ngrd,ltime)
      dimension work(nd),data(nt,nspc),w1d(lag)
      dimension work2(nd),data2(nt,nspc)
      dimension work2d(nd,nmode)
      dimension w(lag),u(nt,nspc),v(nt,nspc),rv1(lag)
      dimension varn(lag)
...
c
      call svdeof(data,nt,nspac,w,u,v,rv1)
c
c
...
c
      ...
      do it=1,nd
        iw=iw+1
        write(60,rec=iw) work(it)
        do nm=1,8
        iw=iw+1
        write(60,rec=iw) work2d(it,nm)
        enddo
      enddo
c
      stop
...
c
      ...
      subroutine svdeof(col,ngrd,ist,ntt,w,u,v,rv1)
      dimension col(ngrd,ist,ntt),w(ntt),u(ngrd,ist,ntt),v(ngrd,ist,ntt),rv1(ntt)
      logical matu,matv
      matu=.true.
      matv=.true.
      call svd(ngrd,ist,ist,ntt,col,w,matu,u,matv,v,ierr,rv1)
      write(6,*)'ierr=',ierr
      if(ierr.ne.0)then
