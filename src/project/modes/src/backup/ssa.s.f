...
c given data to have svdeof v and svdpc u
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      subroutine svd_sub(dtin,uo,vo,ww,ntm,nt,lag)
ccccccccccccccccccccccccccccccccccccccccc
c== dtin: input data matrix
c== uo,vo: outpout of u,v,w
c== ww: % variance explained by each mode
c== ntm: max nt
ccccccccccccccccccccccccccccccccccccccccc
      dimension dtin(ntm,lag),uo(ntm,lag),vo(ntm,lag)
      dimension data(nt,lag),w1d(lag)
      dimension w(lag),u(nt,lag),v(nt,lag),rv1(lag)
      dimension ww(lag),varn(lag)
...
c
c     call vart(data,nt,lag,u,lag,totalvar,varn,w,v)
c
c-----normalize PCs
c
      do n=1,lag
...
c
      do k=1,lag
        do i=1,lag
          vo(i,k)=v(i,k)*w(k)*su(k)  !absorb std of PCs and eiganvalues to eofs
        enddo
      enddo
c
...
c
      ...
      enddo
c
      write(6,*)'total var=', totalvar, var
      write(6,*)'mode','         pct variance','      pct acc. var'
      vr=0.0
      do k=1,lag
      ww(k)=w(k)*w(k)/var
      vr=vr+ww(k)
...
c
      SUBROUTINE setundef(fld,n,m,undef)
      DIMENSION fld(n,m)
      do i=1,n
      do j=1,m
         fld(i,j)=undef
