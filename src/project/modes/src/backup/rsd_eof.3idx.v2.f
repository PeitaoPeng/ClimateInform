...
C===========================================================
      ...
      real sst(ltime),tps(ltime),amo(ltime)
      real anm(ltime) !annual mode index
...
C
      ...
      ENDDO
c
c== read in nino34, glmsst and amo indices
c
      do it=1,ltime
      ...
      write(6,*) 'amo=', amo
c
c== take residuals of nino34 and amo by subtract glm ipcc signal from them
C
...
c
      ...
      enddo
c
c== regr of obs z200 to nino34, glm_sst and amo
c
      do i=1,imx
      do j=1,jmx
        do it=1,ltime
...
c
     ...
     &-ts2(it)*regr2(i,j)-ts3(it)*regr3(i,j)
        enddo
        enddo
      enddo
c
...
c
cc regress to coef(m=1) then have residual Z200
      call eofs(aaa,ngrd,ifld,ifld,eval,evec,coef,wk)
      do it=1,ltime
        ts1(it)=coef(1,it)
      enddo
      call normal(ts1,anm,ltime)  !for residual use
      do i=1,imx
      do j=1,jmx
        do it=1,ltime
        ts1(it)=obsa(i,j,it)
        enddo
        call regr_t(anm,ts1,ltime,corr(i,j),regr(i,j))
      enddo
      enddo
      do it=1,ltime
        do i=1,imx
        do j=1,jmx
          obsa(i,j,it)=obsa(i,j,it)-anm(it)*regr(i,j)
        enddo
        enddo
      enddo
      do it=1,ltime
