...
lcdir=/cpc/home/wd52pp/project/modes/src/backup
...
mskip=0 # skiped EOF for rotation 
trd=1 # =0 detrended; =1 kept trend
mtp=1 # =0 no tp index realated; =1 kept mtp
#
undefdata=-9.99e+8
#
js=45;je=72;lats=20
...
cp $lcdir/reof.z500.dtrd.f $tmp/pure_eof.f
cp $lcdir/reof_skip.s.f  $tmp/reof.s.f
...
#for mon in  May Jun Jul Aug Sep Oct; do
for mon in  Jul; do
/bin/rm $tmp/fort.*         
#
...
#
 infile=R1.z500.jan1949-dec2021
 outfile1=reof.${eof_area}.R1_$var.$mon.mtx$mtx.yre${end_yr}.trd$trd.mtp$mtp
 outfile2=reof.${eof_area}.R1_$var.$mon.mtx$mtx.yre${end_yr}.trd$trd.mtp$mtp.glb
 outfile3=rpc.${eof_area}.R1_$var.$mon.mtx$mtx.yre${end_yr}.trd$trd.mtp$mtp
#
cat > parm.h << eof
       parameter(mons=$imon,ny=$totyr-1)
