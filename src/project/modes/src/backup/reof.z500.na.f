...
C===========================================================
      ...
      real aaa2(ngrd2,ifld),wk2(ifld,ngrd2),tt(nmod,nmod)
      real eval2(ifld),evec2(ngrd2,ifld),coef2(ifld,ifld)
...
c assign to xna4d
      ...
      do i=97,imx
      i2=i2+1
        xna4d(i2,j2,it,ms)=f4d(i,j,it,ms)
      enddo
      enddo
      ...
      j2=j2+1
      do i=2,13
        xna4d(48+i-1,j2,it,ms)=f4d(i,j,it,ms)
      enddo
      enddo
...
c EOF over NA
      ...
      ir=ir+1
        ...
        do i=1,imx2
          ng=ng+1
          aaa2(ng,ir)=xna4d(i,j,it,ms)*cosr(44+j)
        end do
        end do
      end do
      end do
      write(6,*) 'ngrd2= ',ng, ' nt=',ir
c
      call eofs(aaa2,ngrd2,ifld,ifld,eval2,evec2,coef2,wk2,id)
c
