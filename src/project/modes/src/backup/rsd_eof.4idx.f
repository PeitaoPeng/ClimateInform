...
C===========================================================
      ...
      real sst(ltime),tps(ltime),amo(ltime),pdo(ltime)
      real ts1(ltime),ts2(ltime),ts3(ltime),ts4(ltime)
...
c
      ...
      open(24,form='unformatted',access='direct',recl=4)
      open(25,form='unformatted',access='direct',recl=4)
C
...
C
      ...
      ENDDO
c
c== read in nino34, glmsst,amo,pdo indices
c
      do it=1,ltime
      read(22,rec=it) sst(it)
      read(23,rec=it) tps(it)
      read(24,rec=it) amo(it)
      read(25,rec=it) pdo(it)
      enddo
      write(6,*) 'amo=',amo
      call anom(sst,ltime)
      call anom(tps,ltime)
      ...
      write(6,*) 'pdo=', pdo
c
c== take residuals of nino34,amo and pdo by subtract glm ipcc signal from them
C
      call regr_t(tps,sst,ltime,xcor,xreg) !to see their corr
      call regr_t(tps,amo,ltime,xcor2,xreg2) !to see their corr
      call regr_t(tps,pdo,ltime,xcor3,xreg3) !to see their corr
      write(6,*) 'xcor&xreg(nino34 to glmsst)=',xcor,xreg
      write(6,*) 'xcor&xreg(amo to glmsst)=',xcor2,xreg2
      write(6,*) 'xcor&xreg(pdo to glmsst)=',xcor3,xreg3
c
      call normal(tps,ts1,ltime)
c
      do it=1,ltime
        sst(it)=sst(it)-ts1(it)*xreg
        amo(it)=amo(it)-ts1(it)*xreg2
        pdo(it)=pdo(it)-ts1(it)*xreg3
      enddo
      call regr_t(tps,sst,ltime,xcor,xreg) !to see their corr
      call regr_t(tps,amo,ltime,xcor2,xreg2) !to see their corr
      call regr_t(tps,pdo,ltime,xcor3,xreg3) !to see their corr
c
      write(6,*) 'after residual'
      write(6,*) 'xcor&xreg(nino34 to glmsst)=',xcor,xreg
      write(6,*) 'xcor&xreg(amo to glmsst)=',xcor2,xreg2
      write(6,*) 'xcor&xreg(pdo to glmsst)=',xcor3,xreg3
c
...
c
      ...
      enddo
c
c== regr of obs z200 to nino34, glm_sst, amo and pdo
c
      do i=1,imx
      do j=1,jmx
        do it=1,ltime
        ...
        call regr_t(amo,ts1,ltime,corr3(i,j),regr3(i,j))
        call regr_t(pdo,ts1,ltime,corr4(i,j),regr4(i,j))
      enddo
      enddo
c
c== have residual
c
      ...
      call normal(amo,ts3,ltime)  !for residual use
      call normal(pdo,ts4,ltime)  !for residual use
c
      do it=1,ltime
        do i=1,imx
        do j=1,jmx
          obsa(i,j,it)=obsa(i,j,it)-ts1(it)*regr(i,j)
     &-ts2(it)*regr2(i,j)-ts3(it)*regr3(i,j)-ts4(it)*regr4(i,j)
        enddo
        enddo
      enddo
c
...
c
      do m=1,nmod
      do it=1,ltime
        tcof(m+4,it)=rcoef(m,it)
c       tcof(m+4,it)=coef(m,it)
      enddo
      enddo
c
