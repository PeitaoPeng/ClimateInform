...
var=z500
end_yr=2000
if [ ${end_yr} == 2020 ]; then totyr=71; fi
if [ ${end_yr} == 2000 ]; then totyr=51; fi
ntotm=`expr $totyr \* 12`
ltime=$totyr
...
eof_area=NH
#eof_area=TNH
mtx=1 # =1 for corr matrix; =2 for var matrix
#
undefdata=-9.99e+8
#
...
cp $lcdir/pure_eof.z500.cpc.1mon.f $tmp/pure_eof.f
...
#outfile1=reof.${eof_area}.core_$var.$mon.mtx$mtx
#outfile2=rpc.${eof_area}.core_$var.$mon.mtx$mtx
#outfile3=clm_std.${eof_area}.core_$var.$mon.mtx$mtx
#
 infile=R1.z500.jan1950-dec2020
 outfile1=reof.${eof_area}.R1_$var.$mon.mtx$mtx.yre${end_yr}.1mon
 outfile2=rpc.${eof_area}.R1_$var.$mon.mtx$mtx.yre${end_yr}.1mon
 outfile3=clm_std.${eof_area}.R1_$var.$mon.mtx$mtx.yre${end_yr}.1mon
#
cat > parm.h << eof
