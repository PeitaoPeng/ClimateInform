...
c
c normalize and cosr weighting
      do i=1,imx
      do j=1,jmx
        do it=1,ny
          ts1(it)=f3d(i,j,it)
        ...
        if(mtx.eq.1) then
        do it=1,ny
          f4d(i,j,it,ms)=ts2(it)*cosr(j) ! normalized
        enddo
        else
        do it=1,ny
          f4d(i,j,it,ms)=ts1(it)*cosr(j) ! raw
        enddo
        endif
      enddo
      enddo
...
c
      ...
      iw2=iw2+1
      call noreof2d(regr,imx,jmx,is,ie,js,je)
      write(51,rec=iw2) regr
      iw2=iw2+1
      call noreof2d(regr2,imx,jmx,is,ie,js,je)
