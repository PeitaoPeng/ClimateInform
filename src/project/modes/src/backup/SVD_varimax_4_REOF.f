...
c  a space-time array with eigenvectors and PCs rotated according
c  to the varimax criterion.  Varimax subroutine originally from
c  EOF/PC code used at NASA GSFC.
c
c  **NOTE: DO NOT use singular values to compute explained variance
c  of EOFs if using varimax rotation!  Compute explained variance of
c  the rotated mode by calculating the variance of the raw PCs and dividing
c  by the sum of the variance of all raw PCs
c
...
c     Raw PCs (timedim x timedim): Unnormalized Rotated PCs
c
c

c  Specify space and time dimensions
c
c  maxdim is the larger of timedim or spacedim
c  mindim is the smaller of timedim or spacedim
c  rotations is the number of eigenvector rotations
c
      parameter(timedim=60)
      parameter(spacedim=1000)
      parameter(mindim=60)
      parameter(maxdim=1000)
      parameter(rotations=10)
...
c       i,j,l,pass = counters
c       
c    Arrays 
c
c       at = original data in format space-time
c       a = original data in format time-space
c       v = V array (**Note it's not VT) 
c       w = Sigma array: Singular values
c       rawPC = Unnormalized PCs in format time-time
c      
c       temp arrays used for data sorting
      ...
      real w(maxdim)
      real rawPC(timedim,timedim),sumPC
      real temp1,temp2(timedim),temp3(spacedim)


c Input and output files


      open (10,file='Atarray.bin',
     + form='unformatted', access='direct', 
     + recl=4*spacedim*timedim)

      open (11,file='Uarray_rot.bin',
     + form='unformatted', access='direct', 
     + recl=4*timedim*timedim)

      open (12,file='Varray_rot.bin',
     + form='unformatted', access='direct', 
     ...
     + form='formatted')
      ...
      open (14,file='PCsraw_rot.bin',
     + form='unformatted', access='direct', 
     + recl=4*timedim*timedim)    



c Read in data (AT array)
...
c Output U and V arrays, Singular values, Raw PCs
      ...
      write(14,rec=1) ((rawPC(j,l),j=1,timedim),l=1,timedim)
...
C  ON OUTPUT, X CONTAINS THE ROTATED SYSTEM.
      ...
      REAL SUMTERM,SUMTERM2,SUMTERM3
...
C Varimax iteration for all pairs of vectors
      ...
      do L=1,MAXITER
        ...
        if((VVNEW-VVOLD)/VVNEW.lt.(0.0001)) goto 50
          
        if(L.eq.MAXITER) then
          print*, 'VARIMAX rotation iteration did not coverge'
          goto 50
        endif
