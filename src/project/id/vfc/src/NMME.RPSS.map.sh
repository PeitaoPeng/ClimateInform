...
##########################################################
# calculate RPSS on time domain, output is a map
##########################################################
set -eaux
...
outfile1=RPSS.NMME.$var.$otmean.${icmon}_ic.2012-$yre
...
c
      ...
      do j=1,jm
        ...
        enddo
        rps_t(i,j,il)=exp/float(nvy)
        rpsc_t(i,j,il)=expc/float(nvy)
        rpss_t(i,j,il)=1.- rps_t(i,j,il)/rpsc_t(i,j,il)
      END IF
      enddo
      enddo
      ...
      w2d(i,j)=rpss_t(i,j,il)

      IF(abs(rpss_t(i,j,il)).gt.999) then
      w2d(i,j)=undef
      END IF
