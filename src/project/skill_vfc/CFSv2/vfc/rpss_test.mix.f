...
C   Mont Carl test for RPSS by mixing model identity
C==========================================================
      include "parm.h"
...
c
      ...
      dimension ran2(nss),idx2(nss),rdx2(nss)
c
...
c have random #
      ...
      enddo
c
c randomly shuffle 6-month block data between two models
      lb=5 !length of block
      nblk=nss/lb  !total block #
c for the 1st model
      it=0
      do ib=1,nblk
      if(ran(ib).gt.0.5) then
      nv11=nv11+1
        do itb=1,lb
        it=it+1
c       print*, 'ib=',ib,'itb=',itb,'it=',it
        do i=1,imx
        do j=1,jmx
        w3d1(i,j,it)=prd_b(i,j,it)
        w3d2(i,j,it)=prd_n(i,j,it)
        enddo
        enddo
        enddo
      else
...
c       print*, 'ib=',ib,'itb=',itb,'it=',it
        do i=1,imx
        do j=1,jmx
        w3d1(i,j,it)=prd_b2(i,j,it)
        w3d2(i,j,it)=prd_n2(i,j,it)
        enddo
        enddo
        enddo
      endif
...
c for the 2nd model
      it=0
      do ib=1,nblk
      if(ran2(ib).gt.0.5) then
      nv21=nv21+1
        do itb=1,lb
        it=it+1
        do i=1,imx
        do j=1,jmx
        w3d3(i,j,it)=prd_b2(i,j,it)
        w3d4(i,j,it)=prd_n2(i,j,it)
        enddo
        enddo
        enddo
      else
      nv22=nv22+1
        ...
        do i=1,imx
        do j=1,jmx
        w3d3(i,j,it)=prd_b(i,j,it)
        w3d4(i,j,it)=prd_n(i,j,it)
        enddo
        enddo
        enddo
      endif
...
c
      ...
      END DO           !loop nsp
      print*, 'nv11,nv12,nv21,nv22=',nv11,nv12,nv21,nv22 
