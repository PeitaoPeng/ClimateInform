...
cc and calculate lagged HEIDKE skill time series
      do jt=0,maxlag
        ...
        avghs5(jt+1)=0.
        avghs6(jt+1)=0.
       do it=2,lt  !starting from 2nd record, i.e., feb 1995
        do i=1,imx
        do j=1,jmx
        ...
        call heidke1(mask,mwk2,mwk3,coslat,hs3,ntot1,imx,jmx)
        call heidke2(mask,mwk2,mwk3,coslat,hs4,ntot2,imx,jmx)
        call heidke1(mask,mwk1,mwk4,coslat,hs5,ntot1,imx,jmx)
        call heidke2(mask,mwk1,mwk4,coslat,hs6,ntot2,imx,jmx)
c       write(6,*) 'hs5=',hs5,'  hs6=',hs6
          avghs1(jt+1)=avghs1(jt+1)+hs1/float(lt-1)  !(nss-1): no data for it=1 
          avghs2(jt+1)=avghs2(jt+1)+hs2/float(lt-1)
          avghs3(jt+1)=avghs3(jt+1)+hs3/float(lt-1)  !(nss-1): no data for it=1 
          avghs4(jt+1)=avghs4(jt+1)+hs4/float(lt-1)
          avghs5(jt+1)=avghs5(jt+1)+hs5/float(lt-1)  !(nss-1): no data for it=1 
          avghs6(jt+1)=avghs6(jt+1)+hs6/float(lt-1)
       enddo
      enddo
c
 888  format(10f7.2)
