...
c
      ...
      open(unit=54,form='unformatted',access='direct',recl=4)  !seasonal cycle of hdk
...
c= read in prd data
      do it=1,nss
        read(12,rec=it) wk1
        call change_numb(wk1,mwk1,imx,jmx)  !convert prd to -1,0,+1,9
        do i=1,imx
        do j=1,jmx
...
cc and calculate HEIDKE skill time series
      ...
      do it=1,nss
        ...
        call hss_s_MC(mask,mwk1,mwk2,coslat,imx,jmx,nsp,hsmc1,hsmc2)
          ...
          write(6,999) it,hs2,hs1,ntot1,ntot2,xnonec(it)
c
...
c123  write(6,*) 'before seasonal cycle'
c
...
c
      ...
      do k=1,232
      iw=iw+1
      write(54,rec=iw) fld1d1(k)
      iw=iw+1
      write(54,rec=iw) fld1d2(k)
      enddo
c
...
cc lagged corr of ldk skill
      ...
      write(6,*)'lagcor of nonec grid=',corlag
c
      stop
      end
...
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
      subroutine change_numb(wk,mw,m,n)
      dimension wk(m,n),mw(m,n)
c
      do i=1,m
      do j=1,n
      if(wk(i,j).lt.0.1) mw(i,j)=9
      if(wk(i,j).gt.0.9.and.wk(i,j).lt.1.1) mw(i,j)=-1
      if(wk(i,j).gt.1.9.and.wk(i,j).lt.2.1) mw(i,j)=0
      if(wk(i,j).gt.2.9) mw(i,j)=1
      end do
      end do
c
      return
