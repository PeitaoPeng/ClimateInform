...
C==========================================================
      ...
      dimension ev1(nx),ev2(nx)
      real ny_a,ny_b,nn_a,nn_b  !ny_a(b): prd is not above(below)but obs is;
      real na,nn,nb
      real na_t,nn_t,nb_t
c
...
c
     ...
     &aa,an,ab,na,nn,nb,ba,bn,bb,imx,jmx,anmb)
      ...
      bb_t=bb_t/tot
c
      write(6,888)aa_t,an_t,ab_t
      write(6,888)na_t,nn_t,nb_t
      write(6,888)ba_t,bn_t,bb_t

c
      yy_a=aa_t
      ny_a=na_t+ba_t
      yn_a=an_t+ab_t
      nn_a=bb_t+bn_t+nb_t+nn_t
c
      yy_b=bb_t
      ny_b=ab_t+nb_t
      yn_b=bn_t+ba_t
      nn_b=aa_t+an_t+na_t+nn_t

      write(6,*) 'anmb_tot=',anmb_t
      write(6,*) 'tot=',tot
      write(6,777)yy_a,yn_a
      write(6,777)ny_a,nn_a
      write(6,*)
      write(6,777)yy_b,yn_b
      write(6,777)ny_b,nn_b
c
      write(6,*) 'anmb_tot=',anmb_t
...
cc economy value for above normal
      ...
      c=yn_a
      d=yy_a
      write(6,*)'a,b,c,d for above normal='
      write(6,777)a,b
      write(6,777)c,d
      obar=b+d
      H=d/obar
      ...
      alf=alf+1./float(nx)
      if (alf.lt.obar) then
        ev1(i)=(1-F)-(obar/(1-obar))*((1-alf)/alf)*(1-H)
      else
        ev1(i)=H-((1-obar)/obar)*(alf/(1-alf))*F
      endif
c     write(6,*)'alf=',alf
      enddo
      write(6,666) ev1
cc economy value for below normal
      ...
      c=yn_b
      d=yy_b
      write(6,*)'a,b,c,d for below normal='
      write(6,777)a,b
      write(6,777)c,d
      obar=b+d
      H=d/obar
      ...
      alf=alf+1./float(nx)
      if (alf.lt.obar) then
        ev2(i)=(1-F)-(obar/(1-obar))*((1-alf)/alf)*(1-H)
      else
        ev2(i)=H-((1-obar)/obar)*(alf/(1-alf))*F
      endif
c     write(6,*)'alf=',alf
      enddo
      write(6,666) ev2
cc write out economy value
      ...
      do i=1,nx
      iw=iw+1
      write(50,rec=iw) ev1(i)
      iw=iw+1
      write(50,rec=iw) ev2(i)
      enddo
...
c
      ...
      if(mw2(i,j).eq.ic1) anmb=anmb+coslat(j)
      ...
      if(mw2(i,j).eq.ic1) aa=aa+coslat(j)/3.
      if(mw2(i,j).eq.ic2) an=an+coslat(j)/3.
      if(mw2(i,j).eq.ic3) ab=ab+coslat(j)/3.
      if(mw2(i,j).eq.ic1) na=na+coslat(j)/3.
      if(mw2(i,j).eq.ic2) nn=nn+coslat(j)/3.
      if(mw2(i,j).eq.ic3) nb=nb+coslat(j)/3.
      if(mw2(i,j).eq.ic1) ba=ba+coslat(j)/3.
      if(mw2(i,j).eq.ic2) bn=bn+coslat(j)/3.
      if(mw2(i,j).eq.ic3) bb=bb+coslat(j)/3.
