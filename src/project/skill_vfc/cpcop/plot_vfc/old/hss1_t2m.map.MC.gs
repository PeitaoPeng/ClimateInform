...
'draw string 5.5 8.25 HSS1 of CPC Seasonal Temp Fcst & MC Test'
...
*----------
 if(iframe = 1); 'draw string 'titlx' 'titly' Official HSS1'; endif
 if(iframe = 2); 'draw string 'titlx' 'titly' Mean HSS1 in MC Test'; endif
 if(iframe = 4); 'draw string 'titlx' 'titly' Official HSS1 - Mean HSS1 in MC Test'; endif
 if(iframe = 3); 'draw string 'titlx' 'titly' STDV of HSS1 in MC Test'; endif
*
  iframe=iframe+1
'run /export/hobbes/wd52pp/bin/cbarn2.gs 0.6 0 'barx' 'bary''
endwhile
'print'
'printim hs1_temp.map.MC.png gif x800 y600'
*----------
