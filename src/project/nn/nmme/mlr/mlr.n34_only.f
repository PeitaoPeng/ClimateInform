      ...
      open(unit=10,form='unformatted',access='direct',recl=4*ngrd)
      open(unit=20,form='unformatted',access='direct',recl=4)
C
      open(unit=30,form='unformatted',access='direct',recl=4)
c*************************************************
C read nmme & obs nino34
      do it=1,nt
        read(10,rec=it) wk(ngrd,it)
...
C write out
      ...
      do it=1,nt
        iw=iw+1
        write(30,rec=iw) on34(it)
        iw=iw+1
        write(30,rec=iw) fn34(it)
      enddo
      write(6,*) 'n34=',on34
...
C add a ridge
      ...
      enddo
      dg=sqrt(dg/float(m))
      write(6,*) 'rms of diagnal elements',dg
