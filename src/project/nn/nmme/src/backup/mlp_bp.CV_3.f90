...
INTEGER :: it, iy, itgt, itgtm, itgtp
...
!-------------------------------------
    ...
    ! arrange data for CV
      DO itgt = 1, giFILEROWS
        ...
        itgtm=itgt-1
        itgtp=itgt+1
        if(itgt.eq.1) itgtm=3
        if(itgt.eq.giFILEROWS) itgtp=giFILEROWS-2
        ...
        if(iy == itgt)  go to 555
        if(iy == itgtm) go to 555
        if(iy == itgtp) go to 555
 ...
 555  continue
        ...
        garDataArray(giFILEROWS-2,1:giNDU) = garDataArray2(itgt,1:giNDU)
        garDataArray(giFILEROWS-1,1:giNDU) = garDataArray2(itgt,1:giNDU)
