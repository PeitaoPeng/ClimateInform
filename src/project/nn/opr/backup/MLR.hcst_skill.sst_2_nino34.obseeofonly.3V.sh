...
# obs_sst_d20 are represented with eeof
#=========================================================
...
##===============================
## have eof-pc input for var2&var3
##===============================
#
cp $lcdir/grd_2_eeof_4_ic.2V.f input.f
cp $lcdir/reof.s.f reof.s.f
#
iskip=3
...
cat > parm.h << eof
       ...
       parameter(ngrd=$ngrdobs)
       parameter(nmod=$nmodobs,id=0)
eof
#
/bin/rm fort.*
#
...
#
 ...
 ln -s $datadir3/obs.$var3.$icmonw.1979-2020.anom.gr fort.11
 ln -s input2.gr   fort.30
 ln -s eof.$var2.gr fort.40
 ln -s eof.$var3.gr fort.41
#
 input.x
##===================================
...
cat > comb_input.f << eof
      program combing
      parameter(nt=$nyr)
      parameter(nmod1=$nmod1,nmod2=$nmodobs)
      parameter(nfld1=nmod1+1)
      parameter(nfld=$nfld)
c
      dimension v1in(nfld1),v2in(nmod2)
      dimension out(nfld)
c
...
mv acrms.$nlead $datadir2/skill.$tool.$varn.${var1}_${var2}_${var3}_2_nino34.$tp.${icmonw}_ic.1982-$lastyr.ld1-$nlead.nmod${nmod1}_$nmodobs.obs_eeof.gr

cat>$datadir2/skill.$tool.$varn.${var1}_${var2}_${var3}_2_nino34.$tp.${icmonw}_ic.1982-$lastyr.ld1-$nlead.nmod${nmod1}_$nmodobs.obs_eeof.ctl<<EOF
dset ^skill.$tool.$varn.${var1}_${var2}_${var3}_2_nino34.$tp.${icmonw}_ic.1982-$lastyr.ld1-$nlead.nmod${nmod1}_$nmodobs.obs_eeof.gr
undef -9.99e+33
*
TITLE model
*
