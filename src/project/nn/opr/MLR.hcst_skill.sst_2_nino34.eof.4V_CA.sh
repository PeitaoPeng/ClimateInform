...
#=========================================================
# Hybrid_MLR combing CFSv2 and CA_sst and obs_d20 for hcst skill of NINO3.4 
#=========================================================
...
datadir4=/cpc/consistency/id/ca_hcst/seasonal
#
cd $tmp
# 
#for curmo in 05; do
...
nmod4=3
#
nmod=`expr $nmod1 + $nmod2 + $nmod3 + $nmod4` 
...
ngrd=1073
#
# have input data to ANN or MLR package =======================================
#
imon=$curmo
...
C
      ...
      open(unit=11,form='unformatted',access='direct',recl=4*imx*jmx)
C
      open(unit=30,form='unformatted',access='direct',recl=4*nmod)
      open(unit=40,form='unformatted',access='direct',recl=4*imx*jmx)
c*************************************************
C
C== have coslat
...
C read ca sst
      ...
      do it=1,nt
        ...
        read(20,rec=ir) w2d       !ca hcst sst
...
/bin/rm fort.*
#
 gfortran -o input.x reof.s.f input.ca.f
#gfortran -mcmodel=medium -g -o test_data.x reof.s.f test_data.f
#
 ln -s $datadir1/obs.nino34.$tp.${icmonw}_ic.1982-cur.ld1-$nlead.gr fort.10
 ln -s /cpc/home/wd52pp/project/nn/nmme/data_proc/land.2.5x2.5.gr   fort.11
 ln -s $datadir4/cahcst.all_esm.sst.${icmonw}_ic.ld1-14.2.5x2.5.gr fort.20
...
#gfortran -mcmodel=medium -g -o test_data.x reof.s.f test_data.f
#
 ln -s $datadir3/obs.$var4.$icmonw.1979-2020.anom.gr fort.10
 ln -s input4.gr   fort.30
 ln -s eof.gr fort.40
#
 input.x
##===================================
...
cat > comb_input.f << eof
      program combing
      parameter(nt=$nyr)
      parameter(nmod1=$nmod1,nmod2=$nmod2,nmod3=$nmod3,nmod4=$nmod4)
      parameter(nfld1=nmod1+1)
      parameter(nfld=$nfld)
c
      dimension v1in(nfld1),v2in(nmod2),v3in(nmod3),v4in(nmod4)
      dimension out(nfld)
c
      ...
      open(unit=14,form='unformatted',access='direct',recl=4*nmod4)
...
c
      do it=1,nt
        ...
        enddo
        do i=1,nmod4
         out(i+nmod1+nmod2+nmod3)=v4in(i)
        enddo
         out(nfld)=v1in(nfld1)
        write(20,rec=it) out
      enddo !it loop
...
mv acrms.$nlead $datadir2/skill.$tool.$varn.${var1}_${var2}_${var3}_${var4}_2_nino34.$tp.${icmonw}_ic.1982-$lastyr.ld1-$nlead.nmod${nmod1}_${nmod2}_${nmod3}_${nmod4}.gr

cat>$datadir2/skill.$tool.$varn.${var1}_${var2}_${var3}_${var4}_2_nino34.$tp.${icmonw}_ic.1982-$lastyr.ld1-$nlead.nmod${nmod1}_${nmod2}_${nmod3}_${nmod4}.ctl<<EOF
dset ^skill.$tool.$varn.${var1}_${var2}_${var3}_${var4}_2_nino34.$tp.${icmonw}_ic.1982-$lastyr.ld1-$nlead.nmod${nmod1}_${nmod2}_${nmod3}_${var4}.gr
undef -9.99e+33
*
TITLE model
*
