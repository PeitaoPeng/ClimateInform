...
C
      ...
      real corr4(imx,jmx),regr4(imx,jmx)
C
...
C
      ...
      open(unit=43,form='unformatted',access='direct',recl=4*imx*jmx)
c*************************************************
C
C== have coslat
C
...
C read obs sst and d20
      ...
      do it=1,nt
        ...
        read(23,rec=ir) w2d3
          ...
          do i=1,imx
          do j=1,jmx
            w3d3(i,j,it)=w2d2(i,j)
            w3d4(i,j,it)=w2d3(i,j)
          enddo
          enddo
...
c
      do i=is,ie
      do j=js,je
        if(abs(w3ds4(i,j,it)).lt.500) then
        ig=ig+1
        aaa(ig,it)=w3ds4(i,j,it)*cosr(j)
        endif
      enddo
      enddo
      print *, 'ngrd=',ig
...
c
        if(abs(w2d4(i,j)).lt.1000) then
        do it=1,nt
        ts6(it)=w3d4(i,j,it)
        enddo
        call regr_t(ts2,ts6,nt,corr4(i,j),regr4(i,j))
        else
        corr4(i,j)= -9.99E+8
        regr4(i,j)= -9.99E+8
        endif
...
\rm fort.*
 ...
 ln -s $datadir4/cahcst.all_esm.sst.${icmonw}_ic.ld1-14.2.5x2.5.gr fort.21
 ln -s $datadir3/obs.$var3.$icmonw.1979-2020.anom.gr           fort.22
 ln -s $datadir3/obs.$var4.$icmonw.1979-2020.anom.gr           fort.23
 ln -s input.gr                                                fort.30
 ln -s eeof.$var1.gr                                                  fort.40
 ln -s eeof.$var2.gr                                                  fort.41
 ln -s eeof.$var3.gr                                                  fort.42
 ln -s eeof.$var4.gr                                                  fort.43
#
 input.x
#
##===================================
...
mv acrms.$nlead $datadir2/skill.$tool.$varn.${var1}_${var2}_${var3}_${var4}_2_nino34.$tp.${icmonw}_ic.1982-$lastyr.ld1-$nlead.nmod$nmod.eeof.gr

cat>$datadir2/skill.$tool.$varn.${var1}_${var2}_${var3}_${var4}_2_nino34.$tp.${icmonw}_ic.1982-$lastyr.ld1-$nlead.nmod$nmod.eeof.ctl<<EOF
dset ^skill.$tool.$varn.${var1}_${var2}_${var3}_${var4}_2_nino34.$tp.${icmonw}_ic.1982-$lastyr.ld1-$nlead.nmod$nmod.eeof.gr
undef -9.99e+33
*
TITLE model
*
