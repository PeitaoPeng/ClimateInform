...
!! include parameters assigned in script file
      INTEGER :: nt, nfld, ngrd, nld, imx, jmx
      INTEGER :: is, ie, js, je
      REAL :: undef
      include "parm.h"
...
INTEGER, PARAMETER :: giUNIT_TRAIN = 20  !! prd in train
INTEGER, PARAMETER :: giUNIT_TEST = 30   !! prd in test for a single grid
INTEGER, PARAMETER :: giUNIT_VAL = 40    !! prd in validation
...
!
! prepare input data
      INTEGER :: i, j, ir, iro, ig
      INTEGER :: itg, jtg
      REAL :: on34(nt)
      REAL :: out(nfld)
      REAL wk(ngrd,nt),w2d(imx,jmx),w2d2(imx,jmx),w2d3(imx,jmx)
      REAL w3df(imx,jmx,nt),w3do(imx,jmx,nt)
      REAL wout(imx,jmx,nt)
      REAL xx,yy
...
!
      ...
      open(unit=52,form='unformatted',access='direct',recl=4*imx*jmx)

       OPEN(UNIT=10, form='unformatted',access='direct',recl=4*nfld)

! read nmme & obs sst anom

      ir=nld ! for djf data only
      iro=2*nld ! because obs is fron 1981
      do it=1,nt

        read(15,rec=ir) w2d
        read(25,rec=iro) w2d2

          do i=1,imx
          do j=1,jmx
            w3df(i,j,it)=w2d(i,j)
            w3do(i,j,it)=w2d2(i,j)
          enddo
          enddo
...
!===========================================
  ...
  ! mlp for each grid in the interested area  

      do itg=is,ie ! i loop over target grid
      do jtg=js,je ! j loop over target grid

      IF(abs(w2d(itg,jtg)).gt.1000.or.abs(w2d2(itg,jtg)).gt.1000) then

        do it=1,nt
          wout(itg,jtg,it)=undef
        enddo
     ...
     !  do j=js,je
        do i=is,ie,4
        do j=js,je,2
          if(abs(w3df(i,j,it)).lt.1000) then
          ig=ig+1
          wk(ig,it)=w3df(i,j,it)
          endif
        enddo
        enddo
        print *, 'ngrd=',ig
...
! write out Model SST and obs SST for grid (i,j)
      do it=1,nt
        do i=1,ngrd
          out(i)=wk(i,it)
        enddo
          out(nfld)=w3do(itg,jtg,it)

        write(10,rec=it) out
      enddo

    ! read unit 10 as input
    ...
    ! write out model, obs, and mlp
      do it=1,nt
        do i=1,imx
        do j=1,jmx
          w2d(i,j)=w3df(i,j,it)
          w2d2(i,j)=w3do(i,j,it)
          w2d3(i,j)=wout(i,j,it)
        enddo
        enddo
        write(50,rec=it) w2d
        write(51,rec=it) w2d2
        write(52,rec=it) w2d3
      enddo
