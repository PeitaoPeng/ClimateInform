...
neurons=25
#
nmod=`expr $nmod1 + $nmod2 + $nmod3` # +1 for putting obs nino34
...
c
      ...
      open(unit=24,form='unformatted',access='direct',recl=4*imx*jmx)
c
      ir1=itgtbgn
      ir2=itgtbgn-ld
      do it=1,$nyr
        ...
        read(14,rec=ir2) w2d3

        write(21,rec=it) xn
        write(22,rec=it) w2d
        write(23,rec=it) w2d2
        write(24,rec=it) w2d3
...
cat > parm.h << eof
       ...
       parameter(imx=$imx,jmx=$jmx)
       parameter(is=49,ie=121,js=29,je=45) !120E-300E; 20S-20N
       parameter(ngrd=$ngrd3,nfld=$nmod3+1)
...
#gfortran -mcmodel=medium -g -o test_data.x reof.s.f test_data.f
#
 ln -s obs.$var3.gr fort.10
 ln -s obs.n34.gr fort.20
 ln -s input.$var3.gr   fort.30
 ln -s eof.gr fort.40
#
 input.x
#
...
cat > comb_input.f << eof
      ...
      parameter(nfld1=$nmod1+1,nfld2=$nmod2+1,nfld3=$nmod3+1)
      parameter(nfld=$nfld)
c
      dimension v1in(nfld1),v2in(nfld2),v3in(nfld3)
      dimension out(nfld)
c
      open(unit=11,form='unformatted',access='direct',recl=4*nfld1)
      open(unit=12,form='unformatted',access='direct',recl=4*nfld2)
      open(unit=13,form='unformatted',access='direct',recl=4*nfld3)
...
#
mv ac.12 $datadir/skill.$model.3V.${var1}_${var2}_${var3}_2_nino34.nmod${nmod1}_${nmod2}_${nmod3}.neurons$neurons.gr

cat>$datadir/skill.$model.3V.${var1}_${var2}_${var3}_2_nino34.nmod${nmod1}_${nmod2}_${nmod3}.neurons$neurons.ctl<<EOF
dset ^skill.$model.3V.${var1}_${var2}_${var3}_2_nino34.nmod${nmod1}_${nmod2}_${nmod3}.neurons$neurons.gr
undef -9.99e+33
*
TITLE model
*
