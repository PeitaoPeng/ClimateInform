...
! prepare input data
      ...
      REAL :: out(nfld)
      REAL ts(nt)
      REAL coeff(nmodm,nt),coefo(nmodo,nt)
      REAL wout(nfld,nt)
      REAL xx,yy
      open(unit=15,form='unformatted',access='direct',recl=4*nt)
      open(unit=25,form='unformatted',access='direct',recl=4*nt)
!
      open(unit=50,form='unformatted',access='direct',recl=4*nt)
...
!===========================================
  ...
  ! mlp for each mode of obs 

      do mtg=1,nmodo ! m loop over target mode

        do it=1,nt
          do m=1,nmodm
          out(m)=coeff(m,it)
          enddo
          out(nfld)=coefo(mtg,it)
        write(10,rec=it) out
        enddo
