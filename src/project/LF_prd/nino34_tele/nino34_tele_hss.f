...
c== seasonal anomalies
      ...
      call season_anom(sstin,ltime+1,iseason,1,sst1)
c     write(6,*) 'nino34=',sst1
c== anomalies wrt wmo clim for verification later
      call anom_wmo(grid,wmoobs,imax,ltime,ntprd,ndec)
C
C== regress nino34 to temp
c
      do it=31,ltime  !31=1961;
C
c== detrend nino34 index
      call ltrend(sst1,sst2,trend,1,ltime,1,it-1,a,b)
...
c       prd2d(i,it)=sst3(it)*regr(i,ntprd-18)
      enddo
c
