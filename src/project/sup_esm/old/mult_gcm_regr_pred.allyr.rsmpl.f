...
C. weights are spatial dependent & resampling is performed in this version
C===========================================================
      PARAMETER (imax=72,jmax=37,nyr=45,nyin=44,nyin2=nyin-1)
      ...
      real obs(nyin2),gcm(4,nyin2),beta(4)
      ...
      real*8 gg(4,4),gmgm(4,4),obgm(4),og(4)
      real*8 gmgminv(4,4)
      real   gm(4)
...
c     do i=lonpna1,lonpna2
c     do j=latpna1,latpna2
      do i=1,imax
      do j=1,jmax
      ...
      DO kt=1,nyin   !resampling
         kc=0
         do k=1,nyin
           if(k.ne.kt) then
         kc=kc+1
         obs(kc)=obs3d(i,j,k)
         gcm(1,kc)=ib9x*b9x3d(i,j,k)
         gcm(2,kc)=iecham*echam3d(i,j,k)
         gcm(3,kc)=iccm3*ccm33d(i,j,k)
         gcm(4,kc)=igfdl*gfdl3d(i,j,k)
           end if
         end do
         call cmatrix_dd(gcm,obs,gg,og,4,nyin2)
         call add_2d_dd(gmgm,gg,4,4)
         call add_1d_dd(obgm,og,4)

C        call RLSE(nyin,obs,4,gcm,nyin,1,beta,wk3(i,j),wk4(i,j))
C        call mul_regression(obs,gcm,beta,xmean,1,nyin2,4)

      END DO   !resampling

         call avg_2d_dd(gmgm,4,4,nyin2)
         call avg_1d_dd(obgm,4,nyin2)
c        write(6,*)'matrix=',obgm
         call mtrx_inv(gmgm,gmgminv,4)
         call solutn(gmgminv,obgm,beta,4)
         call have_mean(gcm,gm,obs,beta,xmean,4,nyin2)
...
999   format(2x,I2,2x,6(f7.2,2x))
      ...
      SUBROUTINE add_2d_dd(fld1,fld2,n,m)
      real*8 fld1(n,m),fld2(n,m)
      do i=1,n
      do j=1,m
      ...
      SUBROUTINE avg_2d_dd(fld,n,m,k)
      real*8 fld(n,m)
      do i=1,n
      do j=1,m
         fld(i,j)=fld(i,j)/float(k)
      enddo
      enddo
      return
      end

      SUBROUTINE avg_1d_dd(fld,m,k)
      real*8 fld(m)
      do j=1,m
         fld(j)=fld(j)/float(k)
      enddo
      ...
      SUBROUTINE add_1d_dd(fld1,fld2,m)
      real*8 fld1(m),fld2(m)
      do j=1,m
         fld1(j)=fld1(j)+fld2(j)
      enddo
      return
      end
...
c       acc(i,j)=acc(i,j)*(sd1(i,j)/sd2(i,j))
      ...
      SUBROUTINE cmatrix_dd(f,v,ff,vf,m,it)
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
C   construct matrix
C   f(m,it): m-th model fcst at time t
C   v(it):  verification at time t
C   ff(m,m): output matrix
C   vf(m): output matrix
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
      real*8 ff(m,m),vf(m)
      real f(m,it), v(it)
      ...
      do j=1,m
        ff(i,j)=0.
        do k=1,it
          ff(i,j)=ff(i,j)+f(i,k)*f(j,k)/float(it)
        end do
      end do
      end do
...
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
      ...
      SUBROUTINE have_mean(f,fm,v,b,xm,n,m)
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
      real f(n,m),v(m),b(n)
      real fm(n)
