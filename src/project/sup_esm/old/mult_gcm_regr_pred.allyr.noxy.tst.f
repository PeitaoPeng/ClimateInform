...
c            obs2d(i,j)=obs2d(i,j)-stclim1
c            b9x2d(i,j)=b9x2d(i,j)-stclim2
c            echam2d(i,j)=echam2d(i,j)-stclim3
c            ccm32d(i,j)=ccm32d(i,j)-stclim4
c            gfdl2d(i,j)=gfdl2d(i,j)-stclim5
       end do
       end do
...
c       acc(i,j)=acc(i,j)*(sd1(i,j)/sd2(i,j))
      ...
      do  j=1,lat
      do  i=1,lon
c       fld(i,j,k)=fld(i,j,k)-clim(i,j)
      end do
      end do
      end do
      return
