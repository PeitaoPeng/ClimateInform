...
C===========================================================
      ...
      real obs(nyin),gcm(nyin,nmodel),beta(nmodel+1)
...
C=== regression for each grid
      ...
      do j=1,jmax
         ...
         obs(k)=obs3d(i,j,k)
         gcm(k,1)=b9x3d(i,j,k)
c        gcm(k,2)=echam3d(i,j,k)
c        gcm(k,3)=ccm33d(i,j,k)
c        gcm(k,4)=gfdl3d(i,j,k)
         end do

         call RLSE(nyin,obs,nmodel,gcm,nyin,1,beta,wk3(i,j),wk4(i,j))
