...
C=== regression for each grid
      ...
      do j=1,jmax
         ...
         do k=1,nyin
         obs(k)=obs3d(i,j,k)
         gcm(1,k)=ib9x*b9x3d(i,j,k)
         gcm(2,k)=iecham*echam3d(i,j,k)
         gcm(3,k)=iccm3*ccm33d(i,j,k)
         gcm(4,k)=igfdl*gfdl3d(i,j,k)
         end do

C        call RLSE(nyin,obs,4,gcm,nyin,1,beta,wk3(i,j),wk4(i,j))
         call mul_regression(obs,gcm,beta,xmean,1,nyin,4)
