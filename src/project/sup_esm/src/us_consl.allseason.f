...
C===========================================================
      PARAMETER (mdvs=102,nyr=22*4)
      PARAMETER (nyin=nyr-12,nmodel=3,nfcst=nmodel+2)
...
C=== read in obs nad model data
 ...
 444  format(10f6.1)
C
C== filter out trend and normalize data
      do im=1,nmodel+1
        do id=1,mdvs
        do iy=1,nyr
...
c
      ...
      do im=1,nmodel
         do k=1,nyin
            obgm(im)=obgm(im)+obs(k,id)*gcm(im,k,id)
         end do
      end do

      do im=1,nmodel
         obgm(im)=obgm(im)/float(nyin)
      end do
...
c        write(6,*)'matrix=',gmgm
         call mtrx_inv(gmgm,gmgminv,nmodel)
         call solutn(gmgminv,obgm,beta,nmodel)
C== have xmean
      do im=1,nmodel
        gm(im)=0
        do k=1,nyin
...
c... have 1-D acc 
 ...
 1000 continue
      ...
      do iy=1,nyr
      iiy=1982+iy-1
c     write(6,777)iiy,'wgts=',betaout(iy,1),betaout(iy,2),betaout(iy,3),
c    &'acc=',accus(iy,1),accus(iy,2),
c    &accus(iy,3),accus(iy,4),accus(iy,5)
      enddo
      write(6,*)
      do iy=1,nyr
      iiy=1982+iy-1
c     write(6,777)iiy,'wgts=',betaout(iy,1),betaout(iy,2),betaout(iy,3),
c    &'rms=',rmsus(iy,1),rmsus(iy,2),
c    &rmsus(iy,3),rmsus(iy,4),rmsus(iy,5)
      enddo
      write(6,*)
c
C=== have s&t avg AC and RMS
      call acc_in_st(obs3d,mlrprd,accm(1),mdvs,nyr)
      call acc_in_st(obs3d,ewmprd,accm(2),mdvs,nyr)
      call acc_in_st(obs3d,cca3d,accm(3),mdvs,nyr)
      call acc_in_st(obs3d,cfs3d,accm(4),mdvs,nyr)
      call acc_in_st(obs3d,smlr3d,accm(5),mdvs,nyr)
c
      call rms_in_st(obs3d,mlrprd,rmsm(1),mdvs,nyr)
      call rms_in_st(obs3d,ewmprd,rmsm(2),mdvs,nyr)
      call rms_in_st(obs3d,cca3d,rmsm(3),mdvs,nyr)
      call rms_in_st(obs3d,cfs3d,rmsm(4),mdvs,nyr)
      call rms_in_st(obs3d,smlr3d,rmsm(5),mdvs,nyr)

      write(6,888)'MEAN ACC=',accm(1),accm(2),accm(3),accm(4),accm(5)
      write(6,888)'MEAN RMS=',rmsm(1),rmsm(2),rmsm(3),rmsm(4),rmsm(5)
      write(6,*)
      write(6,766)'ridging eps=',eps
...
CC== acc & rms in time domain
      ...
      write(6,988) wk1
      call acc_in_t(cca3d,verify,wk1,mdvs,nyr)
      write(6,*) 'CCA'
      write(6,988) wk1
      call acc_in_t(cfs3d,verify,wk1,mdvs,nyr)
      write(6,*) 'CFS'
      write(6,988) wk1
      call acc_in_t(smlr3d,verify,wk1,mdvs,nyr)
      write(6,*) 'SMLR'
      write(6,988) wk1
cccccccccccccccccccccccccccccccccccc
      ...
      write(6,*) 'simple ensemble avg'
      write(6,988) wk1
      call rms_in_t(cca3d,verify,wk1,mdvs,nyr)
      write(6,*) 'CCA'
      write(6,988) wk1
      call rms_in_t(cfs3d,verify,wk1,mdvs,nyr)
      write(6,*) 'CFS'
      write(6,988) wk1
      call rms_in_t(smlr3d,verify,wk1,mdvs,nyr)
      write(6,*) 'SMLR'
      write(6,988) wk1
