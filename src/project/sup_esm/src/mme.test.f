...
C. regression between the "obs" and the "fcsts" by cca, CFS and SMLR
C. nyin < nyr: cross validation; nyin = nyr: no cross validation.
C===========================================================
      PARAMETER (ltime=500,nyr=50,nyin=49,nmodel=3,nfcst=nmodel+2)
      real obs(nyin)
      real cca(nyin),cfs(nyin)
      real sml(nyin)
      real gcm(nmodel,nyin),beta(nmodel)
      real acc(nyr,nfcst)
      real rms(nyr,nfcst)
      real accm(nfcst)
      real rmsm(nfcst)
      ...
      real mlrprd(nyr),ewmprd(nyr)
      real ccaprd(nyr),cfsprd(nyr)
      real smlprd(nyr)
      real verify(nyr)
c
...
C=== have signal data with stdv=sigma_s
C randomly make composite
      DO is=1,nsmp
c have random #
      nsds=(is-1)*nyr+1
      nsde=is*nyr
      ira=0
      do i=nsds,nsde   !seeds# = nyr
c        print*, i
         ira=ira+1
         ra(ira)=ran(i)
         indx(ira)=ira
c        print*, indx(ira),ra(ira)
...
C===  verify against each case
      ...
      write(6,788)'year','ECHM','GFDL','NSPP','CON','ENS',
     &'ECHM','GFDL','NSIP'
      do 1000 icase=1,nyr
...
c
      ...
      call timespaceanom(obs3d,mdvs,nyin,stclim1)
      call timespaceanom(cca3d,mdvs,nyin,stclim2)
      call timespaceanom(cfs3d,mdvs,nyin,stclim3)
      call timespaceanom(smlr3d,mdvs,nyin,stclim4)
      end if
