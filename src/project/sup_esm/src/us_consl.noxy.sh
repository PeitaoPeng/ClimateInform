...
#cp $lcdir/us_consl.noxy.4mdl.f    $TMPDIR/regr.f
cp $lcdir/us_consl.noxy.f    $TMPDIR/regr.f
cp $lcdir/lubksb.f  $TMPDIR/.
cp $lcdir/ludcmp.f  $TMPDIR/.
cd $TMPDIR

#
f77 -o regr.x regr.f lubksb.f ludcmp.f
echo "done compiling"
...
#ln -s $datadir/2mt.wmoanom.jfm.1982-2006.txt         fort.10
 ...
 ln -s $datadir/skill_spind_t2m_jfm_1982-2006.i3e     fort.50
#
ln -s $datadir/t2m_obs_and_prd.noxy.dat             fort.60
 
regr.x>$lcdir/out.noxy
#
#
# convert data into grads format
#

cat>$datadir/skill_spind_t2m_jfm_1982-2006.ctl<<EOF
dset $datadir/skill_spind_t2m_jfm_1982-2006.gr
dtype station
options sequential
stnmap $datadir/skill_spind_t2m_jfm_1982-2006.map
undef 10e10
title ac_skill
...
f77 -o cd_2_grads.x cd_2_grads.f
   ...
   ln -s      $datadir/skill_spind_t2m_jfm_1982-2006.i3e         fort.11
#
   ln -s      $source1/convert/data_102/lalo102       fort.12

#
   ln -s      $datadir/skill_spind_t2m_jfm_1982-2006.gr    fort.55

   cd_2_grads.x

 stnmap -i $datadir/skill_spind_t2m_jfm_1982-2006.ctl       #create map file
done
