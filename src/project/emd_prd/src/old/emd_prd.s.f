C*****************************************************************
      SUBROUTINE EMD_IMF(LL,LE,ximf,rem,spmax,spmin,
...
c-------------------------------------------------------------
c
c     call dampend(ximf,LEX)
      call dampend(ximf,LE)
c
...
C  IMF
c     DO i=1,LEX           !initialize ttmp1 for calculate SD
      DO i=1,LE            !initialize ttmp1 for calculate SD
          ttmp2(i)=ximf(i)
      ENDDO
c
      ...
      DO II=1,30
c
c       call min_max(LEX,ximf,spmax,spmin,nmax,nmin)     
        call min_max(LEX,LE,ximf,spmax,spmin,nmax,nmin)     
c     write(6,*) 'ximf=',ximf
        write(6,*) 'finished min_max'
c
c  obtain the cubic spline envelope
c
        call splint(spmax,LEX,LE,nmax)
        call splint(spmin,LEX,LE,nmin)
        write(6,*) 'finished spline'
        ...
        enddo
c
      do i=1,LE    !pass data from h(k) to h(k-1) and feed h(k)
        ttmp1(i)=ttmp2(i)
        ttmp2(i)=ximf(i)
      enddo
c     write(6,*) 'ttmp1=',ttmp1
c     write(6,*) 'ttmp2=',ttmp2
      nmc1=nmc2
      nmc2=nmax
...
c
      ...
      ENDDO
  888 continue
      write(6,*) 'II=',II,' nmax=',nmax,' nmin=',nmin,' neq=',neq
c================================================================
...
c  calculate the remainder
      ...
      call hilbert(ximf,LEX,yimf,LXY,LL,LE)
...
c
      do i=2,LL-1
        dximf=(ximf(i+LL+1)-ximf(i+LL-1))/1.047
        dyimf=(yimf(i+1)-yimf(i-1))/1.047
        U2=ximf(i+LL)*ximf(i+LL)+yimf(i)*yimf(i)
        omega(i)=(ximf(i+LL)*dyimf-yimf(i)*dximf)/U2
      enddo
      omega(1)=omega(2)
      omega(LL)=omega(LL-1)
c     call smooth(omega,LL,101)
c================================================================
      return
      end
...
C*******************  extension  *******************************
c
      subroutine end_ext(LL,LE,tavegl,ximf)
#include "parm.h"
c
...
c
      do i = 1, next
        data1(i) = tavegl(i)
c       data2(i) = tavegl(LXY-next+i)
        data2(i) = tavegl(LL-next+i)
      enddo
...
c     do i=1,LXY
      do i=1,LL
c       ximf(i)= -tavegl(LXY-i+1) + 2.0*leftRef
        ximf(i)= -tavegl(LL-i+1) + 2.0*leftRef
      enddo
c     do i=1,LXY
      do i=1,LL
c       ximf(i+LXY)=tavegl(i)
        ximf(i+LL)=tavegl(i)
      enddo
c     do i=1,LXY
      do i=1,LL
c       ximf(i+LXY*2)= 2.0*rightRef - tavegl(LXY-i+1)
        ximf(i+LL*2)= 2.0*rightRef - tavegl(LL-i+1)
      enddo
...
c
      ...
      is = LE - NDAMP + 1
      do i=is,LE
        ximf(i)=(ximf(i)-aend)*float(LE-i+1)/float(NDAMP)
        ximf(i)=ximf(i)+aend
      enddo
...
C****************** min_max ***************************************
C
      subroutine min_max(LEX,LE,ximf,spmax,spmin,nmax,nmin)
C
...
C********************** hilbert *******************************
C
      SUBROUTINE HILBERT(X,N,Y,M,LL,LE)
c  Calculate the imagine part of Hilbert Transform. The method
